...
for i in $(seq 2 $NO_LINES); do  # add the job id for all jobs in the user's queue to JOB_ARRAY. Exclude the first line
    echo $(sed "${i}q;d" queue | cut -c 12-18) >> job_array
done

sort job_array | uniq > job_array_uniq # get rid of duplicates

JOB_ARRAY=()  # intialize job array
ENTRIES=$(wc -l < job_array_uniq)  # find the number of lines in queue
for i in $(seq 1 ${ENTRIES}); do  # add the job id for all jobs in the user's queue to JOB_ARRAY. Exclude the first line
    JOB_ARRAY+=($(sed "${i}q;d" job_array_uniq | cut -c 1-7))
done
...
for JOB_ID in ${JOB_ARRAY[@]}; do  # look at all running jobs
    ...
    if [ -e "log_${JOB_ID}_prev" ]; then  # check for the existence of the file log_${JOB_ID}_prev
        :  # if it exists, do nothing
    else
        echo 'Placeholder' > log_${JOB_ID}_prev  # if it doesn't exist, make a place holding log_${JOB_ID}_prev file
    fi
