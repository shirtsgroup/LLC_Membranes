...
#NOTES: Perpendicularization of the outer layer is set to half the normal, not the full (509)
      # --> Works when no_layers is low, otherwise junctions must be extra spread
      # The inner ring has 2 more layers than the outer ring (41)
...
for i in range(0, no_layers_inner):
    x_old_center = (positions_inner[i][24][0] + positions_inner[i][25][0])/2.0
    y_old_center = (positions_inner[i][24][1] + positions_inner[i][25][1])/2.0
    z_old_center = (positions_inner[i][24][2] + positions_inner[i][25][2])/2.0
    ...
    y_new_center = (positions_perp_inner[i][24][1] + positions_perp_inner[i][25][1])/2.0
    z_new_center = (positions_perp_inner[i][24][2] + positions_perp_inner[i][25][2])/2.0
