# This script creates one arm of the BCC phase.
...
parser.add_argument('-mi', '--monomers_inner', default = 5, type = int, help = 'Number of monomers in inner ring at limiting radius')
parser.add_argument('-mo', '--monomers_outer', default = 12, type = int, help = 'Number of monomers in outer ring at limiting radius')
parser.add_argument('-w', '--pore_width', default = 8, type = float, help = 'Pore width')
...
# The rotation matrix will end up translating, not only rotating, the monomers. This next loop stores the original (before
# rotation) coordinates, and translates the monomers back to them while keeping them perpendicular to the parabola. ex
for i in range(0, no_layers):
...
# This next section will take care of the outer ring. It will essentially do the exact same thing, except that the monomers
# will face the opposite direction. Additionally, they will be translated from the origin farther, to establish the pore
...
for j in range(0, no_layers):
    no_monomers_inner = len(positionsrot_inner[j])
    no_monomers_outer = len(positionsrot_outer[j])
    for k in range(0, 5):
        for i in range(0, no_atoms - no_ions):
            print'{:5d}{:5s}{:>5s}{:5d}{:8.3f}{:8.3f}{:8.3f}'.format(count_monomer, name, identity[i], count_atom, positionsrot_inner[j][k][i][0] / 10.0, positionsrot_inner[j][k][i][1] / 10.0, positionsrot_inner[j][k][i][2] / 10.0)
            count_atom += 1
        count_monomer += 1
    for k in range(0, 12):
        for i in range(0, no_atoms - no_ions):
            print'{:5d}{:5s}{:>5s}{:5d}{:8.3f}{:8.3f}{:8.3f}'.format(count_monomer, name, identity[i], count_atom, positionsrot_outer[j][k][i][0] / 10.0, positionsrot_outer[j][k][i][1] / 10.0, positionsrot_outer[j][k][i][2] / 10.0)
            count_atom += 1
        count_monomer += 1
for j in range(0, no_layers):
    no_monomers_inner = len(positionsrot_inner[j])
    no_monomers_outer = len(positionsrot_outer[j])
    for k in range(0, 5):
        for i in range(no_atoms - no_ions, no_atoms):
            print'{:5d}{:5s}{:>5s}{:5d}{:8.3f}{:8.3f}{:8.3f}'.format(count_monomer, identity[i], identity[i], count_atom, positionsrot_inner[j][k][i][0] / 10.0, positionsrot_inner[j][k][i][1] / 10.0, positionsrot_inner[j][k][i][2] / 10.0)
            count_atom += 1
            count_monomer += 1
    for k in range(0, 12):
        for i in range(no_atoms - no_ions, no_atoms):
            print'{:5d}{:5s}{:>5s}{:5d}{:8.3f}{:8.3f}{:8.3f}'.format(count_monomer, identity[i], identity[i], count_atom, positionsrot_outer[j][k][i][0] / 10.0, positionsrot_outer[j][k][i][1] / 10.0, positionsrot_outer[j][k][i][2] / 10.0)
            count_atom += 1
            count_monomer += 1
