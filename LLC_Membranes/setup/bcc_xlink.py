...
class System(Topology):
    ...
    def __init__(self, initial_configuration, residue, dummy_residue, dummy_name='dummies.gro',
        ...
        self.c1_atoms = [a.index for a in self.t.topology.atoms if a.name in self.xlink_residue.c1_atoms and
                         a.residue.name == self.xlink_residue.name]
        self.c2_atoms = [a.index for a in self.t.topology.atoms if a.name in self.xlink_residue.c2_atoms and
                         a.residue.name == self.xlink_residue.name]
    ...
    def select_eligible_carbons(self):
        ...
        if self.radicals:
            ...
            bonds = self.bond_filter(eligible_c1_rad, eligible_rad, nreact)
    ...
    def bond_filter(self, eligible_c1, eligible_c2, nreact):
        ...
        :param eligible_c1: c1 atoms to bond
        :param eligible_c2: c2 atoms to bond. This list determines the number of bonds based on the percent
        :param nreact: approximate number of bonds to form. This number is adjusted after eliminated same-chain carbons

        :type eligible_c1: np.ndarray
        :type eligible_c2: np.ndarray
        :type nreact: int
        """

        percent = nreact / len(eligible_c2)

        # prevent adjacent carbon atoms from bonding
        keep = []
        for i, x in enumerate(eligible_c1):
            if not self.check_pair(x, eligible_c2[i]):
                keep.append(i)

        eligible_c1 = eligible_c1[keep]
        eligible_c2 = eligible_c2[keep]

        # narrow list to bottom percent of distances
        # redetermine nreact based on number of eligible carbons _after_ above loop
        nreact = int(len(eligible_c2) * percent)
        bond_c1 = (eligible_c1[:nreact] + 1).tolist()  # convert indices to serial
        bond_c2 = (eligible_c2[:nreact] + 1).tolist()

        # do not allow a carbon to bond twice (prioritizes radical reactions)
        keep_c1 = np.sort(np.unique(bond_c1, return_index=True)[1])
        bond_c1 = np.array(bond_c1)[keep_c1]
        bond_c2 = np.array(bond_c2)[keep_c1]

        keep_c2 = np.sort(np.unique(bond_c2, return_index=True)[1])
        bond_c1 = bond_c1[keep_c2]
        bond_c2 = bond_c2[keep_c2]

        # remove excess bonds. i.e. if c1 bonds with c2 on another chain and c1 from other chain bonds with c2 on
        # original chain. Could move this earlier to get a number of bonds more consistent with args.percent
        rm_excess = []
        for i, b1 in enumerate(bond_c1):
            for j, b2 in enumerate(bond_c2):
                if self.check_pair(b1 - 1, b2 - 1):
                    rm_excess.append(i)

        bond_c1 = np.delete(bond_c1, rm_excess)
        bond_c2 = np.delete(bond_c2, rm_excess)

        return bond_c1.tolist(), bond_c2.tolist()
    ...
    def bond(self):
        ...
        for i, x in enumerate(self.bond_c1):
            ...
            c1_name = self.xlink_residue_atoms.name[x - 1]
            c2_name = self.xlink_residue_atoms.name[self.bond_c2[i] - 1]
    ...
    def update_lists(self):
        ...
        self.c1_atoms = [x for x in self.c1_atoms if x not in self.terminate]
        self.c2_atoms = [x for x in self.c2_atoms if x not in self.terminate and x not in self.radicals]
