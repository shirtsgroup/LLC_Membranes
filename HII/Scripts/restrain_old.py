...
if __name__ == "__main__":
    ...
    if args.novsites:
        if args.bcc:
            file_rw.write_assembly(args.monomer, args.out, rings, bcc=True)  # there will be an error if the second argument is 'on' with
                                                            # no virtual sites
        else:
            file_rw.write_assembly(args.monomer, args.out, rings)  # there will be an error if the second argument is 'on' with
                                                            # no virtual sites
    else:
        if args.xlink or args.append:  # don't re-write the topology file if you are iteratively crosslinking
            pass
        else:
            file_rw.write_assembly(args.monomer, args.out, rings, xlink=True)
