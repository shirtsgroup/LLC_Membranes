...
# Confusing algorithm follows: If a, b and c are states, 0.1, 0.4 and 0.5 are their probabilities respectively,
# i.e. initial_prob = [0.1, 0.4, 0.5] then we can visualize each state existing on the number line:
# |-0.1-|----0.4----|-----0.5-----|
# 0----0.1---------0.5------------1  (cumulative total)
# If the probability in probstate for this iteration is 0.7 then you subtract 0.1 from 0.7 to get a new probstae of 0.6
# Next iteration, probstate is 0.6 and initial_prob is 0.4. So you subtract 0.4 from 0.6 and get 0.2.
# Next iteration, probstate is 0.2 and initial_prob is 0.5. 0.2 < 0.5 so the particle will be in state c
# This gives the desired distribution of states as Nparticles --> infinity
for n in range(0, Nparticles):
    for i in range(0, Nstates):
        if probstate[n] < initial_prob[i]:
            ...
            probstate[n] -= initial_prob[i]  # see explanation above this block of code
