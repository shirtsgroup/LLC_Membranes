...
c1 = [1153, 2219, 2919, 3756, 4685, 6740, 7288, 9236, 13209, 13331, 13727, 14275, 14564, 15797, 17563, 19237, 19633, 19648, 19922, 20318, 20455, 22388, 22525, 23728, 23895, 24032, 24291, 26224, 27305, 27427, 28675, 29893, 30197, 31126, 31141, 31537, 32237, 32511, 36088, 36362, 37321, 37991, 39072, 40990, 42390, 43745, 44171, 45100, 49484, 50610, 50884, 51143, 51569, 52117, 53183, 53746, 53883, 54446, 55405, 55816, 56075, 56775, 57186, 58130, 58937, 60474, 61266, 61296, 62392, 63336, 63869, 65406]
c2 = [18140, 34702, 19632, 3740, 4699, 40989, 8124, 8413, 29907, 62939, 45403, 13452, 47017, 65405, 18399, 19206, 2918, 18810, 36468, 20165, 51979, 21565, 22509, 23742, 6906, 40745, 40471, 27030, 26497, 10742, 60199, 63883, 62787, 30470, 30333, 14685, 64857, 31703, 53075, 37168, 37305, 38005, 39086, 6739, 59103, 43759, 11001, 62117, 16892, 163, 34139, 50335, 18414, 52101, 53197, 53593, 53045, 5232, 55374, 39101, 6221, 56759, 57992, 7454, 25812, 10057, 12523, 11701, 45677, 64172, 14822, 15796]
...
for i in range(atoms_index + 2, atoms_count):
    ...
    if int(b[i][0:5]) in c1:
        b[i] = b[i][0:5] + b[i][5:10].replace('c2', 'c3') + b[i][10:len(b[0])]
    elif int(b[i][0:5]) in c2:
        b[i] = b[i][0:5] + b[i][5:10].replace('ce', 'c3') + b[i][10:len(b[0])]
