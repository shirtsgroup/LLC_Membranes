...
\begin{document}
  ...
  %MRS2: an undesired, or all of the undesired?
...
%    \item Most are prone to defects in the crystalline structure.
  ...
  %MRS2: should be a citation here since you make specific claims.
  ...
  %MRS2: making a positive statement rather than negative, clearer
  %lack an appreciable 
  have a very narrrow
...
%  \cite{donnan_theory_1995}.
  ...
  % BJC: should show figure with HII and QI monomer structures
...
%MRS2: Explicitly state here that the first two objectives are completed, and that the others are underway.
  \begin{enumerate}
    ...
    %MRS2: Objectives shouldn't include a description of what has and hasn't been done.  Then it's not an really just an objective anymore.  I would suggest rephrasing these in terms of the intellectual objective, focusing on the questions to be asked and hypotheses to be tested, and then talk in the next section about what has been done (or is to do).
...
%MRS: overall, good description of wha has been done.  You can cut back a little if space is an issue.  Give the paper citations with the points. 
  ...
  %MRS2: provide paper citation here, rather than at end of section.
  ...
  time.
  %MRS2: above: less detail on what you did (for example, 400 ns of equilibration); focus on the point of the experiments (whatt hypothesis was tested) and what the results are.  committee members are going to be less interested in the details than the intellectual framework being used.
  \begin{figure}[!htb]
      \centering
...
%  \end{itemize}
 ...
 \end{wrapfigure}
  ...
  %MRS2: put at the beginning it's been submitted.
...
%    data from the solute trajectories.
  ...
  %MRS2: not sure.  I think there are ways to clear ou space (see above)
...
%  \end{wrapfigure}
  ...
  since they are less electronegative elements.~\cite{biswal_hydrogen_2015} % citation
  ...
  %MRS2: nobody has built an atomistic model, I think - the self-assembly were for very GC models.
  Instead, we used known
  %MRS2: may want to emphasize that for atomistic models, ``long'' is probably at least 2 orders of magnitude longer than our simulations (which are already long)
...
%MRS2: details of how are not that important; focus on what experiments you will do and why.
