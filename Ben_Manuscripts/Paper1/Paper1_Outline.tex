...
\begin{document}
	...
	%MRS3: Take a pass at reprhasing this -- important to get this right. don't start with a preposition.
        %MRS4: maybe ``consistent with physical observations?''
...
%MRS4: I'm not quite sure what ``pores are rotated'' means.
	...
	Proper selection of initial structural parameters results in an equilibrated model that is 
%MRS4: change to be more specific.
consistent with several key experimental measurements.
...
%MRS3: what is the thesis of the figure -- what is it trying to show.  Important when analyzing the outline and deciding what it means.  Make that the caption.
		...
		\end{itemize}
%MRS4: seems like an awful lot to put into one figure, even with multiple panels.
...
%MRS4: I think we will need to be much more specific about how the simulated x-ray diffraction pattern matches experimental masurement, and the different features that do match.  	
...
%MRS4: question: are the monomers themselves in a helical shape (not just the head groups?)
