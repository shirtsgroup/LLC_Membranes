...
\begin{document}
	...
	\end{itemize}
%MRS: don't start with the models.  Instead something like, we wish to obtain a clearler picture of the microscopic struture, by building a model. Focus should always be on the physical things we are trying to study.
	...
	\begin{itemize}
		...
		\item How many monomers are in a layer?
                %MRS: try to rewrite this the way you will in the paper -- set up the question in the introduction, and then decribe what is going on.  Raise the question, but don't answer it here.
	...
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
        %MRS: this is not the best way to phrase the thesis.  I would say that we want to ask what the most likely features of the membrane are using molecular modeling.  we can talk about the phrasing.  But the model should not be the scientific output of the paper. Also, ``physically accurate'' is a high bar - it's probably not more than physically realistic.
	...
	\begin{itemize}
		...
		\item Observations of transport mechanism
                  %MRS: probably not going to make it to transport of water; validation of ion conductivity 
		\item The model is consistent with experiments and will be used in solvent/solute flux and solute rejection predictions
                 %MRS: these are results. And the focus should not also be on what you will do in the future - the focus will be on what can be read here.  The introduction should focus on raising the questions that will be answered by the results. 
	\end{itemize}
	
%MRS: call it methods here.
	...
	\begin{itemize}
		...
		%BJC: Openeye software to get the best charge for multiple configurations.
                %MRS: should all be with the same set of charges, so its the same model.     
...
%MRS: outside resonable for atomistic simulations.
	\begin{itemize}
		...
		\end{itemize}  
                %MRS: features of the code not that interesting (unless making it available for others.
	...
	\end{itemize}
        ...
        \begin{itemize}
                %MRS: this next two sentences below sort of goes w/o saying, I don't know that it needs to be emphasizd.
	...
	\end{itemize}
        ...
        %MRS: important - don't present your work chronologically, but present it to convince people of your theses.
	...
	\end{itemize}
        ...
        %MRS: be careful to be explicit what you mean by parameters here; people might assume FF parameters.
	...
	%MRS: not the right thesis -- focus on the physical constraints put on the membrane by the model.
...
%BJC: This seems dark and unencouraging but I think it's true. Maybe I shouldn't talk about it for now.
%MRS: I think the second point can be made without necessarily mentioning the first one for now.
        \end{itemize}	
