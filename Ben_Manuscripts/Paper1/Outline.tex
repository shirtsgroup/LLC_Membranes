...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Both have reported stacking distances near 3.7 \angstrom % Reference
...
%     \item This difference may explain, in part, which stacking mode is more favorable.
  ...
  % This can be cut down if C and D prove to be no help. 
  ...
  \begin{itemize}
      ...
      \begin{itemize}
          ...
          %\item Final structure strongly influenced by monomer configuration used to build 
      \end{itemize}
      ...
      \begin{itemize} 
          ...
          \item The system is simulated at 335K, close to its isotropic transition temperature, for 200 ns. % BJC: not entirely sure if it needs to be an already equilibrate structure
...
%  \end{itemize}
  ...
  that they were asked.  %BJC: Not a fan of this
  ...
  % BJC: Even though I will shift focus to only 5 monomers per layer, I can put other data
  ...
  \bottomrule
  \end{tabular}
  \caption{The pore spacing (given in $\nm$) of the model increases as
  ...
  closest to the experimental 4.12 $\nm$~}\label{table:p2p} 
  \end{table}
...
%  \end{figure}
  ...
  with layers stacked 4.0 $\nm$ apart and to our surprise, we observed
  ...
  \begin{itemize}
	...
	structure.
        ...
        the OP Basin (open pore) (Figure~\ref{fig:abasin}).
	...
	hydrophilic regions.
	\item This will be called the CP Basin (closed pore) (Figure~\ref{fig:bbasin}).
...
%  \end{figure}
  ...
  configurations in the OP or CP basins.
  ...
  % Three paragraphs instead here? Talk about disordered, parallel displaced and 
  % layered separately? Try it. 
  % Talk about disordered last so it leads in to 
  % Also maybe don't talk about question 4 until after this
  ...
  \begin{itemize}
        ...
        in experiment. The rings prefer to stack $\approx 4.1 \angstrom$ apart as % update distance if needed
...
%	equilibrated disordered phase}\label{fig:XRDsim}
 % \end{figure}
  ...
  The spots that appear in the simulated XRD pattern of the OP basin
  ...
  \begin{figure}[!ht]
	...
	%BJC: They are from the short run, but as I comment above, I need to 
	% replace them with the correct figures
