...
%MRS: Below is a very vague statement  Which steric effects? which electrical effects?  Expand this.
...
%MRS: may want separate out structural issues from mechanistic issues. 
	\item How many monomers are in a layer?
%MRS: is this next one important?
	...
	\end{itemize}
%MRS: but I don't think the paper will address these -- so perhaps this would go into a discussion section?
...
%MRS: lay out which questions will be answered, and what the experiments will be that test them.
Other things to be studied:
\begin{itemize}
	\item Monomers per layer
	\begin{itemize}
		\item Questions: How many monomers are in each layer? 
%MRS: need to make it clearer why the questions of monomer number is important. 
%MRS: fixed-charge models may underpredict pi-stacking (could use a reference on this).
Do we observe pi stacking of benzene rings as hypothesized in papers? Are there even defined layers?
	...
	\end{itemize}
%MRS: IIRC, self diffusion usually only refers to pure solvent diffusion.
...
%MRS: Make it clear how the different mechanisms might affect the transport rates.
%MRS: also, check with Drew on scripts to measure these timescales.
	\end{itemize}
	\item Diffusion of water in pores
	\begin{itemize}
%MRS: probably no diffusion in the hydrophobic matrix -- probably not that interesting a question (obviously you will check, but we expect it not to move there.
	...
	\end{itemize}
%MRS: should be measuring conductivity, since we have experimental measurements?
\end{itemize}
