...
%  \end{itemize}
  ...
  \begin{itemize}
	...
	%BJC: okay, need to run some calculations here.
  ...
  \begin{itemize}
	...
	%BJC: added sentence to beginning of paragraph
        \item We will refer to this large set of configurations, with an open helical pore, as Basin A (Figure~\ref{fig:abasin}).
        %\item We will refer to this system as
        %MRS: maybe Basin O and Basin C for open and closed?
	%BJC: That works but 'capital oh' might get confused with zero
	...
	studying the system which actually dominates.  %MRS: remind people why.
  \end{itemize}
  ...
  pi-stacking modes.
  \begin{itemize}
  	\item We generated simulated X-ray diffraction patterns using simulation trajectories from the highly
...
% Supplemental VVV
  ...
  \begin{itemize}
	...
	%BJC: There aren't spots in parallel displaced and the distribution does not have the spikes that the sandwiched configuration does. At this point in the paper, I've tried to say that parallel displaced configuration is not consistent with experiment and I focus the rest of the paper on the sandwiched configuration
  \end{itemize}
  ...
  We have used a detailed molecular model of the Col\subscript{h} phase formed by NA-GA3C11
