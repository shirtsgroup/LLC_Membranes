...
for i, res in enumerate(sol):
        ...
        ax1.bar(bar_locations[i] - bar_width/2, np.mean(powercut_alpha), bar_width, label=names[res], edgecolor='black', color=colors['powercut_alpha'], alpha=opacity, yerr=np.std(powercut_alpha))
        ax2.bar(bar_locations[i] + bar_width/2, np.mean(powercut_lambda), bar_width, label=names[res], edgecolor='black', color=colors['powercut_lambda'], alpha=opacity, yerr=np.std(powercut_lambda))
