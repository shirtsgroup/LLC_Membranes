...
""" Results of MSD comparison by training on only half the solute trajectories
"""
...
def initialize():
	...
	parser.add_argument('-p', '--portion', default='first', help='front or back')
...
def md_diffusivity():
	...
	MD_MSD = Diffusivity('%s/%s' % (directory,traj), '%s/%s' % (directory, gro), 'z', begin=first_frame, residue=res, solute_indices=solute_indices)
	...
	MD_MSD.t = None
	MD_MSD.com = None
	file_rw.save_object(MD_MSD, '%s_msd_train_%s.pl' % (res, args.portion.lower()))
...
solute_indices = [15, 7, 9, 1, 4, 0, 5, 17, 10, 3, 23, 12]
solute_indices = [2, 6, 8, 11, 13, 14, 16, 18, 19, 20, 21, 22]
solute_indices = np.random.choice(24, size=12, replace=False).tolist()
#solute_indices = np.arange(24).tolist()
#if args.portion.lower() == 'second':
#	solute_indices = np.arange(12).tolist()

endframe = 2000  # 1000 ns

nmodes = args.nmodes 
ntraj = 100  # number of trajectories to simulate
nboot = 10  # number of bootstrap trials when getting errorbars on MSD
...
if recalculate_msd:
	...
	try:
		MD_MSD = file_rw.load_object('%s_msd_train_%s.pl' % (res, args.portion.lower()))
