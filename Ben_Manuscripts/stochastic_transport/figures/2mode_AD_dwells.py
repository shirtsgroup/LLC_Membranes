...
for i, res in enumerate(sol):
        ...
        ax1.bar(bar_locations[i] - bar_width/2, -np.mean(powercut_alpha1), bar_width, label=names[res], edgecolor='black', color=colors['powercut_alpha'], alpha=opacity, yerr=np.std(powercut_alpha1))
        ax2.bar(bar_locations[i] + bar_width/2, -np.mean(powercut_lambda1), bar_width, label=names[res], edgecolor='black', color=colors['powercut_lambda'], alpha=opacity, yerr=np.std(powercut_lambda1))
        ...
        ax1.bar(bar_locations[i] - bar_width/2, np.mean(powercut_alpha2), bar_width, label=names[res], edgecolor='black', color=colors['powercut_alpha'], alpha=opacity, yerr=np.std(powercut_alpha2))
        ax2.bar(bar_locations[i] + bar_width/2, np.mean(powercut_lambda2), bar_width, label=names[res], edgecolor='black', color=colors['powercut_lambda'], alpha=opacity, yerr=np.std(powercut_lambda2))
...
#hatch1 = mpatches.Patch(facecolor=colors['powerlaw_alpha'], label=r'$P(\mathbf{\alpha})$', edgecolor='black')
