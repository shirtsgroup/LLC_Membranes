...
%  \end{figure}
  ...
  in order to calculate the flux (section~\ref{method:mfpt}) of solutes given model 
  ...
  accurate than sFBM and sFLM. The 2-mode model has a broken correlation structure, so 
...
%  \end{subfigure}
  ...
  %BJC5: added this
  It is only necessary to simulate 1000 realizations of each model in order to
  parameterize Equation~\ref{eqn:flux_decay}. We used 10000 trajectory realizations 
  in order to generate Figure~\ref{fig:flux_curves}, however, in Figure~\ref{S-fig:flux_curve_sensitivity}
  ...
  Since the computational cost of the MSDDM is greater than the AD model, we 
  generated flux curves using only 1000 realizations at each pore length.
