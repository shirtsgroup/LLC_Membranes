...
%  additional solute trajectories. 
  ...
  urea and acetic acid, since their trajectories appear stationary. In 
  Figure~\ref{fig:ad_eyetest}, we show that realizations of the AD model appear
  qualitatively similar to those seen in our MD simulations (Figure~\ref{fig:solute_trajectories}).
  ...
  \caption{Simulations of 1 mode AD model trajectories for Urea display 
  qualitatively similar hopping and trapping behavior to that shown in 
  Figure~\ref{fig:solute_trajectories}. Analogous plots are shown for other solutes
  %BJC5: todo: add plots to SI. Show I add a 2 mode model plot?
  and the 2 mode mdoel in the Supporting Information. Dwell times may be exaggerated in the
  sFBM and sFLM models because the power law dwell time distributions have infinite
  variance.}\label{fig:ad_eyetest}
  \end{figure}
...
%  length distribution, favoring longer dwell times or both.
  ...
  This brief qualitative analysis suggests two shortcomings of the AD model. First, 
  positional anti-correlation may dissipate after a sufficiently long time lag, dependent on the 
...
%  by longer simulations.
  ...
  diffusion models agree well with MD-generated data when using an exponential cutoff
  on the dwell time distribution (sFBMcut and SFLMcut). When we do not apply an 
  exponential cut-off (sFBM and SFLM), MSDs are underpredicted. Drawing hops from a
  truncated L\'evy stable distribution yields MSDs similar to when hops are drawn 
  from Gaussian disributions (sFLM and sFLMcut).}\label{fig:anomalous_msds_1mode}
  \end{figure}
...
%  Therefore, we can use the current parameters to understand solute dynamics.
  ...
  glycol is has a slight preference for the pores likely because it is a larger
...
%  methanol spends 91 \% of its time in the tails likely due to its small size.
  ...
  which occur in the tails versus in the pores, however this would lead a broken 
...
%  time in the tails likely due to its small size.
  ...
  examination of Figure~\ref{fig:solute_trajectories} reveals that hops tend
  to be an accumulation of a series of hops in the same direction. All of the 
  hops in the MSDDM are negatively correlated which prevents this from happening.
  ...
  to poor parameterization. As stated earlier, most of a solute's motion is 
  ...
  urea's higher predicted MSD. Although we note that acetic acid's predicted MSD
  ...
  realations and MD simulation trajectories and it may be the source of the 
...
%  \end{figure}
  ...
  parameters extracted from MD simulations. The 1-mode sFBMcut mode generates predictions
  similar to the 1-mode sFLMcut model at a lower computational cost and is far more
  accurate than sFBM and sFLM. The 2-mode AD model has a broken correlation structure, 
  ...
  \end{subfigure}  
  \caption{(a) The distributions of passage times generated from the sFBMcut model
...
%  \end{subfigure}
  ...
  realizations. To be sure, we recommend simulating at least 1000 realizations. 
  ...
  dwell times ($\alpha_d$) or hop lengths ($\sigma$). The MSDDM is fundamentally similar to 
  ...
  itself as well as inaccuracies resulting from of simulation of very long FLM trajectories.
  ...
  $\sigma$ in trapped states. Since we add the transitional hop lengths to each end of
  trapped state sub-trajectories, the transitional hop lengths are shifted with respect 
  to one another largely decorrelating them. We tested this reasoning by modifying the 
  ...
  FLM. When we use FBM, $\beta$ increases well above the Brownian value. $\beta$ increases
  even further if we immobilize the trapped states. Thus the low value of $\beta$ of the
  MSDDM is a consequence of model assumptions and inexact simulation of FLM.  
  ...
  The values of $c$ are likely a complex function of $\sigma$, $alpha_d$, $H$ and $\alpha_d$
  where applicable. The values of $c$ scale the flux curves and are directly correlated to 
  flux magnitude. We showed earlier that $\beta$ is not a function of $\sigma$ or $\alpha_d$,
  therefore dwell times and hop lengths must influence $c$. In fact, any parameter which acts
  to modify the rate at which solutes move through the membrane pores will affect $c$.
  Therefore it is sensible to hypothesize that a higher degree of anti-correlation (lower $H$)
  ...
  \end{subfigure}
  \caption{(a) The c paramters are directly related to your mom (b) }\label{fig:c}
  \end{figure}
...
%  progress through the pores more slowly.
  ...
  selectively separating ethylene glycol from acetic acid.
...
%  We projected the long timescale behavior using this model. 
  ...
  portions of the MSDs predicted by our model fall within the 1-$\sigma$
  confidence intervals of MD. Aside from inaccuracies in parameter estimates,
  the most significant shortcoming of all models is their ability to 
  accurately portray the curvature of the MSD curves. The MD MSD curves tend
  to  become linear on long timescales, but the MSD curves generated by our
  models have persistent curvature that may cause them to under-predict
  the MSD on very long timescales.
  
  We demonstrated how one could used our models in order to to determine
  macroscopic flux and selectivity.
  %BJC5: I'll write the rest of this paragraph after I finish the MSDDM flux calculations.
