...
%    \item The hydrodynamic Ferry model is the simplest pore-flow model, with extensions such
%    as the Donnan-Steric Pore Model which includes extra terms for the pressure gradient
%    across the membrane and membrane charge.
%    \item Dischinger et al. created an empirical correlation to determine the
%    effect of solute physicochemical properties on solute rejection.
  \end{itemize}

%  \begin{itemize}
%    \item More recently, rapid advancements in computational power have made it feasible
%    to observe transport in simulated systems with atomic resolution. This allows us to
%    shift towards analysis of the motion of individual particles and gives an even more 
%    fundamental understanding of observed bulk properties.
%  \end{itemize}
  ...
  %BJC2: this paragraph will contain examples of highly selective separations

  %MRS3: I'd omit next line (That I commented out); you talk about membranes above, so better to start the next paragraph talking about membranes.
  ...
  \begin{itemize}
	...
	pores. They are easier to make.
        %MRS3: probably want to put a more here about why we are studing these membranes, and what advantages they might have, and why transport in them is complicated.  
  \end{itemize}
  
  %MRS3: One missing connection between the above information and what is in the paper would be making the connection between MD simulations
  %and mathematical modeling, using MD to provide/justify the fuctional form and/or some of the parameters for the model, 
  %rather than just experiment.

  %MRS3: a suggested tweak in language
  ...
  %BJC2: simple models from first paragraph fail to adequately describe transport.
  ...
  \begin{itemize}
    ...
    %MRS3: point out that they were all subdiffusive (unlike water)
  \end{itemize}
...
%  \end{itemize}
  ...
  %BJC2: moved this paragraph up from below to clarify novelty of this approach
...
%  	information), which suggests that there is another underlying subdiffusive mechanism.
  ...
  \begin{itemize}
	...
	likelihood function: 
    \begin{equation}
      %MRS: use \ln instead of ln or \text{ln} 
...
%    variables diverges." \cite{klages_anomalous_2008}  % BJC: reword
  ...
  \begin{itemize}
    ...
    \item It has the form: %BJC2: need to see if there is an analytical form for H
    %BJC2: might need to use a different way to estimate H (which is more standard): https://pypi.org/project/hurst/
  \end{itemize}
...
%  \end{itemize}
  ...
  % BJC2: still need a better name
  % name candidates - correlated MSMs, Dynamical mode switching
  % MRS3: probably not ``correlated MSMs'', that's insufficiently descriptive. Many things dynamically change modes.
  % Maybe ``Markov state-dependent dynamics?'' You have dynamics/transport that are a function of the state, which is 
  % determined by a Markov state model. 
...
%  	\end{enumerate}
  ...
  % BJC2: Moved the meat of this paragraph to previous section
  % since I'll be using subordinated levy motion.
  \noindent  % cauchy is a case of a Levy stable distribution. The tails are too heavy though.
%  \begin{itemize}
%    \item For independent and identically distributed (iid) random variables, the 
%    generalized central limit theorem guarantees convergence of the associated 
%    probability distribution function (PDF) to a L\'evy stable PDF. \cite{klages_anomalous_2008}
...
%  \end{itemize}
  ...
  %BJC2: I think it makes sense to show the data supporting that this is likely
  % sFBM here in the discussion. The methods would just point here.
  ...
  % BJC2: Adding all combinations of distributions.
  % Power law + Gaussian, Power law + Levy, Power law w/cutoff + Gaussian, Power Law w/cutoff + Levy
  % BJC2: Probably a good idea to at least mention results when no anitcorrelation is included
...
%	reduced	uncertainty MD MSDs.
	...
	% BJC2: Error looks relatively small now.
  \end{itemize}
  
  % BJC2: A qualitative comparison of sFLM trajectories to MD trajectories might be nice.
  ...
  % BJC: reduce this figure to solutes studied and put 3 bars for each solute: MD MSD, 
  % one-mode sfbm MSD, two-mode sfbm MSD.
  % BJC2: Actually I think I prefer showing the MSD curves on one plot, like in my research journal. Since it shows shape.
  % MRS3: OK.
  \begin{figure}
  \centering
  ...
  % BJC2: This section should parallel the previous section in terms of presentation of results.
  ...
  % BJC2: This figure is just 1 microsecond of data
  \begin{figure}[!h]
  \includegraphics[width=\textwidth]{emissions_MET.pdf}
  \caption{}\label{fig:emissions_MET}
  \end{figure}
  
  % BJC2: I need to look into the motion when a solute is trapped. It should have a very low H.
  ...
  % BJC2: Could explore a second model with an emission distribution for transitions in tails versus pores.

  % BJC2: FIGURE: autocorrelation of motion while trapped in a state and autocorrelation of emissions.
...
%  \end{itemize}
  ...
  %BJC2: TABLE. 
  %BJC2: Would probably be best to combine this with table from anomalous diffusion.
