...
% selelctivity since it's more results oriented. ``of varying complexity'' is unneeded in the tite
...
%MRS7: true, could just be ``predicting selectivity''
...
\begin{document}
  ...
  %BJC6: I could make abstract margins smaller
  \begin{abstract}
...
%  heavily influenced by the quality of insights gained by the modeling approaches above.
  ...
  %MRS6: It was also becasue they thought it was more likely to be interesting initially.  I'm in favor of 
  % cutting for simplicity. 
...
%  mechanistic insight by studying the parameters of each model. 
  ...
  %MRS7: definitely improved, I had a couple of comments to further refine.
  ...
  solute travels over time. Although theoretically possible, we do not use the 
  ensemble averaged MSD to calculate $\alpha$ because we can extract the same 
  parameter with higher precision using the dwell time distributions as described below.
...
%  We modeled solute motion as a CTRW subordinate to FBM, also called sFBM. 
  ...
  %BJC6: I could move these MLE equations to Supporting  
  %MRS7: for PRE, can leave in.
  \begin{equation}
  ...
  %We cannot observe dwell times longer than the length of a single trajectory. 
...
%  \end{equation}
  ...
  %BJC6: I could move the following to supporting and keep last sentence.
  %MRS7: could simplify down to saying that it's hard to estimate when H > 0.5, but you only have to deal with H < 0.5, so the difficulties are not relevant. 
  ...
  defined as less than 0.75 nm from any pore center. All else are in the second mode, the
  tail region. We determined this cut-off by maximizing the difference in dynamical behavior as 
 ...
 \end{table}
  ...
  %BJC6: added this. Not sure how much detail is necessary
  %MRS7: seems fine.
  ...
  the time-averaged MSD up to a 1000 ns time lag with corresponding 95\% confidence
  intervals. 
  %MRS6: somewhere you do talk about agreement to within 1 sigma
  %instead of 95%, so probably should check for consistance.
  ...
  % MRS6: suggest in the supporting information describe what measure was used to 
  % determine that the algorithm sufficiently and reasonably approxmiates.
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  %BJC6: could defer most of this to supporting.  
  %MRS7: yeah, supporting.  Should just be ``Verified the Markovianity of state transitions in MD as described in Section [X]. Generally, how one did verification of most things can be put in supporing, since that's not central to the argument. ''
...
%  simulation procedure described in Section~\ref{method:model_sFBM}.
  ...
  realizations, released at the pore entrance ($z=0$). 
  %BJC6: next two sentences might not be necessary  
  %MRS7: kind of seems relevant here.
  In the case of 
  ...
  %BJC6: could probably remove this paragraph.
  %MRS7: Maybe OK?
...
%MRS6: maybe more subsections in results so people can get a sense of where the flow is going at a glance?
  ...
  to the CTRW. We believe the subordinated process is FBM or FLM because their analytical
  correlation structures are close to those observed in our simulations (Figure~\ref{fig:hop_acf}).
  Fractional motion is common in crowded viscoelastic environments where movement is highly 
  ...
  %BJC6: ^^ Should I capitalize glycol?
  %MRS7: I'd probably just uncapitalize all. 
  \end{subfigure}
  \begin{subfigure}{0.45\textwidth}
  ...
  radial dependence. The longest periods of entrapment typically occur when 
  ...
  \end{subfigure}
  \caption{Using urea as an example, (a) the distribution of hop lengths fits well to 
  ...
  an exponential cut-off may better describe the probability of long dwell times in 
...
%  \end{figure}
  ...
  Figure~\ref{fig:hop_distribution_comparison}. 
  %BJC6: I say the next sentence in plot
  %MRS7: seems OK here, since important description of distribution.
  The L\'evy distribution does a better job capturing the somewhat heavy tails and 
  high density near 0 of the hop length distribution.
...
%  \end{figure}
  ...
  \end{subfigure}
  \caption{(a) Simulated urea trajectories generated by each variation on the 1 mode AD model 
  display qualitatively similar hopping and trapping behavior to that shown in Figure~\ref{fig:solute_trajectories}. 
  Dwell times are exaggerated in the sFBM and sFLM models because the power law dwell
  time distributions are not truncated and have infinite variance. In (b) and (c) we 
  compare additional trajectories simulated with the sFBMcut and sFLMcut models (blue) to
  MD solute trajectories (black).  
  %Analogous plots are shown for other solutes
  %MRS6: I don't think the comparison between the cut and no cut are that interesting for the main text.  Move to supporting?
  %MRS6: comparisons between modeled and actual trajectories side by side would be more interesting. 
  %BJC5: todo: add plots to SI. Show I add a 2 mode model plot?
...
%  only meaningful if the ensemble of solute trajectories is stationary.
  ...
  stabilized (Figure~\ref{S-fig:equilibration}). Nonetheless, we observe evidence of 
  ...
  differences in their MSDs. Urea and acetic acid appear to demonstrate satisfactory
  stationarity. The shapes of their MSDs curves are also very similar (see 
  ...
  %BJC6: These next two paragraphs are a bit awkward without the figure being present. Not sure what the best course of action is.
  %MRS7: I think generally OK.
  Given stationary data, the AD model has satisfactory predictive capabilities. In 
  ...
  equilibrated portion of the solute trajectories.%, doubling the data.
  
%  In most cases, 
%  the magnitude of the MSD predictions after a 
%  500 ns time lag are within or close to within error of the MD MSDs (for sFBMcut and
%  sFLMcut), but still appear to systematically under-predict the mean.  Longer simulations and more independent trajectories may
%  be necessary. 
  
  %BJC: say something about how more independent trajectories may not fix the problem
  % because there are different features of the MSD curves on different timescales.
  ...
  concluded that methanol and ethylene glycol trajectories may not yet be stationary, we 
  ...
  %BJC6: combined 1 and 2 mode models. Easier to compare between and takes up less space. maybe a little cluttered with acetic acid.
  %MRS7: I think OK
  \begin{figure}
  \centering
  \begin{subfigure}{0.45\textwidth}
  ...
  \end{subfigure}
  \caption{In most cases, MSDs generated from realizations of our 1 and 2 mode AD 
  models lie within or near the 1$\sigma$ confidence intervals of MD-generated 
  data. Drawing hops from a truncated L\'evy stable distribution (sFLMcut) yields
  MSDs similar to  when hops are drawn from Gaussian distributions (sFBMcut). 
  In most cases, the 1 mode simulated MSDs are under-predicted at long timescales
  because they show pronounced curvature which the MD MSDs lack. The 2 mode predictions
  show less curvature than the 1 mode MSDs because the hop correlation structure
  is broken every time a transition between tails occurs.
  ...
  in the tail region, parameterized by lower $\sigma$ values (smaller hops) for the normal and L\'evy stable 
  ...
  % when we use truncated power
  %laws to describe the dwell time distributions.
  ...
  However, they undershoot the mean MD MSD as the time lag increases. As 
  discussed in the previous section, long time positional anti-correlation, on
  the order of hundreds of $ns$, may not exist in the MD system which results 
  in a shift from sub-linear to linear MSD behavior. Acetic acid exemplifies
  this point. At first glance, acetic acid's predicted MSD appears to match the 
  ...
  The 2 mode models display curvature more consistent with MD but for non-physical
  reasons. The width of the hop distribution changes every time a switch between
  pores and tails occurs. We are unaware of an appropriate technique which can 
  ...
  Methanol spends $> 90\%$ of its time in the tails (see Figure~\ref{fig:AD_mode_occupation}),
  ...
  however we do not recommend this as it has no theoretical basis.
  ...
  % and can be correlated to physical behavior 
...
% BJC: made more to the point
%  In fact, methanol has the lowest
...
%  with tails are frequent. 
  ...
  %BJC6: Could delete this paragraph or move an abridged version to conclusions
  %MRS7: Good question.  Seemed OK when I was reading it. Summarizing what was found in this section.
  ...
  %BJC5: could remove this transitional paragraph
...
%  correlation structure similar to that seen in the 2 mode AD model.
  ...
  %BJC6: could delete this paragraph since described in methods
  %MRS7: yeahm could say you created one for each solute, as described in the methods.
...
% reordered this section to make the logic more clear  
  
  %Despite qualitative differences in trajectory realizations, 
  ...
  the magnitude of the MD MSDs within their 1$\sigma$ confidence intervals. One 
  could argue that the curves of ethylene glycol and urea are statistically 
  indistinguishable. However, it's possible that they may diverge on longer 
  timescales due to differences in their curvature. Similarly, methanol's MD MSD 
  is nearly linear while the MSDDM prediction is curving downward implying that it
  may underestimate the MSD on longer timescales. 
  ...
  its hop lengths ($\sigma$ too high). As stated earlier, most of a solute's 
  motion is due to displacements during state transitions. Acetic acid has a 
  transitional Hurst parameter close to urea's and a transitional $\sigma$ value
  that is higher than urea's. This suggests that the heavier tails of urea's transitional
...
%MRS6: even if the MSD is well predicted, if the trajectories look different, can it really be a useful model?  I.e. is one getting the right answer for the wrong reasons?  What does it mean if MSD's are good, but the dwell times are not great?
  ...
  Figure~\ref{fig:msddm_eyetest}. There is little evidence of trapping behavior or
  ...
  examination of the characteristic MD trajectories shown in Figure~\ref{fig:solute_trajectories} 
  ...
  Brownian motion. 
  %Note that 
  %the length of the realizations are equal to the length of the equilibrated 
  %portion of the MD trajectories for each solute.
  }\label{fig:msddm_eyetest}
  \end{figure}
  
  
% BJC6: I removed these because I think it doesn't add much. Could add a bit 
...
%  flux and selectivity.
  ...
  diffusion model and the MSDDM in order to demonstrate how one can use their 
  realizations in order to calculate the flux (see section~\ref{method:mfpt}) 
  of solutes given model parameters extracted from MD simulations. The 1-mode 
  sFBMcut model generates predictions similar to the 1-mode sFLMcut model at 
  a lower computational cost. We showed that the 2-mode AD model has a broken
  correlation structure, so we do not consider it here.
  ...
  The computational requirements of the MSDDM can be greater than 10 times longer. 
  For both models, the RAM requirements and performance scales greater than linearly
  and thus would take an infeasible amount of memory and time to simulate transport 
  ...
  length, $L$, to a power law function of the form $AL^{-\beta}$ (dashed lines in (b)
  and (c)). 
...
%  \end{subfigure}
  ...
  %BJC6: could move this paragraph to supporting
  %MRS7: Could push more to supporting.  I think you just need to state that you ran with X number, as determined in the supporting, but because cheap did 10K for the non-MSDDM models.
  ...
  realizations. For better precision, we recommend simulating at least 1000 realizations. 
  ...
  between solute hops. In Figure~\ref{fig:beta}a, we show that $\beta$, as derived 
  from the sFBMcut model, is inversely related to the Hurst parameter. 
  % BJC6: I'm not sure this sentence adds anything
  %MRS7: reminding people the physics behind models can be useful, though, maybe put back in?
  %This makes intuitive sense since higher degrees of anti-correlation should slow the rate at which solutes
  %cross the membrane pore. 
  $\beta$, as derived from the MSDDM, appears approximately constant which is 
  consistent with nearly invariant transitional Hurst parameters (see 
  Figure~\ref{fig:H_v_state}). However, Figure~\ref{fig:beta}a does raise
  the question as to why $\beta$ is so much lower than the sFBMcut model. The MSDDM 
  is fundamentally similar to the AD model in that one makes correlated draws 
  from a hop distribution, therefore, we should expect the $\beta$ value of the
  MSDDM to be larger than that of Brownian motion.
  
  The length dependence of solute flux ($\beta$) for the MSDDM is under-estimated 
  primarily due to innaccuracies in the correlation structure of very long FLM 
  trajectories. To generate the curves in Figure~\ref{fig:flux_curves_msddm}, we 
  needed to correlate 25--1000 times more hops than for the MSD predictions in 
  Figure~\ref{fig:msddm_performance}. To understand the behavior of the MSDDM, we 
  studied the $\beta$ parameter derived from variations on the SFBMcut model 
  (see Figure~\ref{fig:beta}a), since we can generate exact realizations of FBM. 
  First, we removed anti-correlation between hops in the sFBMcut trajectory (set
  $H$=0.5). The $\beta$ parameter drops to values similar to the MSDDM. When we 
  remove hop anti-correlation \textit{and} dwell times between hops (set dwell 
  times equal to 1), effectively simulating Brownian motion, we observe similar 
  $\beta$ parameters. These two variations imply that the $\beta$ parameter is 
  only a function of $H$ and not dependent on dwell times ($\alpha$, $\lambda$) 
  or hop lengths ($\sigma$). In Section~\ref{S-section:beta_estimates} of the 
  Supporting Information we expand on this explanation and show that replacing 
  inexact FLM realizations with exact FBM realizations recovers the pore length
  dependence of solute flux.
...
%  \end{figure}
  ...
  The length dependence of the MSDDM model is much weaker because sub-trajectories 
  between state transitions decorrelate transitional hops and because our FLM 
  simulation procedure does not accurately correlate hops after long time lags. 
  We support this claim by removing hop correlation from the sFBMcut model which
  results in a drop in the $\beta$ value close to MSDDM. These values of $\beta$ are
  consistent with Brownian motion (no hop correlation and unit-length dwell times).
  (bottom left) As solute flux increases, the scaling of the flux curves, $c$, increases 
  (compare ranking with Figures~\ref{fig:flux_curves_ad} and~\ref{fig:flux_curves_msddm}).
  The $A$ parameters of the MSDDM are smaller than the sFBMcut AD model because the AD
  model must compensate for its strong length dependence (higher $\beta$). 
  ...
  model by each parameter independently about the same base parameter set. 
  ...
  %$c$ responds linearly to increases in $\sigma$ and $\lambda$
  and non-linearly to $\alpha$.
  The data suggests that the $A$ parameters do not depend on hop anti-correlation ($H$).
  To test $\alpha_h$, we used the set of parameters defining the MSDDM of Urea (since 
  sFBMcut does not use a L\'evy distribution) and varied the $\alpha_h$ parameter of 
  the transitional hops. As $\alpha_h$ increases, $A$ increases slightly since the 
  density of the L\'evy stable PDF is shifted away from rare hops and broadened at 
  its center. We expect similar dependence of $A$ on $\sigma$ and $H$ of the MSDDM.
  }\label{fig:beta}
  \end{figure}
  
  Parameters represented by physical processes which act to modify the rate at 
  which solutes move through the membrane pores will affect the scaling pre-factor
  of the solute flux curves ($A$). Comparison of Figure~\ref{fig:beta}b with 
  Figures~\ref{fig:flux_curves_ad} and~\ref{fig:flux_curves_msddm} reveals that
  the ranking of the $A$ parameters is consistent with the ranking of solute 
  flux. In general, $A$ values generated by the MSDDM model are smaller than the
  those generated by the sFBMcut AD model because the AD model has to compensate
  for high $\beta$ values. In Figure~\ref{fig:beta}c we demonstrate that 
  decreasing dwell times (increasing $\alpha$), increasing hop lengths (increasing 
  $\sigma$), cutting off the dwell time distribution at shorter times (increasing
  $\lambda$) and increasing tail weights of the hop length distribution (decreasing
  $\alpha_h$) independently lead to an increase in $A$. Figure~\ref{fig:beta}c
  also suggests that $A$ is not dependent on hop anti-correlation ($H$).
...
%  In Figure~\ref{fig:beta}, we plot the $c$ values measured
...
%  because the AD model has to compensate for high $\beta$ values. 
%  Comparison 
...
%  consistent with the ranking of solute flux. 
...
%  membrane pores will affect $c$ for various physical reasons. 
%  In 
...
%  \end{figure}
  ...
  pores. 
  %Greater differences in $\beta$ values leads to selectivities that are stronger 
  %functions of pore length. 
  For $(A_i / A_j)=1$, LLC membranes will be more selective towards passage of solutes 
  with less anti-correlated hopping behavior (lower $\beta$). 
  %versus those with high $\beta$ values.
  For the same degree of hop anti-correlation, 
  %When $\beta$ values are equal, 
  as is the case for uncorrelated motion, selectivity only
  depends on the pre-factors, $A$. In most cases, the length dependence of selectivity 
  ...
  We may gain the most by using the two models in tandem. In both cases, the 
  data suggests that this particular LLC membrane might be most useful for 
  ...
  In their current state, one should use the predictions of our models with caution.
  ...
  immobility. The best solution to this would be to have individual emission distributions
  for each type of transition, but this would require orders of magnitude more data. 
  AD model realizations show appropriate qualitative behavior, but the persistent 
  curvature of the computed MSD curves, due to the correlation structure of fractional 
  motion, under-estimate the MD MSD curves which appear to be trending towards linear
  on $\mu$s timescales. In the future, we could address this by implementing a truncation
  to the positional auto-correlation function.
  ...
  fractional L\'evy motion process. 
  %When we replace FLM with FBM in the 
  %MSDDM algorithm, we observe a flux decay more consistent with that predicted
  %by the AD model. 
  We used the ratio of solute fluxes generated from each model in order to 
  calculate selectivity. Due to differences in hop anti-correlation, we observe 
  a length dependent selectivity. This length dependence may dissipate
  once we gain a more complete understanding of the hop correlation structure. 
  If the length dependence of a properly correlated model always converges to 
  $\beta$=2, it will greatly influence the prediction of selectivity on 
  macroscopic length scales.
%  However, this length dependence mostly plateaus at length scales that can be
%  synthesized experimentally. The data suggests that this particular membrane 
%  may be useful for separating ethylene glycol and acetic acid.
  ...
  % BJC6: took out for now since I was questioning it. Can add back if reviewers ask about these considerations.
  %MRS7: yeah, leave out for now.
...
%  specific monomer.
  ...
  in terms of controlling solute trapping. The anomalous diffusion model is 
