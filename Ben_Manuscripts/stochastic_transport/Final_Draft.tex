...
\begin{document}
  ...
  state of which has  
  %MRS10:
  %each having separate
  distinct transport properties. For each state and transition
  ...
  H\textsubscript{II} phase, it is considerably easier to 
  %MRS10: synthesis implies chemical synthesis, this is really assembly - but produce is more general. 
  %synthesize 
  produce 
  ...
  behavior by extracting 
  %MRS10: their is referring to behavior, which is singular - changing grammar.
  %their 
  particle
  trajectories from MD simulations and studying
  ...
  the complex solute trajectories. 
  %MRS: moving clause out from top.
  %Into a model, 
  We can incorporate fluctuations and
  time-dependent correlations in solute position 
  %MRS added:
  into transport models, 
  as well as the length of time
  solutes are trapped. We can add further detail by integrating
  ...
  We constructed our first model by applying the 
  %MRS10: tweaked
  existing
  ...
  particles 
  %MRS10: added specificity
  simulated at the atomistic level
  %exhibiting
  which exhibit
  non-Brownian, or anomalous, transport
  behavior.~\cite{metzler_random_2000,bouchaud_anomalous_1990} The tools
  ...
  different dynamical behavior of solutes 
  %MRS10: added, to show that this is informed by looking at the trajectories.
  previously
  observed within the pore region
  versus within the monomer tails.
 
  %MRS10: tweaked, also made plural since multiple models are built.
  %Where applicable, the sFBM model 
  Where sufficiently close to observed data, these fitted sFBM models
  provides a way to propagate information
  about solute trajectories gathered at the microsecond time scale to realistic
  experimental time scales. They can also confirm the importance of assumed
  solute-membrane interactions used to define the model's functional form, and
  ...
  %MRS10: tweaked, may not be quick in absolute terms.
  more 
  quickly
  than other solutes studied previously, 
  allowing them to extensively explore membrane structural space,
  these solutes have a range of chemical functionality and experience each of
  the three trapping mechanisms to different extents.
  ...
  parameter we calculated by maximizing the 
  %MRS10: added for precision
  log
  likelihood function: 
  \begin{equation}
  ...
  We determine MLEs of $\alpha$ and $\lambda$ by maximizing the 
  %MRS10:
  log 
...
%MRS9: I'd go with what they said.  Looking at PRE, it looks like you can put double lines above and below the table, though - I added them 
 ...
 \end{table}
  ...
  %TODO: cite github page for fbm
  We used the Python package \texttt{fbm} to generate exact simulations of FBM
  %MRS10: add a reference to where your implementation can be found.
  and our own Python implementation of the algorithm by Stoev and Taqqu to
  ...
  matrix, $T$.  
  %MRS10: split sentence for readability.
  $T$ describes the conditional probability of moving to a
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  \caption{
    %MRS10: added
    Three
    representative trajectories generated from each solute exhibit hops
  ...
  \caption{Using urea as an example, (a) the distribution of hop lengths can be
	  fit by a Gaussian 
          ``as well as'' is ambiguous, can mean ``in addition to'' or ``similar in quality to''
          %as well as 
          almost as well as
          a more general L\'evy stable
  ...
  shapes of 
  %MRS10: which of the solutes does ``their'' refer to?  Be more explicit.
  their MSD curves are also very similar (see
  ...
  equilibration time. The MSD curves generated from 
  %MRS10: added
  both 
  %MRS10: more grammatically correct to say ``one and two'' instead of ``1 and 2''
  the one and two mode models
  are overlayed with the MD simulated MSDs for comparison in
  ...
  \caption{The parameters of the one mode model reveal differences in dynamics
	  ...
	  correlation between hops. 
          %MRS10: tweaked
          Simulations with higher values of $H$ display behavior closer to
	  the Brownian limit.}\label{fig:1mode_parameters}
  \end{figure}
  ...
  the MD MSD curve 
  %MRS10: being a bit more cautious about claims.
  %actually shifts 
  may actually shift
  from a sub-linear to a linear regime around
  500 ns.
  ...
  %Turning first to 
  Examining first
  the parameters of the one mode model, we can begin to break
  ...
  MSDs
  %MRS10: added
  and qualitatively similar MD timeseries.
...
%  The exact amount of data required is dependent on the solute studied. 
  ...
  with each solute trajectory 
  %MRS10: added for clarity to make sure people who how the state sequence was determined.
  based on the geometric indicators of the states indicated in Table~\ref{table:states}.
  and then generated emission distributions of fluctuations
  ...
  section.  
  %MRS10: break for clarity.
  We therefore did not consider the Gaussian case (see
  ...
  \end{subfigure}
  %MRS10: Probably will need to make the font labels bigger in a revision.
 \caption{The parameters of the MSDDM are strong functions of trapping
	  ...
	  decreases. $H$ is 
          ...
          %MRS10
          (closest to Brownian)
          during transitions between states (state
	  T). (b) As more trapping mechanisms simultaneously influence solutes,
  ...
  %MRS10: is it a good idea to say they are well-captured when we get results qualitatively different from MD later?
  ...
  \caption{Realizations of the MSDDM for each solute (blue) do not reproduce
	  ...
	  might expect for Brownian motion
          %MRS10: added for context
  ...
  was 7$\mu$m thick. 
  %MRS10: provide citation here for the fact of 7mum thick.
...
%  \vspace{0.5em}
  ...
  behind the mechanisms leading to selective behavior 
  %MRS10: softened claim a bit - it hasn't been used for design yet.
  %which is very important for design. 
  which could significantly help illuminate any design choices.
  We hope this type of analysis can be leveraged to explore new,
  interesting and complex separations problems.
  ...
  %MRS10: may want to mention a bit about the weakness of the methods (i.e. say something about what 
  % can be done even given the lack of correlation structure), since it's in the conclusion.
  ...
  %are possible. 
  %Our 
  The modeling approaches presented in this paper 
  facilitate new ways of approaching
