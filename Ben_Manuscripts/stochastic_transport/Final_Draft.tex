...
\begin{document}
  ...
  explore two variations of the anomalous diffusion modelling approach. The first variation
  ...
  of which having separate transport properties. For each state and transition
  ...
  H\textsubscript{II} phase, it is considerably easier to synthesize at
  scale.~\cite{hatakeyama_nanoporous_2010}
  ...
  behavior by extracting their trajectories from MD simulations and studying
  ...
  the complex solute trajectories. Into a model, we can incorporate fluctuations and
  time-dependent correlations in solute position as well as the length of time
  ...
  We constructed our first model by applying the rigorous theoretical
  foundation already developed to describe the motion of particles exhibiting
  ...
  different dynamical behavior of solutes observed within the pore region
  versus within the monomer tails.
  
  Where applicable, the sFBM model provides a way to propagate information
  about solute trajectories gathered at the microsecond time scale to realistic
  experimental time scales. It can also confirm the importance of assumed
  solute-membrane interactions used to define the model's functional form, and
  suggest missing components. It can help researchers formulate hypotheses to
  ...
  use the two modelling approaches in tandem, the AD approach to identify
  ...
  quickly, allowing them to extensively explore membrane structural space,
...
%MRS9: I'd go with what they said.  Looking at PRE, it looks like you can put double lines above and below the table, though - I added them 
 ...
 \end{table}
  ...
  matrix, $T$, which describes the conditional probability of moving to a
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  \caption{Using urea as an example, (a) the distribution of hop lengths can be
	  fit by a Gaussian as well as a more general L\'evy stable
	  distribution. The L\'evy stable distribution does a better job of
	  capturing the heavier tails and increased density near 0. We explore
	  models fit to both distributions since Gaussian hops are convenient
  ...
  shapes of their MSD curves are also very similar (see
  ...
  equilibration time. The MSD curves generated from the 1 and 2 mode models
  are overlayed with the MD simulated MSDs for comparison in
  Figure~\ref{fig:anomalous_msds}. The associated parameters for the 1 and 2
  ...
  \caption{The parameters of the one mode model reveal differences in dynamics
	  ...
	  ($\mu$) to be zero for these distributions and there to be no
	  ...
	  correlation between hops. Higher values of $H$ display closer to
	  Brownian behavior.}\label{fig:1mode_parameters}
  \end{figure}
  ...
  the MD MSD curve actually shifts from a sub-linear to a linear regime around
  500 ns.
  ...
  Turning first to the parameters of the one mode model, we can begin to break
  ...
  MSDs. Relative to ethylene glycol, methanol tends to stay trapped for less
...
%  The exact amount of data required is dependent on the solute studied. 
  ...
  section, therefore we did not consider the Gaussian case (see
  ...
  \caption{The parameters of the MSDDM are strong functions of trapping
	  ...
	  decreases. $H$ is highest during transitions between states (state
  ...
  was 7$\mu$m thick. Using 24 cores to simulate trajectory realizations in
...
%  \vspace{0.5em}
  ...
  \caption{The selectivity between pairs of species changes monotonically with
	  ...
	  flux result in high selectivites at any pore length. This membrane
  ...
  behind the mechanisms leading to selective behavior which is very important
  for design. We hope this type of analysis can be leveraged to explore new,
  ...
  lengths and dwell times. The MSDDM is a powerful way to characterize explicit
  ...
  re-designs an LLC monomer to eliminate one source of trapping, then high
  selectivities are possible. ur models facilitate new ways of approaching
