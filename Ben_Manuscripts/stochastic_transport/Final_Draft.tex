...
\begin{document}
  ...
  \affiliation{Department of Chemical and Biological Engineering, University of Colorado Boulder, Boulder, Colorado 80309, United States}
  ...
  %simulated MSDDM 
  MSDDM-generated  %MRS: abbreviation not defined first? Maybe eliminate here, just call it the Markov State dependent model
  trajectories
  ...
  motions 
  %MRS8:  Added the below, since for simple diffusion, you just calculate the diffusion constant. 
  %and 
  ...
  %MRS8: break sentence, tweak phrasing.
  %however 
  ...
  %MRS6: It was also because they thought it was more likely to be interesting initially.  I'm in favor of 
  ...
  %MRS8: added as more clarification
  ...
  %MRS8: really nice flow in the introduction up through here.
...
%  which result in observed transport mechanisms. 
  ...
  %MRS8: this may be a bit of a stretch, since it's not clear it helps that much in the end, but 
  %is fine to leave in now - maybe we revisit if needed during revisions.
...
%  probability.
  ...
  %MRS8: should say something about the molecular colors for (b) so it's clear what is what. 
  \end{figure}
  
%  \begin{figure}
%  \centering
%  \subfloat[\label{fig:monomer_structure}]{%
%  			\includegraphics[width=0.4\textwidth, valign=c]{NaGA3C11.pdf}%
%  			\vphantom{\includegraphics[width=0.5\textwidth, valign=c]{membrane_profile.pdf}}
%  			}\hfill
%  \subfloat[\label{fig:membrane_profile}]{\includegraphics[width=0.5\textwidth, valign=c]{membrane_profile.pdf}}\hfill
%  \caption{(a) The wedge-shaped liquid crystal monomer Na-GA3C11 will form the inverted
%  hexagonal phase in the presence of water where the carboxylate head groups occupy the
%  pore centers. (b) A cross-section of a periodically replicated atomistic unit cell 
%  used for simulations in this study reveals the membrane's aqueous, hexagonally packed,
...
%  data from which to generate statistics on the time scales which we simulate.
  ...
  %BJC7: added following two paragraphs to clarify our usage of the MSD as a tool. Modified a bit below too  
...
%  For a small number of solute 
%  trajectories, this is a much more statistically robust measure of the 
%  average distance a solute travels over time. 
...
%  It may be difficult to calculate  ensemble
%  MSD to estimate $\alpha$ with only 24 solute trajectories.
%  
%  Although theoretically possible, we do not use the 
...
%  \end{equation}
  ...
  %MRS: could reference the figure 4 here?
  We observe that
...
% We generated
 ...
 \end{table}
  ...
  %instead of 95%, so probably should check for consistency
  ...
  % determine that the algorithm sufficiently and reasonably approximates.
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  %MRS7: yeah, supporting.  Should just be ``Verified the Markovianity of state transitions in MD as described in Section [X]. Generally, how one did verification of most things can be put in supporting, since that's not central to the argument. ''
...
%  simulation procedure described in Section~\ref{method:model_sFBM}.
  ...
  For computational 
  %MRS8: computational costs? Clarify.
...
%  % of runners crossing the finish line
  ...
  %MRS8: check to make sure there is goodness-of-fit checks in the SI for fitting to the analytical model.
...
%  passage time distribution:
  ...
  %MRS8: be more specific about which evidence. Made a stab below.   
  ...
  \end{subfigure}
  %MRS8: the description of hop lengths will be confusing to readers. You say it _could_ be fit by a Gaussian, but 
  %then say that a Levy distribution fits better. So readers are not sure which one will be used in subsequent data.
  ...
  %MRS8: may or does overestimate?  I would say does.
  ...
  %MRS8: should this be _either_ fractional Brownian or Levy for clarity.
  ...
  %MRS8: maybe plural, since each has an autocorrelation function.
  ...
  %MRS8: would it be better to put the decision here, or at least say where this decision is made?  Otherwise the reader has a lot of potential decisions to keep in their mind. 
  ...
  %MRS8: be more specific than ``the AD model'' below?  Also, not sure how clear this thesis is since you described 2x2=4 different models in the previous two paragraphs - at this point, the reader isn't sure of a single model. Tried a pass.
...
%  only meaningful if the ensemble of solute trajectories is stationary.
  ...
  %MRS: again ``the AD model'' is confusing since there are multiple models based on AD.   ``The AD approach''?
  ...
  %MRS8: tweak
  %so we admit that those will only be approximations.
...
%  be necessary. 
  ...
  %MRS8: does it make sense to say ``the AD model'', when there are several variants? 
  %would it be better to say ``the AD approach''?
  ...
  %MRS8: I wonder if it makes sense to be consistent between Gaussian and normal just to keep it simpler (even though synonyms)
...
distributions. Values of $\alpha_h$ are significantly lower for urea and acetic acid meaning 
  ...
  %MRS8: do you mean, examining up to a 1000 ns time lag (Since that what is plotted?)
  ...
  %MRS8: maybe be a bit more explicit about which choices described above are used here. i.e. did you actually use a Gaussian  distribution because Levy was so close?
  ...
  %MRS8: maybe there should be something here like ``as the system moves to independent parts of the pore, but this correlation does exist in the time series modeling''.
  Eventual loss of
  correlation would result in a shift from sub-linear to linear MSD behavior. 
  %MRS8: maybe add ``as observed in the MD trajectories.''?
  ...
  %MRS8: is it better to say it has no physical basis, since it doesn't reflect what is physically happening.
...
%  with tails are frequent. 
  ...
  %MRS8: ``the AD model'' vs maybe ``the AD approach''
  ...
  %MRS8: maybe ``either Gaussian or Levy''?
  ...
  %MRS8: helps flow, so probably OK.
  %MRS8: Model vs. approach. Though the 2 state does involve some mechanistic detail (partitioning) so the below isn't quite true.
  ...
  %MRS8: in section below, it would be useful to add a sentence that answers the ``so, what'' after the structural
  %detail for each molecule - just adds reader interest.
  ...
  donors.
  %MRS8: above seems incomplete - are weak h-bond donors so what?
...
%  correlation structure similar to that seen in the 2 mode AD model.
  ...
  %MRS7: yeah could say you created one for each solute, as described in the methods.
...
%  within the pore region. 
  ...
  %MRS8: unclear what multiple tethers means. 
...
%  realizations at each pore length. 
  ...
  between solute hops. In Figure~\ref{fig:beta}a, we show that $\beta$ is inversely 
  related to the Hurst parameter. This makes intuitive sense since higher degrees of
  anti-correlation should slow the rate at which solutes cross the membrane pore. 
  When we remove anti-correlation between hops (set $H$=0.5), the length dependence 
  becomes the same for all solutes, dropping to a value just below 2. This also 
  implies that hop lengths and dwell times do not affect length dependence since 
  each solute exhibits different hopping and trapping behavior yet are all parameterized
  by the same value of $\beta$ when $H$=0.5.
  %BJC7: is above argument enough to get rid of following sentence (and associated bar in the Figure)?
  We further verified this claim by removing hop anti-correlation and setting all 
  dwell times equal to one timestep, effectively simulating Brownian motion.
  The length dependence remains unchanged. 

  %BJC6: TODO: add (a), (b), (c) labels
  %BJC7: TODO: optimize figure dimensions
  \begin{figure}
  \centering
  ...
  %BJC7: I could plot bars with the MSD of each solute to show that trend is same.
...
%  \vspace{0.5em}
  ...
  \end{subfigure}
  \vspace{-1.5cm}
  %MRS8: top figure is very confusing (also, I see a check in the urea green bar?) 
...
%  of the solute flux curves ($A$). 
  ...
  %MRS8: you removed MSDDM from figure.
  ...
  %MRS8: optimal isn't really defined here.
  optimal. 

  %MRS8: IMPORTANT POINT!
  %MRS8: I would play up that there is absolutely no way one could figure this out 
  %MRS8: from looking at the molecules, or the pores, or even the MD trajectories alone.
  %MRS8: There's a lot of complex interplay here that has the potentially to really lead to interesting 
  %MRS8: separations. Even if the models aren't quite right, they show how such selectivtiy CAN arise.
  %BJC7: I don't know why there is a big space before this paragraph when compiled
  ...
  %In both case, selectivity is enhanced as the pore 
  %length increases which comes at the cost of lower solute flux, a phenomenon 
  %ubiquitous in polymer separations membranes.~\cite{geise_water_2011}
  
%  Ethylene glycol shows the least length dependent flux, likely because
%  it spends the majority of its time in the pores (see Figure~\ref{fig:AD_mode_occupation}).
...
%  In their current state, one should use the predictions of our models with caution.
  ...
  more data. 
  %MRS8: (could add something lie below) 
  %MRS8: or more prior assumptions about the distributions.
...
%  to the positional auto-correlation function.
  ...
  %MRS8: ``the AD model'' vs approach, since it's multiple models.
  ...
  %MRS8: maybe ``does not appear to be consistent''?
  ...
  %MRS8: maybe add something like ``as suggested by the physical trajectories and the structural model''?
...
%  This length dependence may dissipate once we gain a more 
%  complete understanding of the hop correlation structure. 
...
%  specific monomer.
  ...
  %MRS8: I wonder if the 2 mode model can be considered a intermediate - a bit of structural information is involved.
...
% LocalWords:  timestep pre selectivtiy foulants subfigures
