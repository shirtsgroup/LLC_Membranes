...
%  data from which to generate statistics on the time scales which we simulate.
  ...
  We primarily use the MSD as a tool for calculating the average properties of 
  solute trajectories. We only compare MSDs calculated from MD simulations to 
  those generated from our models. We do not use them to calculate diffusion 
  constants or to relate our simulations to experimental measurements. Therefore
  it is only important that we use a consistent definition for calculating the MSD.
%  This approach is valid if we adhere to a consistent definition for MSD 
%  calculations.
...
%  We modeled solute motion as a CTRW subordinate to FBM, also called sFBM. 
  ...
  doing this, with easily estimated paramters, is by adding an exponential 
...
%  In their current state, one should use the predictions of our models with caution.
  ...
  more data. This approach is further complicated by the need to make correlated draws
...
%  to the positional auto-correlation function.
  ...
  MD simulations, the curvature of the predicted MSDs may not be consistent
