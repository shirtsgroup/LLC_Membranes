...
\begin{document}
  ...
  %BJC7: added  
  However, qualitative differences between MD and simulated MSDDM trajectories
  may limit its usefulness. 
  Finally, we demonstrate how one can use our models to estimate flux of a solute
...
%  \end{itemize}
  ...
  %BJC7: changed thesis to focus more on this paper
...
%  solute trajectories. Specifically, 
  %BJC7: with changes, above thesis statement is redundant
...
%   is an elegant way to characterize single 
%  particle trajectories and in addition to providing an easy and efficient way to 
%  simulate solute trajectories, 
%  uncover the underlying solute-membrane interactions 
%  which result in observed transport mechanisms. 

  % BJC7: moved last sentence of previous paragraph to here to make it more clear that
...
% the pores, we incremented the $(2, 1)$ entry of the count matrix by 1.
% We generated
...
%  only meaningful if the ensemble of solute trajectories is stationary.
  ...
  not be good enough for molecules with statistical non-stationarity, so we admit that 
  those will only be approximations.
...
%  be necessary. 
  ...
  %BJC7: delete following sentence? I basically just said it above.  
  Although we 
  ...
  \end{subfigure}
  \caption{In most cases, MSDs generated from realizations of both the one and two
  mode AD models lie within or near the 1$\sigma$ confidence intervals of MD-generated 
  ...
  In most cases, the one mode simulated MSDs under-predicted the mean at long timescales
  partially because they show pronounced curvature which the MD MSDs lack. The two mode
  predictions show less curvature than the one mode MSDs because the hop correlation 
  structure is broken every time a transition between tails occurs.
  ...
  \end{subfigure}
  \caption{The parameters of the one mode model reveal differences in solute dynamics.
  ...
  the order of hundreds of ns, may not exist in the MD system. Eventual loss of
  ...
  Due to the much larger accessible volume, methanol spends $> 90\%$ of its time
...
%  with tails are frequent. 
  ...
  %BJC7: I'll leave it here unless we really need to cut stuff.
...
%  correlation structure similar to that seen in the 2 mode AD model.
  ...
  heavy-tailed. Shown here is the emission distribution for transitions between
  states. The maximum likelihood Gaussian fit severely underestimates the empirical
  density of hops near and far from zero while overestimating the density of hops
  at intermediate values. (b) Jumps drawn from the transition distribution are 
  negatively correlated to each other. The normalized version of 
  Equation~\ref{eqn:flm_autocovariance} fits well to the data suggesting FLM is
  ...
  % BJC7: Following is unsupported by data and therefore just a hypothesis
...
% reordered this section to make the logic more clear  
  ...
  %timescales due to differences in their curvature. 
%  Similarly, methanol's MD MSD 
...
% BJC: alt to next paragraph
%  We hypothesize that the predicted MSD of acetic acid is severely over-estimated
%  most likely due to under-estimation of hop anti-correlation ($H$ too high) in 
%  addition to an over-estimate of its hop lengths ($\sigma$ and $\alpha_h$ too high). 
%  Acetic acid's predicted MSD actually lies just within the lower bound of urea's MD
%  MSD 1$\sigma$ confidence interval, but lower than the prediction of the MSDDM for 
%  urea. In all states, the degree of anti-correlation between acetic acid's hops is 
%  higher than urea's (lower $H$) which is consistent with a lower predicted MSD. Although
%  consistent, based on the curvature in Figure~\ref{fig:ACH_msddm}, and difficulties
%  parameterizing $H$, it is possible that we under-estimated hop anti-correlation. 
%  The length of acetic acid's hops and weight on the tails of the hop length 
%  distribution ($\sigma$ and $\alpha_h$ respectively) are greater than or equal to 
%  urea's which is consistent with a higher predicted MSD. There is also the strong 
...
%MRS6: even if the MSD is well predicted, if the trajectories look different, can it really be a useful model?  I.e. is one getting the right answer for the wrong reasons?  What does it mean if MSD's are good, but the dwell times are not great?
  ...
  for transitions. This was necessary because we could not collect enough data
  to fit all of the possible transition distributions and because we could not
  correlate emissions coming from different hop distributions. 
  Many transitions occur between two trapped states where the 
...
%  flux and selectivity.
  ...
  recommend simulating at least 1000 realizations. Even with an order of magnitude
  decrease in number of trajectories, it is still infeasible to simulate
  ...
  fits well to Equation~\ref{eqn:passage_times}. We show similar fits for the remaining
...
%  \end{subfigure}
  ...
  %BJC7: Now that MSDDM is gone, and I have data for 10,000 sFBMcut trajectories, I added 
  % a sentence to paragraph 2 before this one pointing to the SI.
