...
\begin{document}
  ...
  % BJCR: r2_comment2 - replacement to following commented out paragraph  
...
%  modeling to predict long timescale behavior and macroscopic properties 
...
%  solute selectivity in macroscopic length pores.
  ...
  %BJCB: could cut this last sentence. It's kind of awkward  
  %MRSB: agree  
  %In the materials industry, there is a strong interest in creating breathable
  %fabrics which selectively allow passage of water
  %vapor.~\cite{mondloch_destruction_2015}

  % BJCA: I could likely condense this paragraph to focus primarily on the HII phase
  % and its complex pores.
  %MRS: yes.  QI not relevant
...
%  at scale.~\cite{hatakeyama_nanoporous_2010}
  ...
  %BJCB: removed discussion of Q1 from above paragraph
  Lyotropic liquid crystals (LLCs) are a class of amphiphilic molecules that
  can be cross-linked into mechanically strong and highly selective nanoporous
  membranes.~\cite{gin_polymerized_2008} They may provide a promising
  alternative to conventional membrane separation techniques by being selective
  ...
  % BJCB: split into two paragraphs  
  ...
  % We identified 
  % BJCR: r2_comment3 - modified to work in figure reference
...
%  subdiffusion: 
  \label{r1_comment1}  % BJCR: replaced first sentence
  ...
  fractional motion process. % is subordinate to the continuous time random walk.
  
% BJCC: I cut this, doesn't really seem to be necessary for intro
...
%BJCC: I don't see a need to justify 10 over 5 wt% 
%  We chose to focus our study on the 10 wt\% water system because
...
%  previously, thus allowing more statistically valid analyses.
  ...
  \label{r1_comment2} % BJCR: description of force field.   
  ...
  % BJCB: condensed commented out section that follows into this paragraph
  ...
  % BJCA: Could move most of the following discussion to the SI and just present the time-averaged MSD.
  % MRS: good call.
...
%  characteristic measurement timescale of solute motion. In fact, as measurement 
...
% BJCC: left this discussion for supporting. Added last sentence above
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  of an AD approach model all released at the pore entrance ($z=0$). In the 
  case of uncorrelated hops, one can continuously draw from the hop length 
  distribution until $z \geq L$ (or $-L$ for the sake of computational 
  efficiency). The length of time between the last time the particle crossed 
  $z=0$ and the end of the trajectory gives a single passage time. When particle
  hops are correlated, as they are in all cases of this work, we cannot 
  continuously construct the particle trajectories. Rather, we must generate 
  trajectories of length $n$ and measure the length of the sub-trajectory which
...
%  \end{figure}
  ...
  predictions parameterized with fraction L\'evy versus Brownian motion.% (see Figure~\ref{fig:anomalous_msds}).
  ...
  %We demonstrate this effect in the inset to Figure~\ref{fig:powerlaw}, where 
  ...
  % not include the pure sFBM or sFLM models in the
  ...
  %BJCA: This section to supporting
%  \subsubsection{Stationarity of solute trajectories}\label{section:stationarity}
%  %MRS: yes, though report the results in main text.
%
...
%  lower.
  ...
  %BJCB: added next two sentences point towards non-stationarity discussion
  We observe that in some cases, solute trajectories extracted from our MD
  ...
  \end{subfigure}
  \caption{% BJCR: r1_comment5. modified caption. Old caption commented out
...
%	  molecule with two hydrophilic hydroxyl groups.
  ...
  %most likely because
  %their $\alpha_h$ parameter values are relatively close to 2, meaning
  because  
  the hop length distributions are nearly Gaussian even when fit to a more general
  L\'evy distribution ($\alpha_h \sim 2$). 
  
  % BJCB: I'm not sure I want to move this paragraph because I think the curvature
  % is important to discuss and I mention its implications in the conclusion
  ...
  %MRS: this paragraph below may be the sort of summary of the models that would
  %go in the main text, with the details in the SI.
  ...
  %BJCB: maybe move the next two paragraphs to the SI?
  %MRSB: I think that this is more useful in the main text, since it connects models to the simulations and the physics.
...
%  The exact amount of data required is dependent on the solute studied. 
  ...
  %MRSB: I think people like the physical intuition gained. I added bullet points to aid reading. 
  \begin{itemize}
  ...
  %BJCA: potentially move this to SI
  %MRS: seems good.  Again, make sure that the conclusions are referenced in the main text. 
%  \begin{figure}
%  \centering
...
%  \end{figure}
  ...
  trapping behavior.
  %old text
%  We do not consider the two mode AD model because it has a
...
%  \vspace{0.5em}
  ...
  \caption{The selectivity between pairs of species changes monotonically with
	  pore length. The strength of dependence on pore length depends on
 	  ...
 	  %BJCR: r1_comment5. removed redundant details
%     The largest differences in solute
...
% LocalWords:  time step pre selectivtiy foulants subfigures RWFs roman github
% LocalWords:  al linkable nanopores klages BJCR BJCB MRSB BJCA BJCC
% LocalWords:  Donnan steric Antechamber polarizable intramolecular
% LocalWords:  quasiharmonic ester decorrelated nanoscale
% LocalWords:  nanoscopic impactful
