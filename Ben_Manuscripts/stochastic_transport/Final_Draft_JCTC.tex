...
\begin{document}
  ...
  powerful tool
  %MRS: this paper doesn't really address identifying transport, though, does it?  Although (other) modeling can be 
  %MRS: useful, putting it in the abstract implies mechanism identification is in this paper.
  %for identifying transport mechanisms and predicting
  %macroscopic properties.  
  for extracting important physical
  information from molecular simulation. In this study, we present
  two new approaches to use stochastic time series modeling to predict
  long-time scale behavior and macroscopic properties from molecular
  simulation which can be generalized to other molecular systems
  where complex diffusion occurs.

  In this study, we use two different stochastic time series modeling approaches
  ...
  predict solute MSDs. We show that both models reasonably reproduce
  the MSDs calculated from MD simulations. However, qualitative
  differences between MD and Markov state dependent model-generated
  trajectories may limit its usefulness.

  Finally, we demonstrate how one can use these models to estimate
  flux of a solute across a macroscopic-length pore and, based on
  those quantities, the membrane's selectivity towards each
  solute. This work therefore helps to connect microscopic,
  chemically-dependent solute motions that do not follow simple
  diffusive behavior with long-time scale behavior, in an approach
  generalizable to many kinds of molecular systems with complex dynamics.
...
% BJC2: removed this sentence. High uncertainty MSDs don't really relate to long timescale predictions. Unless the MSDs were linear.
...
%  squared displacement (MSD), preventing reliable long timescale predictions.
  ...
  %BJC2: following sentence replaced with above
%  We are in need of a way of studying the collective motion of a much larger
%  set of solutes over much longer timescales.
  %MRS11: Maybe this last sentence could be changed. Because you aren't really finding a way to study larger sets of 
  %solutes over larger timescales, you are using a limited amount of atomistic simulation into long time scale behavior efficiently. We can discuss variations as needed!
  ...
  %MRS11: this next sentence could probably be cut - it's kind of vague.
  %BJC2: True
...
%  model conveys. 
  ...
  %MRS11: in this paragraph, the ``we can'' is vague, and doesn't
  %ascribe agency.  Is ``We'' our group? Is it the field as whole?
  %If it's something someone has done in the past, there should be
  %citations. If it's something that can be done in the future, the
  %phrasing should be a long the lines of ``it is possible to'', ``it
  %can be done'', etc.
...
%  study.~\cite{klages_anomalous_2008}
  ...
  %MRS11: should be clear if first model is previous publications, or what is presented here.
  %If what is presented here, should talk about how it will be presented, not that it was developed. 
  %MRS11: research activities happened in the past, presentation of research is described in the present.
  %or potentially future if describing what will be presented in the rest of the paper. 
  %BJC: replacement to above.
  %BJC1: Made clear that this is talking about the first model of this work.
  %BJC1: went through rest of intro to make sure tense is consistent
  ...
  %MRS12: removing paragraph break
...
%  subordinated FBM (sFBM) for short. 
  ...
  %MRS11: Introduction should not really give the answer so far.
  %Introduction should describe the questions that will be asked, but
  %not answer them.  That's not what is expected in an intro, it may confuse people (answers should be in the abstract).
  %MRS11: this next paragraph could use tweaks/cutting the most, but review the previous paragraphs as well.
  %MRS11: you can think about putting some of these results into conclusion, though - it reads well now, but might be benefited from reemphasizing results if it's currently missing any of this.
  %BJC2: slight tweaks and removed conclusions from following paragraph. Also went back through previous
  % paragraphs to look for conclusions.
...
%  The exact amount of data required is dependent on the solute studied. 
  ...
  %presumably come up with something reasonable. But that's for another time. 
...
% LocalWords:  al linkable nanopores klages
