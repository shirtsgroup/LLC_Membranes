...
\begin{document}
  ...
  %MRS: combined the ``general'' material about models here.  
...
%  subordinated FBM (sFBM) for short. 
  ...
  %MRS: the first sentence is key to the paper thesis and deserves to be emphasized, but I moved it up.
  %MRS: rest is somewhat redundant.
  ...
  %MRS: eh, tables are a clear way to communicated data, so OK. 
  \begin{table}[!htb]
	  \centering
	  \begin{tabular}{ccc}
            \hline
...
%  dimensional reduction which are often necessary to identify representative states.
  ...
  %MRS: Which parts specifically?  Why rationale of 2 mode to SI?  What parts of the results would be presented in main text?What is the main story? (Two model no good, but we tried it, see SI - but that story doesn't seem consistent with the data, they both seem OK)
  ...
  %MRS: some of these discussions might belong in the SI, with just the top line information in the main text.  People don't really care about the details of the individual parameters, just what story that tells.   
  \begin{figure}
  \centering
  \begin{subfigure}{0.325\textwidth}
