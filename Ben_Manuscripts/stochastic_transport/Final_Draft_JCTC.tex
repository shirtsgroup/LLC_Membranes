...
\begin{document}
  ...
  occurs.
  %BJCA: eliminated repetition and merged paragraphs, since the following is one sentence
  %In this study, we use two different stochastic time series modeling approaches parameterized 
  ...
  predict solute MSDs. 
  %BJCA: should we add a pargraph break here? Kind of short, but doesn't quite fit with above
  We show that both models reasonably reproduce
  ...
  %BJCA: should hyphen in long-time scale be there? I would favor the pronunciation that goes with long time-scale with or w/o the hyphen
  ...
  % BJCA: Would it make more sense to have the first paragraph less
  % applications-focused and more centered around a need for our new methods?
  % Something like .. 
  There is a large disparity between the time scales accessible to atomistic
  molecular dynamics (MD) simulations and experiment.
  \begin{itemize}
    \item The dynamics of molecules over nanoseconds of simulation may
    tell a very different story than that inferred by bulk measurements
    on the time scale of seconds.
    %BJCA: can cite something about different diffusive regimes 
    \item Understanding complex dynamical behavior on short timescales 
    and connecting them with much longer timescales can provide 
    fundamental insight which can be used for atomic level design of
    materials.
    \item In this work, we pursue this connection applied to highly 
    selective aqueous filtration membranes and demonstrate how one can
    gain chemical intuition which serves both theoretical and 
  ...
  %BJCA: maybe this paragraph can be cut down?
  ...
  % BJCA: resurrected this paragraph
  ...
  % BJCA: removing less technical version of above
...
%BJCA: I could replace above with this, but I think I like the above better. It's more concise.
...
%  subordinated FBM (sFBM) for short. 
  ...
  %BJCA: I could get rid of this table and define them in text. 
  \begin{table}[!htb]
	  \centering
	  \begin{tabular}{ccc}
            \hline
...
%  facilitate new ways of approaching complex separation problems.
  ...
  % BJCA: Perhaps replace above paragraph with the following:? 
  In a broader context, it is clear that time series modeling may be a useful way
  to form a lucid connection between complex solute dynamics on the nanosecond 
