...
\begin{enumerate}[label={Comment \theenumi :}, leftmargin=3.9\parindent]
  ...
  New text:
  \begin{quote}
  We modeled water molecules using the TIP3P water model,~\cite{} We used the
  antechamber module shipped with AmberTools18 to parameterize monomers and solutes
  using the Generalized Amber Force Field (GAFF).~\cite{??} We assigned charges using
  the AM1-BCC model. GAFF is very commonly used in simulations with diverse organic
  chemical functionality, and was developed for consistency within the AMBER ecosystem,
  which used TIP3P as the water model. We chose these standard force fields and
  generalizable parameterization approaches so that a range of solutes and monomers 
  could self-consistently and easily be studied.  The main goal of this work is to 
  develop stochastic models which can reproduce solute behavior on MD time scales, 
  independent of the choice of force field. In future work, it may be beneficial
  to employ specialized force fields, including polarizable force fields or other 
  higher-order effects in order to improve the accuracy of structure parameters and
...
\item \textit{Next, reading this section I would also like to know how well finite 
        ...
        surroundings (cite two papers I sent), and the LLC membrane (outside of
        intramolecular motion and quasiharmonic vibration around it's average 
        position) is therefore mostly motionless, with a very high effective 
        viscosity. This high surrounding viscosity should nearly eliminate these
        typical finite size effects. Additionally, we are usually comparing ratios
        of diffusion when we look at selectivity, and thus finite size effects
...
%	      molecule with two hydrophilic hydroxyl groups.
  ...
  \item \textit{One question that came to mind reading the paper was: why is the model only in 
    ...
    %BJC: modified to lead with quasi-2D
  		...
  		to point out that the two mode anomalous diffusion model was a quasi-2D 
