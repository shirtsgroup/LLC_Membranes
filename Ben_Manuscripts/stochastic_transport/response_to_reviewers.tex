...
\begin{enumerate}[label={Comment \theenumi :}, leftmargin=3.9\parindent]
  ...
  %MRS: removed the capitalization on anomolous diffusion, not grammatical generally.
  ...
  New text:
  %MRS: add what tools used to parameterize them, like tleap, also that charges are AM1-BCC.
  %MRS: can mention that files are available in supporting information of previous paper, though
  %MRS: less relevant to this comment.
  \begin{quote}
  We parameterized monomers and solutes using the Generalized Amber Force Field (GAFF)~\cite{??} and
  water molecules using the TIP3P water model,~\cite{}
  %MRS: tweaked
  % We made each choice in order to
  % ensure that our approach is accessible and reproducible to a wide audience and so that
  %it is easy to study a broad range of solutes and monomer
  %chemistries in high throughput.
  GAFF is very commonly used in simulations with diverse organic chemical functionality, and was developed for consistency
  within the AMBER ecosystem, which used TIP3P as the water mode.
  We chose these standard force
  fields and generalizable parameterization approaches so that a range
  of solutes and monomers could self-consistently and easily be
  studied.  The main goal of this work is to develop stochastic models
  which can reproduce solute behavior on MD time scales, independent
  of the choice of force field. In future work, it may be beneficial
  to employ specialized force fields, including polarizable force
  fields or other higher-order effects in order to improve
  the accuracy of structure parameters and selectivity predictions.
  \end{quote}
  
  %BJC: not sure if anything should be added to text

  %MRS: generally, something should be added to the text for any
  %reviewer comment, even if just expanding what is already there to
  %include some of the points raised by the reviewers. It shows
  %good-faith efforts, and also if a reviewer has a concern, some
  %readers will have it as well, so might as well head it off.
  
  %BJC: I could use some help on justification
\item \textit{Next, reading this section I would 
        also like to know how well finite size effects are controlled for with this specific 
        membrane and these specific solutes.}
        ...
        %MRS: also, I cut out the part about diffusing between pores,
        %but I don't think that's what they were getting at.

        %MRS: Do the solutes cross the boundary
        %conditions in the z direction much? You say they don't cross
        %between pores.  I don't think that's real point, though.
        ...
        %Our main focus is on the time series of the solutes in the
        %membrane pores. On the 5 $\mu s$ time scales which we
        %simulate, we do not observe any solutes that cross between
        %pores. Therefore, we expect similar solute behavior in larger
        %arrays of pores.
...
\item \textit{Lastly, it would be natural to report the size of the
  ...
  New text:
  \begin{quote}
  %MRS: couldn't hurt to add cell dimensions  
...
%	      molecule with two hydrophilic hydroxyl groups.
  ...
  \item \textit{One question that came to mind reading the paper was: why is the model only in 
    ...
    %MRS: we probably want to phrase it a bit differently. I'm thinking you should you lead with the fact that it is quasi-2D, since radial information is built into having different states in the pores and the tails for the MSDDM and the 2-state anomalous diffusion models, then go into the other details.  Thoughts?
		...
		allows us to overcome these issues and to use cylindrical representations with ease. %MRS: could cite the arXiv paperhere.
  ...
  \item \textit{\textbf{Page 35 line 12-14}: \\
        ...
        long time scale linear behavior of the two-mode model has
        %MRS: one  could argue that the fact the behavior is wrong means there's no mathematical basis - or the wrong mathematical basis, so simplifying.
        %only a mathematical basis and
        no physical basis.
        As discussed in the text, the one-mode model has persistent curvature
        ...
        To clarify this, we have modified the text
        \begin{quote}
          Text here:
          %MRS: Ben, choose where to put this.  It could be a simple one sentence modification or addition - you probably do explain why no 2-mode model, don't you?
        \end{quote}
  \item \textit{\textbf{Page 35 line 20-42:}\\
		...
		already gained.
	  	...
	  	%BJC: Could reasonably go beyond 50 nm but 7 microns becomes tough
                %MRS: probably should explain that you COULD have gone beyond 50 nm, but there was no need, since it didn't improve the fit (if that is true).
	  	...
	  	%BJC: truncated correlation functions may allow us to surpass the limitation
                ...
                %MRS: could also point out that we have come up with a
                %better approach along similar lines now (could cite arXiv paper), and so
                %continuing down this route is probably unnneeded.
...
\begin{addmargin}[5.8em]{0em}
    ...
    %MRS: yeah, we don't want people to have to read thoroughly twice.
