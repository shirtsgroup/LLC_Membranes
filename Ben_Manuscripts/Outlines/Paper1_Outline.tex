...
will provide evidence to support or call into question past drawn conclusions that have largely guided our understanding of separation mechanisms. 
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Calculations based on the volume of the liquid crystal suggest that there are seven monomers in each layer 
			  ...
			  %BJC: This is meant to reference a paper where this calculation was done so it's technically past work
	...
	\begin{itemize}
		...
		\item We are primarily interested in reproducing the conclusions about structure which have been made from XRD experiments and ionic conductivity measurements.    
		  ...
		  %BJC: so don't mention it here then? maybe save it for the methods
...
%BJC: let's brainstorm the right way to justify	
	...
	%BJC: I thought the same thing but just put it here temporarily
...
%BJC: It can stay stable, although there is an apparent shift away from perfect stacking with time. See results of measuring overlap
...
not be stable in a laboratory setting and should not be studied further~\label{table:p2p}}  
	...
	\begin{itemize}
		...
		\item The arrangement of sodium ions (which are closely bound to carbonyl head groups) can be well-approximated by a gaussian distribution % BJC: supplemental info probably %MRS: anything can be fit to.  Do you mean well-approximated by?
...
%BJC: They are the same. See journal entry from week of 4/21/2017
			%BJC: New point vv
		...
		\end{itemize}
		\item Both phases show a ring in the simulated 2D WAXS patterns at q $\approx$ 1.4 \angstrom\texsuperscript{-1} which is typical of packed alkane chains
	...
	%BJC: generated 1D SAXS somewhere?? Just to show that we get the spacing right
        ...
        %MRS: Not clear what ``questions to answer from here'' means.  If it's questions beyond the scope of the paper, should go in discussion.  Rephrase to be clearer thesis topic.  Or is this just notes for us?
	%BJC: Sorry, meant this to be more along the lines of "Questions for us to answer before submitting this paper"
	...
	%MRS: seems like needs to be a bit more discussion about 5 vs 6 vs 7, and which is most likely? 
        %MRS: will have to discuss a bit more about pi-stacking in GAFF and in reality, and how we might expect that will change things.
