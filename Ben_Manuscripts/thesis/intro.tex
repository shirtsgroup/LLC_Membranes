...
%MRS: maybe talk about molecular separations are particularly hard (to set up the work you do).
  \end{itemize}
  ...
  \begin{itemize}
    ...
    %MRS: no explicit pores might not quite be right. . . because there are pores there, just not designed pores. tortuous Pores formed by collapsed polymer networks? 
...
%MRS: provide more of a context for this discussion.  Why are your reviewing this theory here?  
%MRS: are you assuming solution/diffusion here? If so, state explicitly. 
%MRS: OK after reading through, I have a suggestion.  Maybe the theory section should to nearer to the end.  At the beginning you talk more about membrane structure and applications, and talk about more traditional membranes, and then pivot to new membranes. Maybe you should stick with structure/function until you talk about the solute motion, and how it's NOT simple diffusioin, then talk about standard theory, then talk about the the need for alternative theory, and then introduce the ideas you will describe in much more detail in the chapters corresponding to your later papers.  Does that make sense as a strategy?
...
%MRS: Maybe the applications be presented before theory?
...
separations occur.
  ...
  %MRS: is explicitly defined the right word?  ``Created pores?''
...
%MRS: probably could generalize to self-assmbled porous materials as
%the class in general, and the cross-linked LLC's as a particular exemplar.
  ...
  %MRS: be more explicit what you mean by ``study''.
  ...
  \begin{itemize}
    ...
    %MRS: could discuss concept that the effective pore size depends on the solute.
...
%  macroscopically align the hexagonal mesophases. 
  ...
  %MRS: be more explicit that this is synthesis of usable amounts of
  %actual separations membranes, not the molecules themselves.
  ...
  \begin{enumerate}
    ...
    \item Can we estimate experimentally-relevant macroscopic properties?
    	...
    	\end{itemize}
    %MRS: this might be overly ambitions.  Maybe something more like: How can the timeseries data inform us about the mechanism directly?
