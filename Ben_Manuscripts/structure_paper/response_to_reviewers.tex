...
%    such slow dynamics.
    ...
    work, 
    %MRS: suggest reprhasing as below?XS
    %we will validate whether GAFF is sufficient for reproducing trends in
    %experimental separation properties. 
    ...
    %MRS: suggest pointing out that the LLC's here are not really characteristic of 
    %more commonly studied monomers, so using those LLC force fields isn't really possible.
    ...
    system we study, 
    %MRS: Not really regardless above, as we only checked systems that were roughly right.
    ...
    regime. 
    %MRS: tweaked.
    %It is not feasible to carry out these simulations as it would
    ...
    %MRS: for above: Could you propose a textual response to deal with the confusion?
    ...
    %MRS: color is only used twice in this response, and it is not explained at the top 
    % what it is supposed to indicate.  Either that should be clarified and it should be used consistently, 
    ...
    %to vast 
    transferably to a large 
    number of systems. The ultimate goal of our work is to create new monomers
    ...
    We have performed checks to ensure that the
    %MRS: can you be more specific what is meant by ``works properly''?
    ...
    In this case, the simulated diffraction pattern will be Gaussian.
...
%MRS: clarify diffusion in water. 
    ...
    %MRS: tweak
    %As a compromise, we've made efforts to shorten the manuscript by 
    ...
    %MRS: tweak
    %The following changes made this possible:
