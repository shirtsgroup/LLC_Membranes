...
\begin{document}
  ...
  %BJC: need references
  \begin{enumerate}
	\item Create monomer structure file with connectivity : We drew atomistic
        structures using MarvinSketch 17.13. We optimized the 3D geometry of
	the structure using the 'Clean in 3D' function of MarvinSketch 17.13. 
        We saved the structure as a .mol file, then converted it to .pdb
	format using Open Babel 2.4.1. 
	\item Assign GAFF atomtypes using \texttt{antechamber} : Using the .pdb
	structure file as input, we ran \texttt{antechamber} using the AM1-BCC charge
	model. The net charge on the monomer is input as -1 since the sodium ion will
	be kept as a separate residue. The output of \texttt{antechamber} is used to
	create Amber topology files with \texttt{LEaP}. A detailed tutorial can be
	accessed elsewhere (http://ambermd.org/tutorials/basic/tutorial4b/) 
	\item Create gromacs topologies from Amber output : \texttt{LEaP}
	outputs a .inpcrd and a .prmtop file which are Amber topologies. Using
	acpype.py, we convert the \texttt{LEaP} output into GROMACS .gro and .top
	files. 
  \end{enumerate} 
  ...
  \begin{enumerate}
	  ...
	  %BJC: Could put vertical lines to indicate where position restraint values are reduced
		...
		\caption{The pore spacing during the restrained portion of the
			...
			KJ mol$^{-1}$ nm$^{-2}$, pore spacing changes erratically before stabilizing
			when force constants are reduced to 3 KJ mol$^{-1}$ nm$^{-2}$.} 
		\label{fig:p2p}
	  \end{figure}
	  ...
	  Table~\ref{table:radii}. The pore radius in each frame is calculated as the
	  ...
	  representing the average pore radius at each frame.  Equilibration was detected
	  using \texttt{pymbar.timeseries.detectEquilibration}. The average and standard
	  deviation of pore radii were calculated using only data points collected after
	  ...
	  \item Initial distance between layers
          ...
          We tested 3 different initial layer spacings, defined as the distance between the planes of 
          aromatic rings in each layer. Systems built with layers stacked 3.7 \AA and 5 \AA apart
	  are discussed extensively in the main text. We will focus the discussion here on systems
	  built with layers stacked 10 \AA apart. 
