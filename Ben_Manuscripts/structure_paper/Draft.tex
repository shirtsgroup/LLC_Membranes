...
%  to experiment. We believe systems built with 5 columns per pore achieve this goal.
  ...
  data. We generated simulated patterns using the extended of simulation trajectories
  ...
  simulated patterns. They are also partially engulfed by the wide R-$\pi$ reflection. R-$\pi$
...
%  that it does not appear in our simulated patterns in Section~\ref{section:rdouble}.
  ...
  %BJC5: not sure above sentence is clear (or necessary to say)
  ...
  ~\ref{table:relative_inensities_300K} shows the relative intensities of each reflection for 
  all systems tested. The patterns are normalized so that the average intensity of R-alkanes 
  must equal 1.
  % BJC5: show how intensities are measured in supporting info
  ...
  cases. There is no R-double intensity to be measured. Figures are provided in 
  section S\ref{S-section:xrd_intensities} to make these measurements clear.
...
%  \end{figure}
  ...
  %BJC5: add figure reference and figure to supporting info
...
%  model with the closest match to experimental data.
  ...
  decorrelation of ether dihedrals after $\approx$ 11 ns with a plateau at 0, 
...
% line is thickest. That way, when the outlines overlap, you can see the ones in front.
  ...
  % BJC5: maybe move this to supplemental. I think rdf and g(x,y) should be 
  % enough to comment on the effect of water
...
%MRS6: I think we should remove the ``we learned'', and express it more in terms of the observed facts: rather than ``we learned statement X'', you can just say ``statement X''. Or ``observation Y in the simulations supports the conclusion Y''.  
  ...
  because monomer stack too far apart. We found that our model cannot reproduce R-double, 
  however we have come up with a three different mechanisms that lead to the reflection's
  appearance. The most likely explanation is that monomers within each column rotate ca.
  15\degree with respect to their vertically adjacent neighbors so that tails can pack 
  ...
  ions. All systems studied showed a similar distribution of sodium, head groups 
  and tails, which means we can study solute transport in any of the systems here
  and extract similar transport mechanisms. We also observed that there is not a 
  hard partition between hydrophlic and hydrophilic regions, rather there is a 
  gradient. This finding has raised questions about the nature of size-exclusion separations.  
