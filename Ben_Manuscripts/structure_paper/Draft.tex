...
%  \end{enumerate}
  ...
  configuration in the ordered basin as an example, however all systems display this behavior.
  ...
  is larger than experiment. Note that the experimental data is best fit with a Lorentzian 
  ...
  \end{subfigure}
  \caption{(a) Experimentally, R-$\pi$ broadens in the $q_z$ direction with a FWHM of 0.110 $\pm$ 0.003 $\AA^{-1}$ when
  ...
  conditions, forcefield or initial configuration. We assessed each using a simplified 
  ...
  configurations. We measured thermal disorder by measuring the standard deviation in
  ...
  of head group center of masses from their ideal positions. In the z-direction, we measured
  ...
  \bottomrule
  \end{tabular}
  \caption{We measured the deviation of the positions of the center of mass of head groups from
  idealized positions so that we could incorporate the same disorder into our model system.}
...
%  \begin{itemize}
  ...
  center and outer peaks. (c) Correlation between scatterers in the z-direction causes
  ...
  with the same variance, v, implied by Table~\ref{table:quenched_disorder}. We 
  ...
  \begin{itemize}
  	...
  	R-$\pi$ profile to look like that shown in Figure 8d).
