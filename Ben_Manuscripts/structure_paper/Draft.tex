...
%  to experiment. We believe systems built with 5 columns per pore achieve this goal.
  ...
  data. We generated simulated patterns using the extended simulation trajectories.
  ...
  to sample enough independent configurations. Increasing the number of trajectory frames
...
% line is thickest. That way, when the outlines overlap, you can see the ones in front.
  ...
  concentrations from 1 to 5 percent. Figure~\ref{fig:solvation} shows the simulated
  ...
  \end{subfigure}
	...
	of water only increase discrepancies with experiment.}%
  \label{fig:solvation}

  \end{figure}
 
  The addition of water allows R-double to appear. When we add 1 wt\% water to the 
  ...
  experiment. If water is truly responsible for R-double, then the there is likely
  between 1 and 2.5 wt\% water absorbed into the membrane. This does not exclude 
...
%  amounts of water are responsible for the existence of R-double.  
  ...
  Water causes the monomer head groups to restructure. We confirmed that the reflection is 
  ...
  \end{subfigure}
  \begin{subfigure}{0.32\textwidth}
  \includegraphics[width=\textwidth]{rdouble_nowater_rzplot.png}
  \caption{Water removed}\label{fig:rdouble_nowater}
  \end{subfigure}
  \begin{subfigure}{0.32\textwidth}
  ...
  R-double in (a). Removing water from the trajectory (b) results in a simulated 
  diffraction pattern nearly identical to that in (a). R-double is not present
  when we simulate the diffraction pattern of system where only tails are present. 
  (c)}\label{fig:rdouble_origin}
  \end{figure}
...
%  \end{figure}
  ...
  %BJC5: probably should show solvated system measurements
  % 1 wt % water, parallel displaced, ordered basin
  % Nernst Einstein : (6.52 +/- 0.04) e-05 S/m
  % Collective Diffusion : (20.6 +/- 9.8) e-05 S/m
