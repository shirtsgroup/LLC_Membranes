...
\begin{document}
  ...
  %MRS7: some more of the conclusions should be in the abstract.  'introduction' just raises questions, but 
  %'abstract' should give a executive-level summary of the answers.
  \end{abstract}
  ...
  %MRS: abstract seems a bit limited, describe more generally (and
  briefly!) other things selective membranes are good for. The paper is primarily about selective membranes, not fracking water.  It seems out of balance to be talking about only one application.
...
% MRS7: Do a pass for clarity at the sentence level.  For example ``degree of stochasticity'' is vague.
...
% BJC5 : This is how it is described in Xunda's papers. Should I shift to something
% that doesn't rely on stacked disks? Maybe illustrate monomers stacked into
% columns instead?
  \begin{figure}
	\centering
  ...
  %MRS7: changing future tense to present tense here.
  ...
  \centering
	...
	\end{subfigure}
    %MRS7: Adjusted the phrasing
    \caption{(a) Previous understanding of the pores are essentially speculations based on limited chemical and experimental data. (b) We use a detailed molecular model in this paper in order to appropriately model the pore's
      complex architecture which is crucial to understanding the mechanism of solute
	  transport. In both pictures, the head group region is colored blue and the tail region is
	  colored cyan. 
          %MRS7: don't think this is needed.
          %This image was generated from the molecular simulations run for this paper.
          }~\label{fig:detail}
  \end{figure}
 
  %Here 
  In this paper,
   we develop a molecular model of the Col\textsubscript{h} assembly formed by
...
%  since it encodes all structural details down to the sub-nm scale.  
  ...
  \begin{figure}[!htb]
        ...
        \end{subfigure}
        %MRS7: picture says R-helix still.
  ...
  \begin{enumerate}
    ...
    \label{point:monomernum}
 	...
 	layer~\cite{resel_structural_2000}. 
        ...
        %MRS7: is this really a liquid crystalline system, even if it is made of liquid crystal monomers? Should we say
        % in many liquid crystal systems?
 	...
 	hexagonal array of pores (xy plane) and short range correlations in the z-direction.
        %MRS7: look carefully at tenses.  Don't describe the research in future tense. 
        % if it's describing your presentation of the research in the paper , use present tense. 
        % If it's describing something you did, use past tense. I changed some of them, but not all.
 	%We will use 
        In this study, we use
        our atomistic molecular model to study how the system's structure is 
        affected
 	%effected 
        by the density of monomers surrounding each pore's hydrophilic core. 

	\item What structural motif best matches experimental 2D WAXS patterns?\label{point:xrdmatch}

	On the short timescales accessible to MD
        %MRS7: added the following to make it clear the scale of the effort
        (even the 100's of nanoseconds of simulation peformed here are short compared to experimental timescales),
        we observe distinct metastable configurations
	...
	%BJC5: I could add brief references to other LLC studies which look at hexagnoal phase, but
...
%  \end{enumerate}
  ...
  %BJC5: section showing self-assembly set up to be added
  ...
  %MRS7: refer to supporting information (if it's there) for more discussion on stability as a function of height?
...
%  information.
  ...
  %MRS7: could put this mostly in supporting. 
  ...
  %MRS7: is there a typical total time as well?
...
%  of mass to the pore center was aligned with the positive x-axis. 
  ...
  %In order to fully match synthetic procedures, 
  In order to maximally match the experimental membrane synthesis process
  we created a cross-linking
  algorithm that one can apply to equilibrated structures. 
  %MRS7: I think the main purpose of crosslinking is mechanical stability.
  The purpose of
  ...
  %MRS7: Since the initial structure isn't that well captured by experiment, then it's probably
  % better to say that we are interested in knowing how much and in why way cross-linking modulates the structure. 
  the consistency of the final structure with experimental structural data. 

  %BJC5: perhaps this paragraph should be moved to supporting info.
  %MRS7: probably?  Describing the _idea_ of the procedure can be in
  %main paper, but perhaps more details of the procedure in the supporting info. 
...
%  close to experiment.   
  ...
  % BJC5: trying things out with 'columns per pore' instead of 'monomers per layer'.
  ...
  % BJC5: this achemso format does not have section numbers so I can't reference sections
  ...
  For simplicity, we 
  %MRS7: tenses
  %will group the systems studied here into
...
%  to experiment. We believe systems built with 5 columns per pore achieve this goal.
  ...
  %BJC5: thesis needs rewording.
  %MRS7: probably, since the disorered - 6 are close to the experimental distances.  Is it accurate to say
  %that they don't have the experimental structural features? 
  ...
  \begin{figure}[!htb]
	...
	\includegraphics[width=\linewidth]{p2p.png}
        %MRS7: need to deal with the fact that layer-6, more disordered are within error of 4.12
        %MRS7: I think the answer is that the equilibrated distance is much bigger. you talk about the INITIAL distance being bigger, but I think you also need to emphasize that it stays there.
  ...
  data. 
  %MRS7: sentence above reads strangely. Was ``more'' sufficient? what criteria was used to stop at 10?
We generated simulated patterns using the extended simulation trajectories.
  ...
  % BJC5: I think something like this has to go somewhere.
  The diffraction patterns show some noise, especially along the axis where $q_r$=0. This
  noise can be suppressed by sampling more uncorrelated equilibrium configurations (See 
  section S\ref{S-section:xrd_noise}). Unfortunately, our simulations are not long enough
  to sample enough 
  truly
 independent configurations. 
 %MRS7: two things going on - they are not smooth because of lack of sampling WITHIN the metastable basin (somewhere in there mention angle averaging leads to less data at q=0 than anywhere else, but this is separate from being truly independent, which would mean totally converged in the equilibrium ensemble (i.e. reaching a single equilibrium ensemble in the >miliseconds timescale)  Increasing the number of trajectory frames
  used to simulate the structure factor within a given interval of time does not offer 
  any improvement.
%MRS7: not sure what you mean above. Increasing the frequency of frames used? 
  %BJC5: put figure letters in upper left hand corner of each pattern.
  \begin{figure}[!htb]
	\centering
	...
	\end{subfigure}	
        %MRS7: be consistent on capitalization
	...
	In all cases, R-double is not present.}~\label{fig:XRDsim} 
        %MRS7: may need to make clearer the differences between the simulated structures. I think its worth pointing out
        %that although rdouble isn't there, for the more structured ones, there is a 7.4 band, even if it disappears at q=0.
  \end{figure}
  ...
  % MRS4: comments from Matt suggest that domain misaligment shouldn't alone be responsible for 
...
%  that it does not appear in our simulated patterns in Section~\ref{section:rdouble}.
  ...
  %BJC5: reference something like a dna diffraction paper
  ...
  and justified. We will explore 
  the 
  %MRS7: possible or likely origin
  origin of each experimental reflection and form structural
...
%  columns\cite{chaikin_principles_1995}. 
  ...
  not present. We will use 
  %MRS7: be more specific when you say ``this configuration'' Just the simulation at 280K? The system is a set of configuraitons. 
this configuration to more thoroughly explore the origin of R-spots.
  ...
  %MRS7: slightly more than experiment; 1.3 and 1.5 are pretty comparable given the scale of other things.
  %MRS7: a and b aren't explained. 
  \end{figure}

%MRS7: maybe ``evidence strongly suggests that?''  
...
%MRS7: a thought - moving some of figures 10,11,12 together with some subfigures, since they all communicate the same idea.
%11a is probably unneeded.
  \begin{figure}[!htb]
	\centering
	\begin{subfigure}{0.45\linewidth}
		\centering
...
%MRS7: what is the purpose of this paragraph?  I think most people would know why a lower T simulation would be more ordered (though the degree is not known).
  ...
  \begin{enumerate}
  		...
  		may not correctly model the behavior of the tails at 300 K (citation?).
                % look for papers on protien folding temperature dependence: see Lee-Ping Wang's AMBER-15fb paper
...
%MRS7: discrepancies between experimental and simulated R-\pi?  
...
%tail entropy, and for some reason ours is frozen in a configuration that is a bit too far apart. I see you talk about this later, but might need to be brought up again.
  ...
  since the peak heights do not decay significantly. 
%MRS7: perhaps ``do not show clear patters than can easily be fit to exponential decay?''
...
%MRS7: is this still true with water hydrogen information? do you need to clarify dry?
  There is no way to produce R-double without a more complex initial configuration. We 
  calculated the structure factor of simple systems 
%MRS7: a little more detail, e.g. simple idealized systems with points representing monomer head groups 
that mimic the parallel''.  Also, is ``neither'' correct? Did you do a few variants?
  ...
  a completely dry system. We will address this point in section~\ref{section:water}. There is
  a not a unique solution to this problem, however we can speculate based on what makes the most
  physical sense.
%MRS7: above: vague.  Define ``this problem''. Move ``speculate'' idea to next paragraph, since that describes the speculation?  
  ...
  Simulations of 
  %this type of system 
  unevenly spaced systems are only stable if position restraints
  ...
  way for R-double to appear in a dry system. The long-term stability of a configuration
  ...
  %BJC5: this section may fit better after the next one?
  %MRS7: yes, put it after water structuring.
  We observe unusually slow dynamics in our system relative to other liquid crysal
  %MRS7: are columnar liquid crystals the right comparison? Are these stickier? One could imagine that 
  % energetic considerations in the LLCs smush them tighter together, which makes it harder for them to reorient. 
  % in which case ours aren't atypical, but just not simulated long enough. 
  ...
  constants of monomers in each of the systems we studied (See Table S-\ref{S-table:msd}) 
  and learned that they are all on the order of $10^{-14}~m^2/s$.
...
%MRS5: let's talk about the implications of this (above); might want to move some of this into the SI.
  ...
  %MRS7: is ``structure'' also a good word to put here? 
  ...
  %MRS7: this section reads a little bit as ``and then, and
  %then''. Think how to make it more focused on core ideas to communicate. Thes is they are all about the same. Perhaps should be the beginning of the section?

  %MRS7: rephrase? Partition implies more abrupt.
  ...
  %MRS7: or could say that the size could be different depending on the the nature of the molecle. Could bring up
  ...
  %MRS7: above: may impede compared to what?

  In general, the composition of each region, particulary the pore region, is 
  similar between all systems. We believe we can solvate and study transport in any
  of the systems presented here and extract similar transport mechanisms. We will 
  need to verify that this assumption is true. 
  
% BJC5 : I think this is making progress towards a better representation of these plots. I
% outlined the tops of the bars on the histograms so the individual rdf don't get covered.
% I think it helps. I also varied the thickness of the outlines so that the farthest back 
% line is thickest. That way, when the outlines overlap, you can see the ones in front.
%MRS7: looks weird, though - we should discuss more. Maybe just draw the tops of the histograms, and don't fill them in?
%MRS7: maybe do as three figures across?
  \begin{figure}[!htb]
  \centering
  ...
  \end{subfigure}
  %MRS7: lead with ``pretty much the same''
...
%  \end{figure}
  ...
  %MRS7: hmm. Good question. Probably, as long as it's pretty straightforward. For diffusion, you can do just one calculation, and put the alternate method in SI (you can mention you did it, i.e. we did X, it was noiser, but agreed within data (see SI). 
  ...
  experiment. Our models give reasonable estimates of ionic conductivity when
  ...
  % names of equations capitalized?
  ...
  \begin{figure}[!htb]
        ...
        agreeing values of ionic conductivity for both systems simulated. Both methods
        estimate the value of the ionic conductivity to be an order of magnitude higher 
  ...
  We applied our crosslinking algorithm to equilibrated sandwiched and parallel
  displaced configurations in the ordered pore basin. We allowed the crosslinking
  algorithm to propagate until greater than 90\% of vinyl groups were either involved
  in a crosslinking reaction or were terminated. See section S\ref{S-section:xlink}
  for further details on the crosslinking algorithm. We allowed the crosslinked
  ...
  they are crosslinked (See Figure ~\ref{fig:xlink}). The ionic conductivity of 
  ...
  values of crosslinked configurations fall within uncertainty of their un-crosslinked
  counterparts. The correlation length decreases in both systems, 
  %MRS7: from what to what?
