...
%  origin of R-spots.
  ...
  R-spots originates from the monomer tails. By removing all non-tail atoms from 
  the trajectory and simulating a diffraction pattern with the remaining atoms, 
  we were able to isolate the cause of the spots to the tails 
  (Figure~\ref{fig:tails_rzplot}).

  The tails pack together hexagonally. We plotted the center of mass of the 
  ...
  mass and its nearest neighbors with respect to the $xy$ plane of the membrane. 
  We see distinct peaks in the distribution of these angles located ca. 
  $-60\degree$, $0\degree$, and $60\degree$ which is consistent with a 
  hexagonally packed configuration (Figure~\ref{fig:layered_tails}). This 
  ordering dies off at the tail ends, where there is more space to fill 
  causing them to pack nearly isotropically (See supporting information). 
...
%  \end{enumerate}
  ...
  % BJC9: This still needs to be worked out. Rest of section should still hold up
  ...
  configurations and initial velocity randomization can
  ...
  spaced points surrounding the pore center. This method for calculating 
  quenched order inherently includes thermal disorder. The values measured for
  each system and type of disorder are given in Table~\ref{table:quenched_disorder}.
  ...
  noise to each subsequent frame. In this scenario, the positions of each scatterer
  will not change in a way that significantly affects the intensity of R-$\pi$
  since most of its character is defined by the far more disordered initial configuration.
  To uncover the expected intensity of R-$\pi$, we would need to average over many
  such trajectories. To bypass this expensive calculation, we instead calculate the
  R-$\pi$ intensity from a trajectory consisting of many independent initial 
  configurations. This simulates the long time scales over which we expect columns
  to sample many configurational states. %We use the latter method for the remainder of
%  this section since we can extract the trends we need using a single trajectory at
%  a fraction of the computational cost.
 
 %BJC9 : Could show some number like the following
%  The average R-$\pi$ intensity of 40, 100 frame trajectories created in this (1st way mentioned above)
...
%  with the mean of 40 simulations.  
...
%  agreement with experiment.
  ...
  thermal noise of our parallel displaced simple system in the $z$-direction by
  ...
  the sandwiched and parallel displaced systems. 
  %BJC9: ^need to prove that in supplemental?  
  Allowing columns to move
  ...
  R-$\pi$ generated from our simulations versus experiment is primarily due
