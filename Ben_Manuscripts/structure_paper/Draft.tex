...
%MRS9: should be a little more clear why you mean by ``all systems''. In some sense the problem is more that the best explanation might be unaligned columns, which is easier with sandwiched, which are 15 times as much. So here it would make more sense to focus on both types of columns, so you can explain how unaligned (sliding) columns also have a mechanism for reduction. Note that I don't think the columns have to be dynamically sliding (Which would be hard with all the tails entangled) but can be just off-register statically.
  ...
  are in moderate agreement with experiment (Figure~\ref{fig:rpi_exp_comparison}). The
  ...
  spectra do not incorporate the effects of pressure broadening and instrumental broadening
  ...
  %MRS9: Could be stronger thesis.  Suggest
  We have identified a number of potential ways that simulation conditions, forcefield 
  and initial configurations could affect the intensity of R-$\pi$ as well its FWHM in 
  ...
  the influence of thermal noise in each dimension, correlation between scatterers within
  columns, as well as correlation between columns since these are the variables we 
  believe have the largest influence on peak shape.
  ...
  early equilibration and is locked into place for the remainder of the simulation time. The
  ...
  different quenched configurations. We measured thermal disorder by measuring the standard 
  deviation in the distribution of head group center of mass positions from their average 
  ...
  the pore center. The values measured for each system are given in Table~\ref{table:quenched_disorder}.
  Using these parameters, we built a trajectory consisting of 1000 independent configurations. 
  We also made the simple system 4 times taller in order to get better resolution in the $q_z$ 
  direction. Figure~\ref{fig:sf_qy_nocorrelation} shows the $q_y$ cross-section of R-$\pi$ 
  generated by simulating the structure factor of the trajectory.
...
%BJC9: I could probably get rid of qy cross-sections and inset in (c)
  ...
  \caption{}\label{fig:sf_qy_correlation.png}
  \end{subfigure} 
  \begin{subfigure}{0.3\textwidth}
  \includegraphics[width=\textwidth]{sf_qz_correlation.png}
  \caption{}\label{fig:sf_qz_correlation.png}
  \end{subfigure}   
  ...
  peaks. (b) When we make points in each column correlated, the intensity of R-$\pi$ 
  increases but the sharp peaks remain with the same ratio of intensity between the
  center and outer peaks. (c) 
  %MRS9: I'm not sure what you mean by ``correlation'' broadening peaks here.  Do you mean replacing exact periodicity by 
  %correlated random points  Because that reduces correlation (from 100% to less than 100%).
Correlation between scatterers in the $z$-direction causes
  R-$\pi$ to broaden in the $q_z$ direction.}\label{fig:sf_correlation}
  % MRS9: in c, weird that the orange peaks are broader, but taller. Not sure I understand.
  % BJC9: Good catch, not sure why I didn't question that. Was inputting square root of standard deviation 
  % rather than square of standard deviation as variance, so there was less noise. Fixed now.
  % BJC9: Heights are still slightly different after 1000 independent configurations
  % BJC9: Shape is also different. Looks like the points are more evenly spaced in correlated
  % system so there are less points at high and low q.
  \end{figure}
  
  Scatterers in each column are correlated with each other. In the simplified 
  system described above, we added uncorrelated Gaussian noise in the 
  z-direction which ignores any interaction head groups might have with each
  other. To remedy this, we randomly sampled a multivariate normal distribution
  with the same standard deviation, $\sigma$ shown in by Table~\ref{table:quenched_disorder}. We 
  defined the distribution using a covariance matrix such that the covariance, v,
  in position, z, between scatterers decays exponentially from v according to the
  equation $ve^{-z/L}$, where L is the correlation length. Experimentally, the
  correlation length can be estimated as the inverse of the FWHM of the $q_z$
  cross-section of R-$\pi$.  %BJC8: needs references 
  ...
  of other reasons. %BJC8: the reasons I can provide more detail on above.  
  ...
  11\% and 28\%, we see a 3 and 15 fold decrease in the intensity of R-$\pi$ respectively (Figure
  ~\ref{fig:znoise}). Despite the decrease in intensity, the 3 cross-sections have the same shape,
  whose sharp Bragg-like peaks are not consistent with experiment. The peaks are spaced apart
  according to the pore spacing, and line up with R-alkanes. The center of mass of all columns in
  our initial configuration are situated at the z coordinate. 
  
  % BJC9: could overlay these plots, but might look messy. Maybe there is a more compact representation
  \begin{figure}[!htb]
  \centering
  \begin{subfigure}{0.3\textwidth}
  \includegraphics[width=\textwidth]{sf_qy_correlation.png}  %BJC9: z_variance = 2.074 angstroms
  \caption{}\label{fig:sf_znoise_control}
  \end{subfigure}
  \begin{subfigure}{0.3\textwidth}
  \includegraphics[width=\textwidth]{sf_qy_3xdecrease.png}  %BJC9: z_variance = 2.55 angstroms
  ...
  distribution bounded by (0, $f \times \mathit{d}_{equil}$) where $f$ is between 0 and 1. We
  limit this discussion to configurations close to a sandwiched configuration since it is not
  feasible for columns in the parallel displaced configuration, as we have set them up, to move
  past each other. Allowing columns to move independently smooths out the $q_r$ cross-section
  ...
  Increasing thermal noise in the $xy$ plane causes the FWHM of the $q_r$ cross-section of R-$\pi$
  to decrease.
  \begin{itemize}
    \item $q_r$ cross-section still too broad
    \item Adding more thermal noise sharpens it
    \item Figure: Overlay thinner FWHM plot on top of wider FWHM 
  \end{itemize}
  
  \begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{qy_fwhm.png}
  \caption{}\label{fig:qy_fwhm}
  \end{figure}
  
%MRS9: the statement below is pretty strong, need to back it up.  We know that we have quenched noise, which is a type of over-ordering - is that taken into account?  
  We have nearly the right amount of disorder / noise in our system, but correlation 
  between columns really messes up R-$\pi$.  
