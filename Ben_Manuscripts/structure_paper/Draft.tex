...
\begin{document}
  ...
  selective separations. In this work, we investigate an experimentally characterized 
  LLC membrane using an atomistic molecular model. 
  %MRS8: I think this is a little too subtle a point to be making for the abstract. 
  %I added a bit different spin on this at the end of the abstract to gesture towards the idea. Thoughts?
  %While it is relatively 
  %straightforward to create a stable hexagonal phase using atomistic modeling, we demonstrate that 
  ...
  %basins. 
  We use simulated X-ray diffraction (XRD) patterns in order to ensure 
  ...
  careful examination of the metastable states observed.
...
%MRS8: preliminary evidence has shown they may be?  It's more than a guess.
  ...
  examined in this paper are salts, and therefore lead to Donnan exclusion of ions
...
% columns instead?
% BJC6 : I will get to redo-ing this. Just takes some time to refamiliarize myself with
...
Atomistic molecular dynamics simulations can provide the required level of detail
  ...
  %BJC6: is this too bold considering the work done at University of Wisconsin?
  %MRS8: Maybe take out significantly
  In this study, we achieve a %significantly 
  more realistic atomistic description of LLC
  ...
  In this paper, we perform molecular modeling of the Col\textsubscript{h} assembly 
  formed by Na-GA3C11. Compared to the H\textsubscript{II} phase, the Col\textsubscript{h}
  phase is a simpler starting point. The system 
  %MRS8: check rephrasing on this, since we do say later it does have some water
  %has no water 
  is not assembled in aqueous phase
  which allowed us to simulate longer timescales, and there exists detailed experimental
...
%  since it encodes all structural details down to the sub-nm scale.  
 ...
 \end{figure}
  ...
  definitively answer. 
  %MRS8: grammatically a bit redundant.
  %There are a number of structural questions we wish to answer in this article. 
  ...
  \begin{enumerate}
	...
	ambient water are 
        %leached 
        absorbed
        into the system. The hydrogen bonding network formed by the 
	water may play a role in structuring the pore. We used simulated X-ray diffraction (XRD) patterns
	to see if there is any meaningful structural difference between a "dry" and a "wet" system.

	\item What is the 
          %MR8: chemical composition is a bit ambiguous; implies mole fractions, which isn't what we mean
          %chemical composition 
          detailed atomistic structure of the pores?\label{point:composition}
...
%  information.
  ...
  guidelines. For wet systems, we added the desired concentration of water to 
  the initial configuration and carried out equilibration in the same way
  ...
  of NVT simulations. We allowed the resulting unrestrained structure to equilibrate for
  ...
  %More precise details are 
  More equilibration details are 
  available in 
...
%  of mass to the pore center was aligned with the positive x-axis. 
  ...
  More details are provided in the Supporting Information, Section~S-\ref{section:ionic_conductivity}.
...
%  close to experiment.   
  ...
  quantified the ordering of the head groups using the nematic order parameter (see section S~\ref{method:nematic_order} for details of calculation). 
  %MRS8: the reference above isn't appearing. I'm not sure why after quick check.
...
%  to experiment. We believe systems built with 5 columns per pore achieve this goal.
  ...
  (Figure~\ref{fig:SAXS}). Ordered basin systems built with 4 columns per pore equilibrate
  to an average pore spacing 0.25 nm lower than experiment. Ordered basin systems built with
  6 columns per pore, have an equilibrated pore spacing ca. 0.50 nm higher than experiment.
  Monomers in disordered basin systems built with 6 columns-per-pore agree with experimental pore-to-pore distances 
  within error, but stack too far apart. Those in the disordered sandwiched and disordered
  parallel displaced configurations stack $\sim$4.87 and 4.94 \AA~apart respectively, 
  which is ca. 1.2~\AA further apart than suggested by experiment. 5 column-per-pore systems
  stack, at a maximum, 0.9 \AA~further apart than experiment (see Table~\ref{table:correlation_length}). 
  The remainder of this discussion will focus on the analysis of systems built with 5 columns per pore. 
...
%  origin of R-spots.
  ...
  more likely origin of R-spots, we determined which atoms gave rise to the 
  feature. 
  ...
  tails stay nearly flat, we plotted their centroids (Figure \ref{fig:centroids})
...
%BJC6: figures are in order that they are talked about in text.
  \begin{figure}[!htb]
  ...
  \end{subfigure}
  %BJC6: Brought this back to make 6 figures.
  ...
  \end{subfigure}
  %MRS8: nicely organized figure! I think in (c), you do want the red/green dots showing hexagonal symmetry to be bigger; 
  %when the figure is reduced they are hard to see.
...
%  Our final hypothesized dry configuration which produces R-double focuses on the orientation of the tails. 
  ...
  %MRS8: above: be more precise, for example, QM studies or crystallographic investigation of stacked pi-pi system, 
  % can cite the reference found.
  Our force field causes our system to tend towards uniformly spaced
  ...
  %MRS8: added below.
  There is little evidence from QM studies of stacked $\pi-\pi$ systems that this behavior is incorrect.~\ref{???} 
  
  Adding water to the system 
  %facilitates 
  appears to facilitate 
  the appearance of R-double,
  %MRS8: added above. I realize the H-bonding isn't brought up until later paragraphs, but I don't want to leave people in suspense; it serves as essentially a thesis of the next few paragraphs, which do group together.
  through the creation of hydrogen bonding of water to carboxylate groups of consecutive layers.
  We added water to the 
  ...
  in Figure S~\ref{S-fig:solvation}, however R-double is not present.
...
%  \end{subfigure}
  ...
  R-double appears in the solvated system due to the structure of the head groups. To 
  %prove 
  demonstrate
  this,
...
%  \end{subfigure}
  ...
  In general, the composition of each region, particularly within our definition of 
  the pore region, is similar between all systems. 
  %MRS8: weakened a bit
  This suggest that the details of transport may be relatively independent of the structural differences between
  different possible structures studied here.
...
%  experiment. 
  ...
  %MRS8: this section reference not working either.
...
%  Nernst-Einstein relation in future calculations of this type. 
  ...
  %BJC6: If I say that, then shouldn't I say it for all metrics? That sounds
  % a bit clunky especially when they are plotted in the figure and I just want to make
  % somewhat general comments on the effect of cross-linking.
...
%MRS6: I think we should remove the ``we learned'', and express it more in terms of the observed facts: rather than ``we learned statement X'', you can just say ``statement X''. Or ``observation Y in the simulations supports the conclusion Y''.  
  ...
  can only produce
  %reproduces 
  R-double when we add 1 wt\% water to the system. This observation has
  %important 
  possible
  implications for the reproducibility of the experimental results from Feng 
...
%  we add 1 wt\% water to the system.  
  ...
  and tails while the wet system shows evidence of 
  %MRS8: modified; water isn't changing things very much.
  slight
  swelling, with minor changes in the distributions due to the presence of 
  water molecules. We also observed that there is not a hard partition between hydrophilic
  ...
  size, which potentially could give rise to separations that vary with chemical identity as well as size. 
...
%  begin to swell, which may be more conducive to transport.
  ...
  %MRS8: added some.
  however, all changes are relatively minor, preserving most of the features well.
...
%  that is in good agreement with experimental data.
  ...
  %MRS8: better to rephrase what can be done (which is a present thing) instead of what we will do (which is future and undetermined).
  With the structural understanding gained by these simulations, 
  %we will
  it becomes possible
  evaluate transport of various solutes within the system, and apply the
