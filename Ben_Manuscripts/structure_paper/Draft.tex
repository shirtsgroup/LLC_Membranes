...
%  valuable resources which can be separated using highly selective membranes~\cite{sales_resource_2015}.
 ...
 % BJC14: (2) existing nanoporous membranes + drawbacks
  ...
  commercially 
  %MRS15: may be too strong an assertion.
  %dominant
  available 
  ...
  %MRS15: popilar for separations applications would imply the are getting a larger market share now, 
  %though my impression is that they are research topics of heavy interest, but not yet that commercially viable.  Check my chainge below.
  ...
  scale 
  %MRS15: not sure what you mean by ``fundamentally sound technologies''. THat the underlying principles are good, but that scaling is hard? Fundamentally sound seems to suggest that both principles and scaling are good, but that's not what the implication is.
...
%		  with vertically aligned nanopores}~\label{fig:soft} \end{figure}
  ...
  %BJC14: nothing new after this point
...
%  information.
  ...
  % BJC15: added to address Matt's concern
...
%  agreement with experiment.
  ...
  % BJC15 : Matt comment: "I found the discussion of pore structure in the first 
  % full paragraph on page 38 is a bit hard to follow. Maybe a schematic of what 
  % “in between sandwiched and parallel displaced” looks like would be helpful"
  The architecture of each column is therefore likely in between our sandwiched and parallel
  displaced configurations. The parallel displaced configuration that we have simulated in this
...
%  rearrangements of the system are much longer than what we can feasibly simulate.   
  ...
  Since the dynamics of this system appear to be quite slow, we can learn more about
  their time-averaged structure by studying ensembles of simulations.
  Disorder among head groups in the simulated atomistic systems is largely determined
  by each system's quenched configuration. We observe rapid reorganization of our 
  initial configurations into configurations with quenched disorder that does not
  vary significantly for the remainder of the % BJC15: I could quantify this more and put in supporting info
  simulation time. For that reason, we created ensembles of 40 simulations in both
  the parallel displaced and sandwiched configurations. We used the same equilibration
  technique as above (see section~\ref{section:equilibration}), however we stopped 
  the equilibration after 5 ns of unrestrained simulation. We simulated XRD patterns
  from the 40 concatenated trajectories. The $q_r$ cross-sections generated from the 
  two concatenated trajectories are shown in Figure~\ref{fig:ensemble_XRD}. 
  Qualitatively, the $q_r$ cross-sections of R-$\pi$ are nearly smooth, with small
  spikes near $q_r$=0. The data suggests that the average
  intensity of R-$\pi$ in sandwiched systems is about 3$\times$ higher than that in the
  parallel displaced configuration. Overall, the intensity of R-$\pi$ generated from the
  parallel displaced ensemble shows the best agreement with experiment so far. For
  simplicity, we will limit the following discussion to the parallel displaced 
  configuration. Equivalent data is available for the sandwiched configuration in the
  supporting information.
  %BJC15: limiting discussion to parallel displaced since it seems most consistent with experiment
  % However, I can switch it sandwiched very easily. 
  
  %BJC15: This figure might not be necessary. Do the comparisons of emperical
  % cumulative distribution functions make the same point? 
  \begin{figure}[!htb]
  \centering
  \begin{subfigure}{0.45\textwidth}
  ...
  \end{subfigure}
  \caption{The $q_r$ section of R-$\pi$ is much smoother when simulated XRD patterns
  		   are generated from an ensemble of 40 independent simulations. The maximum 
  ...
  Independent simulations exhibit similar amounts of quenched disorder. For 
  each, we measured the deviation of the head group centers of mass from their
  idealized positions (see section~\ref{method:simple_systems}). 
  We created histograms of the standard deviations of each distribution which
  are shown in Figure~\ref{fig:ensemble_stds}. The distributions of standard deviations
  are fairly tight with ($\sigma_\sigma_z$, $\sigma_\sigma_r$, $\sigma_\sigma_\theta$) = (0.011 nm, 
  0.007 nm , 0.022 radians). We pooled the deviations from idealized positions 
  from all independent simulations in order to create the distributions shown in 
  Figure~\ref{fig:offset_ensemble_pooled}. The distributions appear 
  ...
  this is the case for individual center of masses. 
  % For a given system, there are 400 head groups. 
  If we consider the center of mass of the first head group, we expect that the
  distribution of its deviation from its idealized position among all the 
  independent simulations
%  the $z$, $r$, and $\theta$ directions 
  should be symmetric about its idealized position. We can calculate the 
  empirical cumulative distribution function (ECDF) 
  of this distribution of deviations. 
  % Each ECDF contains 40 data points, one from each independent simulation.
  The ECDFs of 5 randomly selected centers of 
  mass are shown in Figure~\ref{fig:offset_ecdfs} (colored lines). We also calculated 
  ...
  confidence intervals, by randomly sampling 40 values (the same as the number 
  of independent simulations) from the distributions in 
  Figure~\ref{fig:offset_ensemble_pooled}, then measuring the deviation between
  the sampled ECDF and the full ECDF. The center of mass ECDFs lie between the
  error bars of the full ECDF within reason. This provides further evidence that
  each center of mass moves independently, in random directions when systems
  quench.
  
% Sandwiched: (0.019 nm, 0.007 nm, 0.023 radians)
  
  \begin{figure}[!htb]
  \centering
%  \begin{subfigure}{0.45\textwidth}
%  \includegraphics[width=\textwidth]{sandwiched_ensemble_stds.png}
%  \caption{Sandwiched}\label{fig:sandwiched_ensemble_stds.png}
%  \end{subfigure}
%  \begin{subfigure}{0.45\textwidth}
  \includegraphics[width=0.45\textwidth]{offset_ensemble_stds.png}
%  \caption{Parallel Displaced}\label{fig:offset_ensemble_stds.png}
%  \end{subfigure}
  \caption{There is a narrow spread in the standard deviation of
  quenched disorder for each independent configuration.}\label{fig:ensemble_stds}
  \end{figure}
  ...
  \centering
%  \begin{subfigure}{\textwidth}
%  \includegraphics[width=\textwidth]{sandwiched_ensemble_pooled.png}
%  \caption{Sandwiched}\label{fig:sandwiched_ensemble_pooled.png}
%  \end{subfigure}
  \begin{subfigure}{\textwidth}
  ...
  \caption{Emperical cumulative distribution functions generated from (a) and
  from simulation ensembles.}\label{fig:offset_ecdfs}
  \end{subfigure}
  \begin{subfigure}{\textwidth}
  \includegraphics[width=\textwidth]{offset_ensemble_regional_density.png}
  \caption{Radial density of select monomer components.}\label{fig:offset_ensemble_regional_density}
  \end{subfigure}
  \caption{The distributions of the deviations of head group center of masses
  ...
  or negative $z$, $r$ or $\theta$-direction.}\label{fig:full_ensemble_distributions}
  \end{figure}
...
%  that is in good agreement with experimental data.
  ...
  % BJC15: first pass at a brief future work paragraph
