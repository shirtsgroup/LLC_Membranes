...
%  of mass to the pore center was aligned with the positive x-axis. 
  ...
  quenched disorder inherently includes thermal disorder. 
...
%  experimental R-double, half the $q_z$ distance of R-$\pi$.
  ...
  temperature better resolved R-spots (Figure~\ref{fig:sandwiched280K}),
  ...
  First, we demonstrated that R-spots comes primarily from the tails by
  removing all non-tail atoms from the trajectory and simulating the
  XRD pattern with the remaining atoms, which preserves
  ...
  \caption{(a) R-spots increases in intensity when the temperature of the system is 
      ...
      Since the tails stay relatively flat, we plotted the COM of the first
      four carbon atoms of each tail originating from the head groups (for example,
      green colored COMs in the plot surround the red COM in hexagonal fashion).
      ...
      sandwiched configuration simulated at 280 K. Peaks appear in the distribution at 
...
%  groups within the same column. 
  ...
  Figure~S-\ref{S-fig:z_correlation_overlay} in the Supporting Information).
...
%  agreement with experiment.
  ...
  \caption{As we increase column independence using the $f$ parameter, the
	  ...
	  to 3.2. The edges of the peak are beginning to smooth out, but sharp peaks
	  still exist. (c) When columns are completely independent, $f$ = 1, the
	  intensity of R-$\pi$ decreases 8-fold to 2.1 and the cross-section is
...
%  the $z$, $r$, and $\theta$ directions 
  ...
  from the first 5 ns of the main (400 ns) ordered parallel displaced system studied in
  ...
  atoms, head group atoms and sodium atoms, as a function of distance from the pore center,
  calculated from the first 5 ns of the main system simulation trajectory (black lines) 
  and from the ensemble of trajectories (purple) look qualitatively similar. The 95 \%
  confidence interval generated based on the ensemble of simulations is shaded in purple.}\label{fig:ensemble_stds}
  \end{figure}
  ...
  ordering in the $z$-direction in addition to a high degree of correlation 
  ...
  the $xy$ plane, from the COM of their vertically adjacent neighbors 
...
% Following is in previous paragraph
%It appears that we cannot achieve a long-term stable
...
%MRS12: I think it is useful to put in just to show we've been thinking about them.  Can omit the figure in the main text.
  ...
  since the group extends the system's $\pi$
  conjugation~\cite{carey_organic_2011}, which is significant even considering
  possible overestimates of the barrier height in the classical force field (see
  Figure S-\ref{S-fig:carboxylate_dihedral_rb} of the Supporting Information). 
%  
...
%  carboxylate bond would be sterically hindered. 
  ...
  vertically adjacent monomers on the $xy$ plane. In this configuration, the LLC
  monomers are rotated so that the vector created by the bond extending from the
  carboxylate carbon to the phenyl ring is oriented $\pm 15 \degree$ with respect
  to the vector extending from the head group COM to the pore center (Figure
  ...
  stable for a few nanoseconds before R-double quickly fades. 
  ...
  evidence from QM studies of stacked $\pi-\pi$ systems that suggest uneven stacking
...
%  \end{subfigure}
  ...
  \caption{(a) When monomer head groups are rotated with respect to vertically
	  ...
	  fades once the position restraints are released (bottom). (b) When monomers are
	  non-uniformly spaced (top), R-double appears if all heavy atoms of the head
	  groups are held in place with position restraints (middle). Again, R-double quickly
...
%  the $\pi$-stacking distance.
  ...
  that these membranes are less likely to assemble under completely dry
...
%BJC16: is it an overestimate or underestimate? 
...
%  model with the closest match to experimental data.
  ...
  an unfavorable configuration. We implemented distance restraints between the COMs 
  of monomer head groups to preserve the hexagonal phase, then raised the temperature 
