...
%  close to experiment.   
  ...
  The pore-to-pore spacing is equilibrated, as defined in section~\ref{method:equil_time},
  ...
  systems built with 5 columns per pore. Systems built with 6 columns per pore, and 
  $\mathit{d}=3.7~\AA~$, have an equilibrated pore spacing c.a. 0.50 nm higher than experiment.
  Systems built with 4 columns per pore equilibrate to an average pore spacing 0.25 nm lower
  than experiment. 
  
  %BJC5: thesis needs rewording.
  Systems built with 6 columns-per-pore and $\mathit{d}=5~\AA~$ are not compact
  in the z-direction. The final pore spacing is close to experiment within its uncertainty.
  Relative to 6 column-per-pore systems started with $\textit{d}=3.7~\AA~$, the unit cell
  is elongated in its z dimension and contracted in its x and y dimensions, however the
  box volume stays nearly constant in all cases (Figure~\ref{fig:box_dimensions}). The system
  is likely not fully equilibrated and may eventually rearrange into something that resembles
  a 5 column-per-pore system. Further investigations into this are beyond the scope of this
  paper.
  ...
  prefer to stack at least 4.2 \AA~apart rather than 3.7 \AA. This behavior is not surprising 
  since GAFF models atoms as point charges and does not appropriately model the aromatic
  $\pi-\pi$ interactions which would cause the monomers to stack closer together. R-double 
  does not appear in any of the patterns. 
...
%  columns\cite{chaikin_principles_1995}. 
  ...
  (Fig.~\ref{fig:tilt}). 
   ...
   %BJC5: bootstrap this data to make it smoother.
  \begin{figure}
  \centering
...
%  \end{figure}
  ...
  %BJC5: redo after discussing with Michael
  \begin{figure}
  \centering
  \begin{subfigure}{0.45\textwidth}
  \centering
  ...
  %BJC5: conclusions here will need modification
  ...
  %BJC5: forgot an amplitude on exponential fit. New results show lower correlation length
  ...
  %BJC5: Something about disordered systems.
  ...
  %BJC5: this statement might not be true anymore. 
  ...
  % BJC5: could benefit from an adjusted colorbar experimental pattern.
  \end{figure}
...
% BJC5 : Matt wants  a section like this somewhere.
...
%  model with the closest match to experimental data.
  ...
  %BJC5: add diagram of which oxygens I'm talking about to the supporting info.
  ...
  %BJC5: supporting info section on this doesn't exist yet.
...
%MRS5: put a little more of the rational for looking at this here. Why should they care?
  We are most interested in the pore architecture. What's happening in the tails is unimportant.
  ...
  %BJC5: redo the numbers in this section.
...
% BJC5 : Move this figure to supporting
  \begin{figure}
  \centering
...
%BJC5 : play with color scheme to make this viable
  \begin{figure}
  \centering
