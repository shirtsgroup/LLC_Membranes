...
%MRS13: we might end up adjusting the author list a bit; I will ask Matt where he thinks he and Joe should go.  I will talk to Rich when we get this going; not clear that he actually has contributed.  But that's my job. 
  ...
  %MRSF: I changed the description of the ``demonstrating'', ``showing'' verbs to present tense.  The experiments are past tense, but the ``showing'' is present tense, since it is occuring while the reader is reading (like in a presentation - you are showing as they listen).
  \end{abstract}
  ...
  wastewater. 
  For exaple, 
  Sodium chloride and boron in seawater
  \cite{fritzmann_state---art_2007} and organic micropollutants found in
  municipal and industrial wastewaters \cite{schwarzenbach_challenge_2006} represent just a few of the diverse contaminants 
  of water sources. By efficiently
...
%MRSF: a few selelected edits above, but I think strikes the right balance.
...
%		  with vertically aligned nanopores}~\label{fig:soft} \end{figure}
  ...
  \begin{figure}
	...
	\end{subfigure}
        %MRSF: in the actual paper, there would only be room for about a half-page figure, max.  Can hyou reduce the especially the c/d/e sizes so that it fully fits in a half-page figure? 
  ...
  %BJC16: This figure could probably be cut or moved to supporting info
  \begin{figure}[!htb]
  \centering
	...
	\end{subfigure}
    %MRSF: will probably need smaller figures for the paper. 
...
%  \end{enumerate}
  ...
  %MRSF: code language is not as important
  %Python scripts 
...
%  patterns, R-$\pi$ intersects with R-alkanes.
 ...
 % within the regions highlighted in red in Figure~\ref{fig:rspots}. 
  ...
  %MRSF: specify below what 'here' is.  This graph, this study, this section?
...
%  of mass to the pore center was aligned with the positive x-axis. 
  ...
  %MRSF: How much greater? Be a bit more specific (10% greater?  100x times greater?)
  This method 
  %MRSF: specify exactly what of the preceeding is meant by ``this method''
...
%  close to experiment.   
  ...
  %MRSF: is there some comment that you can make above about the disordered ones not having many of the structures seen in WAXS to make it clearer that they are not as interesting from the get-go, and provide more details in the follow up sections?
...
%  to experiment. We believe systems built with 5 columns per pore achieve this goal.
  ...
  %MRSF: I',m experimenting with ``columns _around_ each pore'' instead of ``Columns _in_ each pore''. Around makes the geometry a bit more explicit. 
...
Those in the
  ...
  %MRSF: maybe use $\sim$ instead of ca., for consistency?
...
%  \end{figure}  
  ...
  %MRSF: be consistent on ``Section'' vs. ``section''. Check j chem phys b papers for which one is more correct. 
...
%  experimental R-double, half the $q_z$ distance of R-$\pi$.
  ...
  %MRSF: any uncertainty analysis for the 2% that can be included?
...
%MRSF: suggest breaking up this paragraph into 2-3 paragraphs.  Thesis is just that we studied ensembles of short simulations,but would be clearer stronger of some of the results are separated out into their own paragraph.
...
%MRSF: maybe here break?  might need a bit of rephrasing of thesis.
...
%MRSF: maybe split here?  Not sure, but definitely some split.
  ...
  %MRSF: add section number here, consistency on capitalization?
  ...
  \end{subfigure}
  %MRSF: explicitly refer to the previous graph that this is ``much smoother'' than.
...
%  involved are unequally spaced. These conditions seem likely to give rise to R-double.
  ...
  %MRSF: see before by 'we'.  You mean previous studies? Tried to tweak.
