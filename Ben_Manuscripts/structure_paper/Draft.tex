...
%  rearrangements of the system are much longer than what we can feasibly simulate.   
  ...
  Since the dynamics of this system are slow, and consequently most of
  our analysis has been focused on a set of quenched configurations with some structural characteristics that do not 
  vary significantly with time, we attempted
  to learn more about their time-averaged structures by studying ensembles of shorter simulations started
  from more independent initial configurations.
...
%  vary significantly for the remainder of the
%  simulation time. For that reason, 
  We created ensembles of 40 simulations in both the parallel displaced and 
  sandwiched configurations. We used the same equilibration scheme as above 
  ...
  looked at the results after 5 ns of unrestrained simulation, which should capture any quenched disorder built in 
  from the initial configurations. We 
  then simulated XRD patterns from the 40 trajectories, concatenated together. The $q_r$ 
  ...
  of are nearly smooth, with only small spikes near $q_r$=0. These data 
  %MRS15: for sentence below - qualify about the ensemble?
  suggest that the average intensity of R-$\pi$ in sandwiched systems is 
...
%  the $z$, $r$, and $\theta$ directions 
  ...
  sampled from the full distribution. The similarity between entirely indepdently sampled CDFs and the CDFs from individual independent simulations supports the that each quenched configuration is random and mostly independent.
...
%  \end{subfigure}
  ...
  \end{subfigure}
  \begin{subfigure}{\textwidth}
  % BJC15: I don't think it's necessary to put the 3 randomly selected ECDFs in since it
  % doesn't provide extra information and clutters the figure. Left there for now.
  % MRS15: kind of nice to guide the eye?
  ...
  frames of each independent configuration is symmetric, implying that 
  ...
  % BJC15: How important is it to prove in the main text (rather than supporting)
  % that the long systems sample the same distribution as above. Since they were
  % generated using the same procedure, why should the reader question it? 
  % MRS15: because they could all collapse to the SAME (or similar) quenched disorder, rather than 
  % different, mostly independent ones.
  
  The systems run for 400 ns and used for most of our analysis are
  ...
  % BJC15: The mean z-value seems a little silly since it is basically zero except for numerical error I presume
  % The mean is required to be zero for the calculation to work since columns are randomly displaced.
  %MRS15: good point.  Probably can leave that off. 
  \end{subfigure}
  \begin{subfigure}{\textwidth}
  \includegraphics[width=\textwidth]{offset_ensemble_regional_density.png}
  %BJC15: I can explicitly define the colors for each system since all the ugly colors come out on top
  %MRS15: sounds good.  Maybe a bit narrower lines?
  ...
  %MRS15: make it more clear that you are now summing up this section.  Perhaps put
  %subheadings in this section, as I discussed.
  ...
  % BJC15: could move figure to supplemental to save space
  % BJC15: Added a transition type clause to the beginning of this first sentence
  % to help with flow. It might be too much though.
  ...
  %MRS15: yeah, cut the last bit.

  %BJC15: figure in response to Matt.  
  %MRS15: This seems like something for supporting information.
  \begin{figure}[!htb]
  \centering
  \begin{subfigure}{0.45\textwidth}
  \centering
...
%  that is in good agreement with experimental data.
  ...
  structural agreement between experiment and simulation. 
  %We may be able to 
  %modify the GAFF force field or use
  Force fields explicitly including $\pi-\pi$ interactions may be able to draw stacked monomers closer together. 
  We can study ensembles of long simulations in order to observe more averaged 
  structure and to create more experimentally consistent XRD patterns. Extremely long simulations 
  (on the order of \SI{}{\micro s}) may be necessary to better 
  quantify the time scales for large scale rearrangements, and the precise arrangements themselves.
