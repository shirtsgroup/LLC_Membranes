...
%  of mass to the pore center was aligned with the positive x-axis. 
  ...
  carefully chose a scaling factor of $f=3.1$ in order to visibly display all
...
%  patterns, R-$\pi$ intersects with R-alkanes.
 ...
 % within the regions highlighted in red in Figure~\ref{fig:rspots}. 
  ...
  We measured the intensity of R-pores based on the intensity of the leading peak
  (closest to $q_r$ = 0) of the $q_r$ cross-section of the patterns at $q_z$=0
  (Figure~\ref{fig:saxs_xsection}). The beamstop covers most of the small angle
  reflections in the experimental 2D WAXS pattern. In order to compare the simulated
  intensity of R-pores to experiment, we used the $q_r$ cross-section of 2D SAXS 
  which was generated from the same sample. Since the d\textsubscript{110} peak is  %BJC12: verify which crystal plane it is.
  partially exposed in the experimental 2D WAXS pattern, we normalized the experimental 
  2D SAXS cross-section by matching the intensity of the d\textsubscript{110} peak 
  between it and the experimental 2D WAXS cross-section. It is possible that the peak
  is not fully captured in the 2D WAXS pattern and that our estimate of the intensity
  of R-pores for the experimental pattern is underestimated.     
  ...
  simulated patterns differ from experiment in those regions in all cases. (b) We
  ...
  R-pores by measuring the height of the leading peak (closest to $q_r$=0) of 
  the cross-section of the diffraction patterns along the $q_r$ axis at $q_z$=0.} \label{fig:xrd_intensities}
  \end{figure}
...
%  \end{figure}  
  ...
  diffraction patterns produced from equilibrated MD trajectories and comparing
  them to experiment (see section~\ref{method:xrd}). We tested systems built with
  5 columns per pore in the parallel displaced and sandwiched configurations at
  ...
  each reflection are described in section S-\ref{S-section:xrd_intensities}. 
...
%  columns\cite{chaikin_principles_1995}. 
  ...
  The error in the simulated diffraction patterns is less that the error due
  ...
  R-pores is about 4 times more intense than experiment in all simulated
  systems. We hypothesize that this is primarily due to the relatively perfect
  ...
  the exact location relative to the bin size used for the Fourier space
  ...
  simulations in Section~\ref{section:section:mon_per_pore}. 
  ...
  We measured the tilt angle of the alkane chains of the 280K system and found
  ...
  relative to the $xy$ plane to an angle statisically neglibible from zero. (See 
  Section S-TBD). 
...
%  groups within the same column. 
  ...
  does not appropriately model the aromatic $\pi-\pi$ interactions, which make
  ...
  system at 500K then annealed it to 300K in an attempt to coerce the tails into
  a more tightly packed configuration, however we saw no improvement in packing
  distance over our 300K models. See section S-TBD for details.
...
%  influence on peak shape and intensity.
  ...
  production simulations. We hypothesize that the quenched disorder is the 
...
%  agreement with experiment.
  ...
  of the head groups may need improvement.
...
It is difficult to physically justify this system. Systems built this way are
only stable if position restraints are placed on all head group heavy atoms.
Carboxylate groups quickly revert to the parallel position as restraints are
released. There is an appreciable energy barrier that prevents rotation of
carboxylate groups attached to phenyl rings since the group extends the
system's $\pi$ conjugation (citation) (See Figure
...
%  carboxylate bond would be sterically hindered. 
  ...
  We can also produce R-double if layers are not uniformly, but instead form
  pairs that stack less than 3.7~\AA~apart, with center of masses are spaced 7.4
  \AA~ from neighboring pairs of monomers (Figure
