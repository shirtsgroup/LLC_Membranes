...
%  \end{enumerate}
  ...
  R-$\pi$ is more intense relative to the experimental system. In this section, we use the parallel displaced
  configuration in the ordered basin as an example, however 
...
all systems display this behavior.
  %since it is the most experimentally consistent system we've studied so far. 
  The intensity of R-$\pi$ in the ordered parallel displaced system is approximately 3 times
  larger than experiment (Table~\ref{table:relative_inensities_300K}). 
  
  The full width at half maximums (FWHM) of the $q_r$ and $q_z$ cross-sections of R-$\pi$ 
  are in moderate agreement with experiment (Figure~\ref{fig:rpi_exp_comparison}). Each FWHM
  is larger than experiment. 
  %MRS9: state how much larger they are than experiment in the sentence before - big difference beween 30% bigger and 300% bigger.
Note that the experimental data is best fit with a Lorentzian 
  function while the simulated patterns are best fit using a Gaussian function. The experimental
  pattern also has additional sources of broadening which, if deconvoluted, would increase the discrepancy with
  simulation.
  ...
  \end{subfigure}
  \caption{
   %MRS9: suggest something like this to make crystal clear A and B are from the experiments.
  In experiments ((a) and (b)), 
%Experimentally,  (a) 
  (a), R-$\pi$ broadens in the $q_z$ direction with a FWHM of 0.110 $\pm$ 0.003 $\AA^{-1}$ when
  fit with a Lorentzian function. (b) R-$\pi$ broadens in the $q_r$ direction with a FWHM of 0.792 
  $\pm$ 0.009 $\AA^{-1}$ when fit with a Lorentzian function. (c) In our simulations, the FWHM of R-$\pi$ in
...
We have identified a number of potential ways that simulations conditions, forcefield and initial configurations could affect the intensity of R-$\pi$ as well its FWHM in the $q_z$ and $q_r$ directions
%Here we present our understanding of what factors influence the intensity of R-$\pi$ 
%  as well its FWHM in the $q_z$ and $q_r$ directions. We have identified a number of 
%  potential contributions which can be modified in our model by adjusting the simulation
%  conditions, forcefield or initial configuration. 
We assessed each using a simplified 
  model. Our simplified model represents each monomer head group as a single point and
  places them in either a parallel displaced or sandwiched configuration. We apply 
  various transformations in order to study its simulated diffraction pattern. 
  ...
  latter has important implications for the reproducibility of this work since velocity
  randomization at the beginning of equilibration will result in different quenched 
  configurations. 
  %MRS9: could be that initial starting configurations would ALSO give rise to different quenched configurations.
We measured thermal disorder by measuring the standard deviation in
  ...
  of head group center of masses from their 
%ideal positions. 
idealized average positions.
In the $z$-direction, we measured
  the deviation of the head group center of masses from an equally spaced column of head groups.
  In the $xy$ plane, we calculated the deviation in radial position from the pore center and 
  the angular deviation from equally spaced points surrounding the pore center. The values
  measured for each system are given in Table~\ref{table:quenched_disorder}. We built a simple
  system consisting of 1000 independent configurations using these parameters. We also made
  the simple system 4 times taller in order to get better resolution in the $q_z$ direction. 
  Figure~\ref{fig:sf_qy_nocorrelation} shows the $q_y$ cross-section of R-$\pi$ generated by
  simulating the structure factor of the trajectory.

  %BJC8: Need to add thermal noise to this table
  \begin{table}[h]
  \centering
  \begin{tabular}{c|ccc}
  \toprule
  System                        & $\sigma_z$ ($\AA$) & $\sigma_\theta$ (rad) & $\sigma_r$ ($\AA$) \\
  \midrule
  Ordered Sandwiched            &        1.48        &     0.43              &     2.30           \\
  Ordered Parallel Displaced    &        1.45        &     0.43              &     2.28           \\ 
  Disordered Sandwiched         &        1.58        &     0.43              &     2.93           \\
  Disordered Parallel Displaced &        1.65        &     0.43              &     2.63           \\
  \bottomrule
  \end{tabular}
  %MRS9: tweaked
  \caption{Deviation of the positions of the center of mass of head groups from
  idealized positions to use as simulated disorder into our model system.}
...
%  There is no major difference between R-$\pi$ of a parallel displaced system and a 
%  sandwiched system.
%  \begin{itemize}
%  	\item Intensity does not change
%  	\item 
%  \begin{itemize}
...
Correlation between scatterers in the $z$-direction causes
  ...
  implied by Table~\ref{table:quenched_disorder}. We 
  defined the distribution using a covariance matrix such that the covariance
  ...
  cases,
  %MRS9: added
  with disordered basins at the high part of that range 0.2 and 0.3 \AA~more distant.
...
This behavior is not surprising since GAFF models atoms as point charges and does not 
  ...
  favorable for the monomers to stack closer together. 
...
%MRS9: make it clearer if the last sentence is a conclusion based on the other evidence in the paragraph.
  ...
  the $q_r$ profile remains unchanged. 
  \begin{itemize}
  	\item Show how I can decrease the intensity by a factor of 3-15 x by just increasing z-direction
  	thermal noise
  	\item Can put thermal noise vs. intensity in supporting info.
  	\item The peaks are still sharp though
  	\item 3 panel figure : regular intensity, 3x decrease, 15x decrease. Peak shapes all the same
  \end{itemize}
  ...
  smooths out.
  \begin{itemize}
  	\item Allowing columns to move independently smooths out the $q_r$ cross-section. Any amount
  	of dependence results in sharp peaks.
  	\item R-$\pi$ intensity decreases by up to 10x
  	\item Can put R-$\pi$ intensity vs. column independence in supporting info.
  	\item Predict level of independence in simulation (what amount of displacement cause the 
  	R-$\pi$ profile to look like that shown in Figure 8d). %MRS9: this is a bit complicated, since it could be a combination of factors.
  	\item Figure: (a) x \% column displacement give Figure 8d (b) Independent columns give a smooth
  	curve %MRS9: not quite sure what you are talking about in this figure.  Could you give more information?
  \end{itemize} 
  
  Increasing thermal noise in the x and y directions causes R-$\pi$ to sharpen in the 
  $q_r$ direction. %MRS9: this is unanticiated (noise creating sharpening) so will have to make it clear.
  \begin{itemize}
...
%  supporting information for illustrative examples.
  ...
  %MRS9: added
  in the $z$-direction.
  does not significantly alter the intensity of R-$\pi$. We equilibrated a 
