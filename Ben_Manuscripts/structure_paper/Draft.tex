...
%  \end{figure}  
  ...
  %MRS10: Would be btter to lead with something describing what we found, not a caveat, just for clarity. Might belong better in the previous paragraph if left here, since it is a clarifying point that should be communicated, not an actual finding.
...
%MRS10: I rephrased the below, check the diffs.
  In order to more clearly study R-spots, we equilibrated an ordered basin sandwiched 
  configuration at 280K. Many force fields do not obtain the correct equilibrium structure at precisely the experimental temperature. Wang et al. highlighted the 
  shortcoming of some of the most popular protein force field in predicting the temperature
  dependence of protein structural ensembles \cite{wang_building_2017}. It is therefore possible that   % page 12
  our membrane system, simulated at 300K, is overly structured compared to the experimental structure at 300K.
  We found that lowering the temperature better resolved the R-spots (Figure~\ref{fig:sandwiched280K}), suggesting that our 'effective' temperature might indeed be too high, overstabilizing structure. 
...
from the simulations
  strongly suggests that the R-spots signal is not a result of alkane
  chain tilt. Previous literature has attributed the spots in this particular
%MRS10: clarified that it was THIS exact data, and not similiar images. Correct if wrong.
WAXS 
%pattern 
dataset
as the product of tilted alkane chains~\cite{feng_scalable_2014}.
  ...
  % to this figure?
  %MRS10: rephrased.
  The evidence strongly suggests that R-spot pattern occurs because tails pack together hexagonally. 
  We plotted the center of masses of the 
  ...
  %MRS10: is it importat that the ordering dies off? IF there is ordering, it will show in the spectra, and it doesn't matter that it dies off in other parts of the spectra.
...
%  \end{enumerate}
  ...
  %MRS10: in the list of questions above, you just talk about intensity, but this title talks about more discrepnacies. It could go either way (as I mention before, there's the issue of R-pi placement, too), but it should be consistent.
  The experimentally measured R-$\pi$ is more intense relative to the experimental system. The difference in intensity
  ...
  %MRS10: should specify this is in the 300 K simulations.
  ...
  The true electronic structure is likely much more complicated. 
  %MRS10: modified
  However, we can explore reasons for which differences in the simulation structure could changes the peak shapes to be in better agreement with experiment.
...
%  of the ordered parallel displaced configuration ((b) and (d)). 
    %MRS10: don't make the thesis what was done, but what the reader should know. 
  ...
  %MRS10: somewhere in here, we need to address whether quenched and static disorder are 
  ...
  %MRS10: which are the important implications?
for the reproducibility of this work since both 
  ...
  %MRS10: again, more of a methodolgical thesis.  Will not be clear to the reader what they are 
  % expecting to learn, which is the difference in the simulation that could explain the differences in the spectra.
  % I'm thinking some of this should be in the supporting info.  Maybe not, if key enough, but it should still be in support of the conclusion. 
...
%BJC9: hahaha^
  ...
  %MRS10: OK this is finally addressed here, but it it's sort of buried in the middle of a paragraph, and not laid out as an outlined point, whereas most readers would ask about it early.
...
%MRS10: I wonder if it would be better to lead with what we think IS the answer, and then rule out other effects.  
