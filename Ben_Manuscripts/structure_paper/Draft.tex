...
\begin{document}
  ...
  crosslinked polymer active layer which near indiscriminately separates all
  solutes. NF membranes have porous architectures with pores on the order of 1 nm
...
%  an undesired species \cite{werber_materials_2016}. 
  ...
  selectivity due to their strict molecular weight cut-off (MWCO).  Additionally,
  the LLCs chosen to make membranes for this application are salts.  The membrane
  ...
  concentration and electric potential. This effect, which causes the membrane to
  ...
  transport. The most promising Q\textsubscript{I} phase for membrane
...
%  information.
  ...
  \end{equation}
  where $\rho$ is the average number density of particles and $\delta(\mathbf{r})$
  is the Dirac delta function\cite{kuriabova_linear_2010}. We applied equation 
  \ref{eqn:correlation} in one and two dimensions to learn about various spatial
  correlation functions.

  In the 1D case, we measured the pair correlation function between centers of
  ...
  We looked at the xy and xz pair distribution functions based on the center of
  masses of the aromatic head group rings. We gave special consideration to the
  xy cross-sections since there are multiple rings in the same plane at different
  vertices of a polygon. We used a rotated frame of reference to avoid
  ambiguities in the final pair distribution functions. For a given center of
  mass, we rotated the coordinates so that the vector created from the center of
  mass to the pore center was in line with the positive x axis. 
...
%  \end{figure}
  ...
  configuration layers stack 4.39 \AA~apart while parallel displaced
  configuration layers stack 4.38 \AA~apart.
