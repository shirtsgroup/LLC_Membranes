...
%MRS2: suggest starting with conclusion.  Something like ``The simulations best support the model of 5 monomers per layer based on the pore-to-pore distances.'' Then provide evidence. Suggest for all points.
  ...
  %BJC3: Feel free to suggest different words
  ...
  \begin{itemize}  
  %MRS4: somewhere you need to talk about the range of systems that were built, as that indicates the range of behaviors that are observed.
  		\item We built systems in both the parallel displaced and sandwiched configurations 			with layers initially spaced 3.7 \AA~apart according to the dry equilibration 
  		procedure.
  		\item The pore-to-pore spacing is stable in all systems are stable after 400 ns of 
  		simulation. Figure~\ref{fig:p2p} shows the equilibrated pore-to-pore distances for all 
  		systems tested. 
...
%  \end{table}
  ...
  \begin{itemize}
		...
		displaced and sandwiched configurations at 300K.
...
%  a simulated XRD pattern with all major experimental reflections except for R-helix.
...
%BJC2: Sure, I'll add that to supporting info
  ...
  \begin{itemize}
    ...
    %MRS4: comments from Matt suggest that domain misaligment shouldn't alone be responsible for the difference in intensity ratio. Some of the experiments should help explain it better. Let's talk!
	  \item R-$\pi$ appears at a lower $q_z$ value than experiment. In both systems, the
	    reflection reaches its maximum at c.a. $q_z=1.4$ which means that monomers prefer 
	    to stack c.a. 4.5 \AA apart rather than 3.7 \AA. This behavior is not surprising since
	    GAFF models atoms as point charges and does not appropriately model the aromatic
	    $\pi-\pi$ interactions which would cause the monomers to stack closer together.
	    \item R-spots is missing in both places, at least visually. The tails in our model
	    are more disordered than the experimental system at 300K.  
            %MRS4: are they really missing?  If Fig 7 is 300K, sandwiched seems to have it, and parallel displaced is hard to tell because it's covered by the main pi-pi-stacking
            %MRS4: interesting that the main pi-pi stacking follows the isotropc alkane curve so significantly.
	    \item R-helix does not appear in either case.
            %MRS4: is r-helix a good name, when a helix would appear on either side of the center, not the center itself?
  \end{itemize}
  ...
  \begin{itemize}
  		...
  		appear for the same reason as R-helix. 
  		\item It is possible that these reflections contribute to the continuation of
  		R-helix into R-alkanes, as seen in the experimental pattern. 
  \end{itemize}
  
  There are numerical discrepancies between the relative intensities of each reflection when
  comparing experimental and simulated patterns.
  \begin{itemize}
  		\item Table~\ref{table:relative_inensities_300K} quantifies the relative intensity
  		of each reflection.
  		\item The intensity of R-spots is much lower than experiment (need to update the numbers in that table with 300K results)
  		\item There is no R-helix intensity
  		\item R-$\pi$ is nearly an order of magnitude higher in our simulations
  ...
  R-$\pi$   & 2.8 & 18.0 & 14.0  \\
  R-spots   & 1.5 &  1.6 &  1.2  \\  % Need to get these actual data points
  R-helix   & 0.9 &  --  & --    \\ 
  \bottomrule
  \end{tabular}
...
%  		high intensity. 
%  		\item R-helix is not present, however off-axis reflections appear at the same value 
%  		of $q_z$ where we expect to see R-helix. These appear since there is a helical 
...
%  The simulated diffraction patterns indicate that there is something going wrong
%  with the way the system is modeled. We must address all of these issues and justify
%  their occurrence.
  ...
  reflection and hypothesize why they do not appear in our simulated patterns. Specifically,
  we want answer:  
  %BJC: I could number these and answer them individually
  \begin{enumerate}
	\item What is the origin of R-spots?
  	\item Why is the intensity of R-$\pi$ so much greater than experiment?
  	\item What is the origin of R-helix?
  \end{enumerate}

  \subsubsection{Origin of R-spots}

  We observed R-spots when we ran simulations of each system at 280K. (figure to be added - include with 300K patterns or keep separate??)
  \begin{itemize}
  		\item R-spots is most intense in the sandwiched configuration
  		\item Still no R-helix
  \end{itemize}
...
%  before in XRD studies of hydrocarbon chains~\cite{small_lateral_1984}.
  ...
  There are a couple reasons why we see ordered chain packing at 280 K and not at the
...
%  \end{figure}
  ...
  \subsubsection{Discrepancies between R-$\pi$}
  ...
  experiment for systems simulated at 300K.
  ...
  \subsubsection{Origin of R-helix}
  
  R-helix will not show up in any of the models tested so far.
  \begin{itemize}
        \item All monomers used to build initial configurations are identical. There is no way 	
        to produce R-helix without a more complex initial configuration 
        \item We calculated the structure factor of simple systems that mimic the parallel
        displaced and sandwiched configurations. Neither give rise to R-helix.
  ...
  We can produce R-helix if our initial configuration contains alternating parallel and
  ...
  We can also produce R-helix if layers are not uniformly spaced. Rather, monomers might form 
...
%MRS: don't need to include this point.
  ...
  Our final hypothesized configuration which produces R-helix focus on the orientation of the
   tails. 
  %MRS4: we will need to wait to see the results for this come out to really discuss this well.
   \begin{itemize}
  		...
  		\item This may be the most reasonable explanation for the appearance of R-helix. The
  		long-term stability of a configuration similar to this may be feasible if monomers
  		stay stacked 3.7 \AA apart. 
  \end{itemize}
  
   %BJC3: 6 panel figure. Top 3 show each real space configuration in 3D. Bottom 3 show their
