...
%MRS2: is ``well-defined'' the right word if there is a ``pore size distribution''?
  ...
  \begin{itemize}
    ...
    building blocks which form these materials. % BJC: "these materials" --> "nanostructured membranes", or is that redundant? %MRS2: seems fine for now, we can tweak this later. 
  ...
  \begin{itemize}
    ...
    % BJC: New paragraph?
    %MRS2: shorter paragraphs generally good.
...
%MRS2: slowed research because it wasn't clear there was a route to scaleable filtration.
    ...
    reinvigorated.
%MRS2: not THAT many people are doing it, so hard to say reinvigorated quite yet. Pick other verb.
  \end{itemize}
...
%MRs: specify what type of modification.
    ...
    \item  We do not yet understand how to reduce the effective pore size 
%MRS2: controllably
      or 
...
%MRS2: explicitly state the presence of WAXS data.
...
%     monomers in each layer are positioned with respect to other layers. 
   ...
   \end{itemize}
    \end{enumerate}
%MRS2: size and how well well defined the pore is (how suddenly the RDF changes in the pore) is also a question.

%MRS2: hmm.  This last point still seems somewhat relevant.  Why left out?
...
%  \end{itemize}
  ...
  \end{itemize}
%MRS2: and potentially used to help design membranes.  
  \section*{Methods}
  ...
  assigned using tools from Openeye Scientific Software. 
%MRS2: be specific what tools
All molecular dynamics 
  simulations were run using the latest version of Gromacs 2016. %MRS2: give specific version, since latest may be different when this is published.
  ...
  \begin{itemize}
    ...
    shipped with Openeye Scientific software's QUACPAC % BJC: how to cite? %MRS2: webpage if no publication, 
...
%MRS2: seems like the choice of separation distance here should be detailed in the supporting information, and how it changes  the structures formed.  Also, not quite clear. 
...
%  rejection data
%MRS2: maybe this fits somewhere else, since it's not really used in the same way - it's more descriptive.  In fact, maybe better to say it WASN'T used at all to constrain initial choice, and we simply looked at it after to understand how the pore size is affected by other choices.  Or maybe I'm still misunderstanding.
...
%MRS2: seems like the number per layer might be listed here, since that was adjusted during the fitting process?  Hmm.  I see it's later in the results. It's a bit confusing what variables are listed here (initial geometry settings), and what is discussed in the results section. (like number per layer)  We should discuss.
  ...
  \begin{itemize}
      ...
      \begin{itemize}
        %MRS2: what initial force constant, and does it matter (that can be in supplemental) 
          ...
          slowly released 
          %MRS2: be more quantitative about what ``slowly'' means.
          until there is no more restraining potential.
      ...
      \end{itemize}
      \item In all cases, %MRS2: for system equilibration?
the v-rescale thermostat was used with tau-t = 0.1 % BJC: supplemental? %MRS2: probably here.
  \end{itemize}
...
%MRS2: The phrase ``iterative improvement'' isn't enough for someone to repeat the procedure.  Adjusted initial distances between layers?  
...
%MRS2: suggest starting with conclusion.  Something like ``The simulations best support the model of 5 monomers per layer based on the pore-to-pore distances.'' Then provide evidence. Suggest for all points.
  ...
  To discern the composition of the monomer layers, addressing (\ref{point:monomernum}), we ran
  ...
  \begin{itemize}
	...
	\item All systems were prepared according to the dry equilibration procedure
        ...
        \item Table ~\ref{fig:p2p} shows the pore spacing for all systems tested.
        \item Systems built with 5 monomers in each layer equilibrate to a pore spacing
        that is most consistent with the experimental value of 4.12 nm derived from
        SAXS measurements (Figure~\ref{fig:SAXS}).
        \item The remainder of this discussion will focus on the analysis of systems
  ...
  To answer (\ref{point:layers}), we verified that the system stays partitioned into layers by
  ...
  \begin{itemize}
	...
	\item In both cases, we see 
%MRS2: more precise than ``a degree''?
a degree of layer partitioning
...
%  \end{figure}
  ...
  \begin{itemize}
	...
	is not feasible to do so with a large model. %MRS2: large system. 
...
%  \end{figure}
  ...
  \begin{itemize}
	...
	\item We were able to deduce that layers are composed of 5 monomers 
          %MRS2: be more precise about conclusions: evidence strongly supports 5 monomers / layer.
	...
	\item Finally, water is not needed to maintain membrane structure 
          %MRS2: Maybe ``water disorders the pore'' (some thought it might order it).
