...
\begin{document}
  ...
  %MRSF: if 10 monomers per column gave inconsistent pores when tried, might as well mention that here. 
  \begin{figure}[!htb]
  \centering
  \begin{subfigure}{0.45\textwidth}
 ...
 %any of the other stats I calculate. 
  ...
  spacing and its uncertainty according to the following procedure:
   \begin{enumerate}
	...
	spacings.
        %MRSF: I think this is the spread in a single pore, not the average pore.  Should probably make it clearer to readers which it is (i.e. uncertainty in one, not uncertainty in mean of the four).
  \end{enumerate}
  \begin{figure}[!htb]
	\centering
	...
	\label{fig:p2p_diagram}  %MRSF: this label is multiply defined. 
  \end{figure}
...
%  \end{figure}  
  ...
  %MRSF: Don't capitalize Collective Diffusion. Also, add citation on first mention.
...
%  \vspace{1em} \\
  ...
  %MRSF: probably needs some context, since this is for a force field, not for QM. Did you have plots for several different force fields, which all are presumably trying to capture the QM that all basically gave the same results? That would be better evidence.
  \begin{figure}[!htb]
  \centering
