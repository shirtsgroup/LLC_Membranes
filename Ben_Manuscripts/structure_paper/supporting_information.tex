...
\begin{document}
  ...
  %\graphicspath{{./figures/supporting_figures/}}  % put all the figures here
  ...
  \texttt{/analysis/hexagonal\_packing.py} & \ref{M-section:rspots} & Calculate Fourier transform of
  2D array of hexagonal scatterers as shown in Figure~\ref{M-fig:hexagonal_ft} of the main text. \\ \hline
  ...
  Figure~\ref{fig:correlation}). The correlation function of the system which we
...
%  \end{figure}
  ...
  \section{Pore radius as a function of monomers per column}\label{section:pore_radius} 
  ...
  \end{subfigure}
  \caption{The pore radius increases as we increase the number of columns per pore.}\label{fig:pore_radius}
  \end{figure}
...
%  \vspace{1em} \\
  ...
  \section{Correlation Functions}\label{section:correlation_functions}
  ...
  \caption{1D correlation functions of the center of masses of aromatic head
	  ...
	  (Equation~\ref{M-eqn:decaying_exponential} of the main text) to the peaks of $g(z)$. The
	  correlation length is longer for ordered basin systems
	  (Table~\ref{M-table:correlation_length} of the main text). We did not attempt to calculate the
  ...
  throughout $g(z)$ in both cases (See Figure \ref{fig:z_correlation_overlay}).
  ...
  (see Section~\ref{M-section:correlation_length}), $g(z)$ shows oscillatory 
  ...
  \section{Structure factors of simplified systems}\label{section:simplified_sf} 
  
  In the Section~\ref{M-section:rpi} of the main text, we used simplified systems
  ...
  the maximum intensity of R-$\pi$ respectively (Figure~\ref{fig:intensity_vs_zsigma}).
  ...
  the data in Figure~\ref{M-fig:sim_rsection_fit}, the FWHM of R-$\pi$ agrees well
...
%  \end{figure}
  ...
  Section~\ref{M-section:rdouble} of the main text). It is not a consequence of the 
  structure of water itself because R-double still persists if we removed water from 
  the trajectory which gives rise to the feature (Figure~\ref{fig:rdouble_water_removed}). 
  ...
  head groups.}\label{fig:rdouble_water_removed}
  \end{figure}
  ...
  in Figure~\ref{fig:dihedral_illustration}. The plots of the autocorrelation
  functions versus time lag are shown in Figure~\ref{fig:dihedral}.
  ...
  dynamics.}\label{fig:dihedral_illustration}
  \end{figure}
  ...
  of the system is increased to 500 K.}\label{fig:dihedral}
  \end{figure}
...
%  \end{figure}
  ...
  \section{Influence of Cross-linking on physical properties}\label{section:xlink_physical}
