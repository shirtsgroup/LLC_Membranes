...
\begin{document}
 ...
 \begin{enumerate}
  ...
  to assemble monomers into a structure close to one of a number of hypothesized equilibrium configurations
  (Figure~\ref{fig:build_procedure}).
  %MRSF: where's the reference for the build scripts repository?
  ...
  both parameters is presented in the Supporting Information, Section S-\ref{S-section:initial_config_dependence}. The 
  ...
  S-\ref{S-fig:stacking} of the Supporting Information for a detailed illustration 
  ...
  %MRSF: Suggest moving these references to where each code is described instead of separately here.
  %BJC: the hyperref package really messes up the labels
  ...
  %BJC: llcsim repository only contains necessary scripts. Should probably move to Shirts-group repo
  ...
  S-\ref{S-method:nematic_order} for details of the calculation). Disordered basin systems have
...
\begin{table}[h]
  ...
  of a fixed lorentzian might not be the best comparison to experiment and some form of initial configuration dependence causes extra structure. 
  %MRSF: what does optimization mean here? Better forcefield or longer time? Better fitting?
  ...
  S\ref{S-section:sandwiched_ensemble}.
...
% Remind people of randomly displaced columns. Structural collapse is random and symmetric.  
  ...
  disorder. We pooled the deviations of each head group COM from idealized positions
  from all independent simulations in order to create the distributions shown in 
  Figure~\ref{fig:offset_ensemble_pooled} (see Section~\ref{method:simple_systems}).
  ...
  We verified that this same symmetry exists when looking at individual COMs.
...
%  \end{subfigure}
  ...
  of the Supporting Information for a diagram). We averaged the histograms over the equilibrated
  ...
  % to be even slower.  
  % energy barriers are larger therefor they are different. Lyotroic has hydrogen bonds so it makes sense it could be slow
