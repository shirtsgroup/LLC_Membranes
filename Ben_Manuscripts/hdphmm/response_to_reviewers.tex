...
\begin{enumerate}[label={Comment \theenumi :}, leftmargin=3.9\parindent]
    ...
    %MRS: the paper will get in, I'm sure, and now we know better how he thinks, so all good.
    ...
    % We would also prefer to have
    %sufficient time to appropriately implement the technique for our system.
...
%    our system which uses a monoclinic unit cell and has 2 dimensional order. 
    ...
    %MRS: you mean your 2nd paper? refer specifically
    ...
    \begin{quote}
    %BJC: this is verbatim from last response to reviewers so far
    ...
    assigned charges using the AM1-BCC model
    %MRS: added (here, not in text)
    as implemented in Antechamber.~\cite{jakalian_fast_2000,jakalian_fast_2002}
    ...
    studied. 
    %BJC: The following wasn't explicitly asked for, so I could omit
    %MRS: I think it's appropriate to include.
    The main goal of this work is to develop stochastic models which can reproduce
    ...
    standard MD.
    %MRS2: is this always the case?  I think a more appropriate response would be "in systems with relatively unimpeded cores, such as in many cases LLCs with a higher water content, most waters will be relatively unimpeded, and diffusion constants can be obtained by standard techniques.   
    ...
    %MRS: maybe "including the dynamics of solutes that have longer residence times and interact with portions of the system away from the pore center."
    ...
    %BJC: If it would be better to modify the text, then we could add something to the effect: If you only care about diffusivity, then this level of
    % modeling isn't necessary in many systems.
    %MRS: I think good to talk about.
    %r1_comment5
...
%    between pores (although we have not observed any).
    ...
    %MRS: how about this?  We are grateful for the opportunity to
