...
\begin{document}
  ...
  % BJC3: Cut this paragraph
%  We carefully applied the IHMM algorithm in a way which takes advantage of the
%  system's cylindrical symmetry. While there are a number of viable ways in which
%  one could choose to analyze these time series, we chose to use a multi-step 
%  procedure that we believe adequately distinguishes the diversity of dynamical 
%  behavior exhibited by the solutes. The final parameterization of the model is 
%  in cylindrical coordinates since each pore is a cylinder where we expect 
%  solutes to exhibit radially symmetric dynamics. We present a step-by-step 
%  demonstration of the procedure that follows in 
%  Section~\ref{S-section:ihmm_procedure} of the Supporting Information.
  ...
  %BJC3: maybe could say something about multivariate Gaussian noise with VAR in cylindrical doesn't work with current code.
  %MRS3: tweaked above, probably OK.
  ...
  % BJC3: This paragraph no longer necessary
...
%  dimensions as the equilibration time point. 
  ...
  %MRS3: I think these belong after clustering.
  ...
  %BJC3: this is new, not sure of clarity
  ...
  %BJC3: added a little to the end

  %BJC3: reworked the following and split into paragraphs
  ...
  %MRS3: tweaked: 
  %We converted the $x$ and $y$ eigenvalues of $\Sigma$ and $A$ as $\lambda_x$ + $\lambda_y$
...
%  there can be up to, but not necessarily exactly, $mnk$ states. 
  ...
  % BJC3: would these subsubsection headers look better as just italicized and on 
  % the same line as where the next paragraph starts?
  %MRS3: yes, that's what I'm thinking of.
