...
\begin{document}
  ...
  \begin{abstract}
  %MRS: added ``in soft matter systems'' to make it clear that the focus on understanding things in physical systems.
  ...
  %MRS: in above sentence: try to rephrase around the physical information obtained (mechanism or properties) rather than the mathematical route to it. 

  %MRS: below - it's not the solute behavior, though, it's states identified by them model. Rephrase?
  ...
  mechanisms 
  %MRS: emphasizing things, not just analysis.
  ...
  % BJC: Not sure if this should be the first paragraph, or if I could just
  % delete this paragraph and start with the next one. 
  ...
  \begin{itemize}
  	...
  	%BJC: this could potentially be moved further down
  ...
  \begin{itemize}
    ...
    %MRS: Then say it hasn't been used in this context (do we know of any, if not say so).
  ...
  %BJC: this may be redundant from the second paragraph. Maybe better suited here?
  First, we want
...
%  their mean squared displacement (MSD) to those measured by MD.
...
%  \end{itemize}
  ...
  %MRS: consider added subsubsections to make it more readable.
  ...
  %BJC: fox_bayesian is citation Emily Fox asked me to use.
  ...
  %BJC: This paragraph could probably be cut down quite a bit. Leave justification for SI
  ...
  %BJC: cutting down on verbosity
  % We could use all 3 dimensions straight
...
%  to their priors.
  ...
  In these cases, 
  %MRS: maybe plateauing of the mean below?  Be more precise.
  ...
  %MRS: not needed.
  %because $x$ and $y$ aren't meaningful alone 
  ...
  %MRS: maybe add subsubsections naming/labeling each part of the process so that people have a sense of the steps,
  %MRS: makes it easier to read.
...
%  there can be up to, but not necessarily exactly, $mnk$ states. 
  ...
  %BJC: to be reworked after I have a working procedure.
...
%  operation for this problem. To circumvent this problem, 
  ...
  %BJC:This next paragraph can probably be moved
...
%  before the state transition occurred.
  ...
  %MRS: maybe ``using the parameterized model to explore mechanisms''
  ...
  % BJC: It probably makes sense to put the trajectory generation and MSD predictions
  % first since it will be part of how we justify our clustering choices.
  ...
  %MRS: subsection lettering is really big - makes it seem like a very different section?
  ...
  %MRS: maybe phrase it around types of mechanism, and use methanol as an example.
  ...
  %MRS: are thsse h-bonding states?
  ...
  state. 
  %MRS7: it's confusing to have the R not in order of r. Emphasize this more in the caption, 
  %maybe think of something a bit different for the final paper. Reads odd to have numbers not in order along an axis.
  %BJC7: indeed
...
%BJC: will move this earlier
  \subsection{Reproducing MD Trajectories and MSDs with the IHMM}

  %MRS: maybe makes sense to talk a little about what features/parameters of the models the MSD is sensitive to?
