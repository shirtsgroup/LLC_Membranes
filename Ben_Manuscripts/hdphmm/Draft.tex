...
%  \end{itemize}
  ...
  % (see Section~\ref{S-section:selfT_cluster} of the Supporting Information for a graphical illustration). 
  ...
  \textit{Calculating MSD}: We quantified solute motion using the time averaged mean squared displacement (MSD)
  ...
  The MSD reported for each solute measured by our MD 
  ...
  behavior to the MD trajectories. It is important that our model reproduces this 
  behavior so that we can have confidence in its usefulness for quantitative 
  predictions.
  ...
  \textit{Deviations of the IHMM from Molecular Motion}: From a quantitative
  standpoint, using the IHMM as a predictive tool has two 
  shortcomings. First, in many cases, the MSD predicted by realizations of the 
  IHMM under-predicts that of MD (see Figure~\ref{fig:unclustered_msds}).
  Second, the shapes of the MSD curves at short time lags are very different.
  ...
  The IHMM under-predicts the MD MSD due to its tendency to favor fewer states. 
  Disjoint segments with similar VAR parameters, but different lengths of entrapment
  can be parameterized as the same state. In Figure~\ref{fig:dwell_distributions}, 
  we compare the overall dwell time distributions generated directly from MD simulations to that 
  generated from realizations of the IHMM. We do this by measuring the time spent in 
  each independent same-state segment. Some of the probability density of short 
  trapping times in the MD simulations gets shifted to medium length trapping times
  in realizations of the IHMM. In Figure~\ref{fig:dwell_distributions_modT} 
  and~\ref{fig:msd_modT}, we show that by modifying the transition matrix to 
  encourage transitions towards states with shorter dwell times, we can more closely
  reproduce the dwell time distribution and MSD of MD. This is because most solute
  motion is a consequence of transitions between trapped states.

  % BJC4: A lower priority supporting information section I've had in mind is one
  % with tips for fitting and validating the IHMM on individual trajectories.  
%  In many cases, especially evident where MSD predictions over-estimate MD, there
%  is some error in the state sequence segmentation. In Section~\ref{S-section:fitting_tips}
%  of the Supporting Information, we give some tips based on our experience of how to 
%  initialize IHMM fits in order to reduce error.
...
%    MD, we may still obtain reasonable estimates of selectivity.
  ...
  We can use the ratio of solute fluxes predicted by realizations of our models in order to 
  estimate selectivity.
  \begin{itemize}
    \item Exact methodology under development
    \item Will probably just take advantage of long timescale MSD linearity in order to get
    flux, rather than mean first passage time approach.
    \item Need to figure out how to deal with differences in MSD under-estimates
  \end{itemize}
  ...
  Section~\ref{method:clustering}. In Figure~\ref{fig:mechanism_map}, we show
  the results of clustering on the same trajectory from 
  Figure~\ref{fig:rz_unclustered}. We chose to use X clusters based on the
  criteria described in Section~\ref{method:clustering}. Therefore, we reduced 
  the total state space from 300 to X. By working to understand the parameters
  of each cluster, we can gain an understanding of associated solute transport
  mechanisms responsible for those parameters.
...
%  finally, the local number density surrounding the solute. 
 ...
 \caption{(a) We can learn a significant amount of detail about solute motion by viewing 
  ...
  %MRS2: should clarify.
  %BJC3: It was meant to be transitory but can probably cut out with the addition of the subsection title
