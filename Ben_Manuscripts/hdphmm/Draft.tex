...
\begin{document}
  ...
  %MRS: citation missing for coscia_chemically_2019
  ...
  %MRS: maybe ``physically observe?'' instead of physically expect.
  ...
  %MRS: some some of the parameters not being interesting in general, not just the values for some states?
  ...
  %MRS: check again agreement on $T$ and $P$ - you switched to $P$, right? Or are they 2 different things?
  consensus $T$ matrices, describing transitions between all possible clustered states 
  %MRS: do you explicitly say how to get the consensus transition matrices?
...
%  Information, we provide a step-by-step illustration of the following process.  
  ...
  %MRS: this is a bit confusing.  Is there an expectation these should give consistent parameters?
  %MRS: the second is boostrapping an MSD over 1000 trajectories, and the first is boostrapping an MSD over 24 trajectories.
  %MRS: these should have different amounts of noise, making them hard to compare? Is this an issue?
  \begin{enumerate}[label={Method \theenumi :}, leftmargin=3.5\parindent]
  ...
  %MRS: what do you think about this? It maks it easier to read.
  \begin{itemize}
  ...
  %MRS: yeah, I think it looks good.
  \begin{subfigure}{0.49\textwidth}
  ...
  %MRS: this can be explained in the text.
