...
\affiliation{Ursa Analytics, Inc., Denver, CO 80212, USA} %CPC:  > 90% of my time spent officially under Ursa 
...
\begin{document}
  ...
  prevalent clusters in order to relate their 
  %MRS: suggestion to add:
  dynamical
  parameters to physical interactions between 
  ...
  In this work we attempt uncover similar physical meaning through a data-driven process. 
  ...
  observations.
  %BJC: added by Chris  
  Rapid state switching in the original HDP-HMM not only reduces the predictive
  ability of the model, but 
  %MRS: tweaked; sometimes it might be physical.
  % it
  in many cases
  is also physically unrealistic and can complicate
  ...
  datasets. 
  %BJC: reworked by Chris
  For example, in both MD and single particle tracking, there is strong temporal
  dependence/correlation between successive measurements even within a 
  fixed state.~\cite{calderon_data-driven_2014}
  ...
  % BJC: fair number of changes to this paragraph based on Chris' suggestions.
  % BJC: he really wants us to emphasize our contribution with this part
  ...
  dynamical modes, but to uncover unknown transport mechanisms and create 
  ...
  %MRS: possibly should give the permanent reference to a shirtsgroup fork?  We can discuss which is likely to be more permanent  
  ...
  occupy the tail region with the rest near the pore center.~\cite{coscia_chemically_2019}
  ...
  statistics on the time scales which we simulate and prevents solute clustering
  facilitating our study of pure solute-membrane interactions. Further details on
  the setup and equilibration of these systems are detailed in our previous work.~\cite{coscia_chemically_2019}
  ...
  It is usually beneficial to employ 
  %MRS: tweak
  %intelligent 
  well-informed
  state sequence initialization procedures
  in nonparametric Bayesian approaches.~\cite{fox_bayesian_2010}
  ...
  %BJC: The following paragraph used to be the first paragraph after italics above. 
  % I moved it down here as suggested by Chris below. What do you think in terms of flow?  
  ...
  an infinite number of states, a Dirichlet process 
  %MRS: tweak
  %tends to exhibit a 
  exhibits a 
  ``rich get richer" effect, favoring a fewer number of states.~\cite{dreyer_discovering_2011}
  %MRS: this next sentence is a bit vauge.  Why would we prefer thtem to stay separated?
  By applying the algorithm to each trajectory independently, we reduce the 
  possibility of lumping together multiple similar states which we would prefer
  to stay separated before clustering. 
  %BJC: added following sentence to aid reader in response to Chris' question below 
  In Section~\ref{method:realizations}, we 
  explain how we use these 24 independent models in order to generate a single
  MSD prediction.
  % BJC: abridged Chris comment with respect to this paragraph here:
  % CPC: the question i have at this point as a reader in this area, given 
  % how this is setup both as predictive and clustering, is "how do they plan to
  % generate data and predict with this model if they have 24 different transition
  % mat estimates"
  % CPC: returning to comment after reading.  section 2.4 explains what you do 
  % (though i'm still fuzzy on how you estimate transition matrix probs in method 2). 
  % (BJC: Note I tried to clarify this process for method 2 below)
  % CPC_continued: in any case, you might want to avoid obvious critiques and head 
  % scratching from expert reviewers. i'd suggest moving this paragraph to end of subsection and
  % expand on above considerations after you've given more cut and dry details below and reference 2.4 stating     
  %that you say how you simulate from a model estimated one trajectory at a time via HDP-AR-HMM inference    
  ...
  %BJC: added following in response to Chris' below comment. I think I'm interpretting what he's saying right
  % CPC: perhaps keep hammering home not only reducing, but also reemphasize why absolute
  % value of some parameters really not of interest 
  %MRS: explain why parameters of single states don't give insight; more context needed?  Is CPC talking about
  %MRS: some some of the parmameters not being interesting in general, not just the values for some states?
  and because the parameters of a single states does not always give meaningful physical insight. 
  ...
  the pore axis so we do not cluster in means in the $z$ dimension. Before carrying
  out the agglomerative clustering algorithm, we normalized each clustering dimension
  by its variance and shifted the mean to zero.
  
  %BJC: small modifications to this paragraph for clarity
  ...
  $y$ respectively.
%  rather than $\mu_x$ and $\mu_y$ individually, with the
%  square $\mu_r$ converting it into the same units as $A$ and $\Sigma$.

  To cluster on $P_{ii}$, we first cast it in terms of the expected dwell times. The expected
  ...
  %MRS: did you mean to change to T to P here?
  %MRS: also, maybe ``consensus'' or ``merged'' instead of ``single''
  single $T$, describing transitions between all possible clustered states exhibited by
  the ensemble of trajectories, using the inference component of the HDP-AR-HMM
  algorithm. We applied the HDP-AR-HMM simultaneously on all 24 zeroed solute
  trajectories with state sequences fixed according to the remapped clustered 
  state assignments. 
  %MRS: be specific what ``This'' is in the sentence.
  This is only possible because the state sequence is fixed. 
  In cases where one does not know the state sequence and would like to 
...
%  Information, we provide a step-by-step illustration of the following process.  
  ...
  % BJC: Chris wasn't clear where T came from for method 2. I tried to make it clearer in 
  % last paragraph of previous section. Let me know if it's not
  %MRS: P vs. T agin?
  ...
  %MRS: ref below to figure not recognized?
  ...
  %BJC: need to confirm these details
  For all solutes, we generated 200 ensembles of 24 5 $\mu s$ trajectories and 
  estimated $D$ from the slope fit between 10 ns and 1000 ns. We used weighted 
  ...
  The number of states found for each trajectory varied between 2 and 45.  %BJC4: MET: (5, 28), URE: (5, 45), ACH: (2, 25), GCL: (7, 35)
  ...
  %BJC6: added. Should I add a figure?
  %MRS: I think fine as supporting.
  ...
  timescales.
  %BJC6: could hypothesize why using previous work as support
  ...
  %MRS: I feel like this should be caveatted a bt. 
  The data suggests that the particular LLC membrane studied in this work, 
  up to the resolution of the moleceular modeling,
  is capable of selectively separating urea and acetic acid from ethylene glycol
  ...
  %MRS5: well-written paragraph, tells a convincing story.
  ...
  \end{subfigure}
  %MRS5: should make it clearer these are from the 10 clusters studied earlier. 
  \caption{(a) Stochastic trajectory realizations generated from our model (blue) based on
  the 10 clustered methanol parameter sets do not qualitatively reproduce the diversity
  of behavior seen in our MD trajectories. (b) MSD predictions by clustered parameter sets consistently
  ...
  dominant states, the amount of time spent in each varies widely. For example, methanol
  does not spend a significant amount of time in State 3 because, as discussed later, it 
  is a short-lived state. We still consider State 3 as a dominant state because it
  is may be a significant contributer to solute MSD.
  %MRS5: maybe explicitly indicate the 5 stattes?  I only see four red bars with appreciable sampling.
  % The solutes spend varying amounts of time in each of these states.
  %BJC6: I'm focusing on the blue bars. It's a hopping state so not much time is spent there,
  % but most of the solutes still show that behavior. Added a couple sentences to clarify
  }
  ...
  \begin{enumerate}[label={State \theenumi :}, leftmargin=3.5\parindent]
     ...
     tail region based. Fluctuations are small and there is little past hop dependence.
  ...
  times. These are fleeting interactions since it is likely a high variance 
  transitory state.
  %MRS5: not clear what a ``jumping state'' is.
  %BJC6: tried to be more precise. changed to 'high variance transitory state'
  ...
  \end{subfigure}
  %MRS5: specifically mention at the beginning which solute.
  ...
  (green) and the tails (red). (b) For each dominant state (States 1--5 described in 
  ...
  \end{subfigure}
  %BJC: Is it worth putting a bars with the solute MSDs on this plot
  \begin{subfigure}{0.49\textwidth}
  ...
  far from the pore center. (c) The largest expected value (see 
  Equation~\ref{eqn:dwell_times}) of dwell times among the dominant clustered 
  states for each solute are inversely related to the solute's MSD. 
  %MRS: is the c) point actually made in the figure?  There's no MSD in that graph.  
  % BJC: Copying Chris' comments below:
  % CPC: Discussion is confusing. guessing error bars are from measured simulation 
  % dwell times in dominant state.  
  % BJC: will add error bar explanation once I figure out exactly how to modify the figure
  % CPC: what if trajectory has more than one dominant state? (seems from fig like 
  % you take one with largest dwell time from your model. 
  % BJC: All solutes have at  least 4 dominant states by my definition. maybe it would 
  % be better to sample dwell times from all of the dominant states. Just weight 
  % their contributions. Not sure if weighting should be with respect to the fraction
  % the total simulation time spent in each state, or the number of times the state
  % was visited.
  % CPC: might want to better connect MSDs of model in fig.3 to fig 9 c and 
  % expand/clarify discussion on connection of fig 9c to other panels just lots
  % of ambiguity here, but results seem promising.
  % BJC: is it worth plotting bars with the MSD after 1000 ns as part of (c)? 
  (d) The expected dwell time of solutes increases 
  in states with high local number density of surrounding heavy atoms.
  }\label{fig:summaries}
  \end{figure}
  
  % BJC: Chris was confused by this paragraph
  %MRS: maybe switch ``dominant'' for something else, like predominant, or consensus
  Solutes with dominant states characterized by long dwell times tend to 
  have lower MSDs. We define the dominant states exhibited by each solute
  as any state visited by at least half (12) of the solute trajectories.
  By our definition and based on our choice to study 10 clusters for each 
  solute, methanol, ethylene glycol, urea and acetic acid are characterized
  %BJC: added this supporting figure 
  %MRS: for some reason Not referencing this figure in supporting despite xr? 
  by 5, 6, 5 and 4 dominant states respectively (see Figure~\ref{fig:state_prevalence}
  of the Supporting Information). In Figure~\ref{fig:dwell_time_summary}, we
  plot the expected dwell time of each solute in the dominant state which 
  has the the highest self-transition probability. The MSDs shown in 
  Figure~\ref{fig:unclustered_msds} are inversely related to the maximum 
  expected dwell times. Both acetic acid and urea participate in many hydrogen
  bonds and sodium ion association interactions and the states visited by both 
  solutes have similar distributions of radial means, however acetic acid is 
  subject to much longer dwell times, and hence a lower MSD. This may be 
  because acetic acid is a stronger hydrogen bond donor than urea.
...
% LocalWords:  shirtsgroup superdiffusive diffusivities
