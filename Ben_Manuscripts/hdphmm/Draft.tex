...
\begin{document}
  ...
  %MRS: starts to mix interpretation in with the machine learning too early?  You don't really 
  ...
  %MRS: changing to lead with WHY we group them.
  ...
  %MRS: OK, did a pass below, reorganizing it a bit.  See what you think. I think it's OK to talk
  %about subdiffusion because it's general problem, we aren't making claims about this one. 
  ...
  % BJC5: Baum welch is most widely used for pure HMM but is an expectation maximization algorithm
  % There are pure Bayesian ways to do it. Tried to clarify above.
  % MRS5: there's some implication in the above about what you will use; I tried to make it more general. 
  ...
  before.
  % this is a novel application and analysis of the sticky HDP-AR-HMM.
  ...
  %MRS5: I actually think passive voice works better here, since we want to know if it helps in general.
  %researchers
  ...
  %MRS5: might want to mention above that there was no sign of clustering of solutes (if that is true)
...
%  We analyze solute motion in terms of their
...
%  \subsubsection*{Applying the HDP-AR-HMM to MD Trajectories} 
  ...
  %MRS5: grammar in previous sentence unclear?  I tried to clean up, but failed.
  ...
  procedure. 
  %For example, one can gain a thorough understanding of the dynamics of
  ...
  %MRS: not sure we can say quite the same 
  %the same 
  similar hopping
  and trapping behavior exhibited by MD (black).
%  We can use realizations of the HDP-AR-HMM to generate trajectories which 
%  bear qualitative resemblance to the MD trajectories. 
  ...
  behavior (see SI).  %BJC5: TODO. There are a couple trajectories I want to show have very high MSDs because all the hops are in one direction along the pore. The curves look superdiffusive. I think on longer timescales, this wouldn't happen. I'll also show the MD MSD curves without the 'outliers' in the SI.
  On long time scales, the model's MSD appear to have a slope similar to the MD 
  MSD.
  
  %BJC5: need to update legend to say HDP-AR-HMM
  \begin{figure}
  \centering
  ...
  %MRS: seems odd not at least mention clustering here.
  %BJC5: I haven't talked about clustering yet. Should this section get moved later? I think it leads into the next section well here.
  %MRS5: I think OK now in current presentation.
...
%  Since we hypothesized that the lack of short dwell
%  times is largely responsible for the under-prediction of the MSDs, we could
%  modify the dwell time clustering procedure in order to better emphasize short
%  dwells times.
  However, we did not pursue this too far in order to avoid over-fitting the data.
  %BJC5: maybe an SI figure showing some different approaches
  %MRS5: I'm not sure it's necessary.
  ...
  %Although a majority of the solutes
  ...
  \begin{enumerate}[label={State \theenumi :}, leftmargin=3.5\parindent]
     ...
     % BJC5: That's kind of what I mean. Behavior is similar near pore center and far
     % from pore center. Half happen at pore, half happen in tails, average r is 1.5 nm
     % MRS5: maybe be explicit - ``essentially equally in radial distance through the pore? 
     % Otherwise, it seems to imply there may be some other possibility besides those 2.
  ...
  %MRS5: be more clear what ``dominant state'' means in connection with the text.
  ...
  %MRS5: four subfigures, should all be described in caption
  ...
  %MRS5: maybe, `` generating inexpensive models'' -> ``modeling long-time scale trajectories''
