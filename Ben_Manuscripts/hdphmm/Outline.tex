...
%  	exhibited by solutes in our MD simulations.
  ...
  \begin{itemize}
   ...
   \item The concentration parameter $\gamma$ expresses one's confidence in H relative to the posterior 
...
%    finds within the range of 5-15 states. 
  ...
  \begin{itemize}  
   ...
   \item Most parameters converged quickly (within 50-100 iterations) while
    ...
    %MRS3: other possbility is maximum a posteriori estimate, but for peaked symmetric distributions, it's essentially the same.
    ...
    parameter estimates. 
    %MRS3: not sure what you mean that they all were multidimensional?
  ...
  %MRS3: because of the radial symmetry, x and y aren't really meaningful alone. 
  \begin{itemize}
    ...
    we reapplied the IHMM to the cylindrical trajectories with a fixed 
    state sequence, only allowing inference on the VAR(1) parameters. 
    %MRS3:did you cluster the state transitions as well? 
  \end{itemize} 
  
  We clustered like parameter sets in order to reduce the state space to
  a more easily interpretable size.
  \begin{itemize}
  %MRS3: Maybe ``independent sets'' -> ``independent states, each with separate VAR(1) parameters?''
...
%   \item This is necessary for us to perform any kind of mechanistic speculation.
  ...
  parameter sets.
  \begin{itemize}
    %MRS3: not sure what is meant by ``Weighted by the rows of T''? Should it just be given by T? 
