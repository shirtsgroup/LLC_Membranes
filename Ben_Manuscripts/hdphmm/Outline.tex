...
%  	exhibited by solutes in our MD simulations.
  ...
  \begin{itemize}
    ...
    %MRS3: This next line is a bit vauge (expresses confidence, closely related)
    %BJC3: tried to make more clear
  ...
  \begin{itemize}
    ...
    %MRS3: be more specific about which parts from the above are in the references
    %BJC3: made more specific
...
%    each solute's center of mass position. % ALready said this above
  ...
  \begin{itemize}
    ...
    %MRS3: I'm not following the below? I don't know that we WANT to find more states, in my mind doing it this way 
    %the math is simpler this way since we don't have to rewrite things in term of radial diffusion
    %MRS3: we can brainstorm this as needed!
...
%    finds within the range of 5-15 states. 
  ...
  % the mean in $r$ is a useful clustering variable.
  \begin{itemize}
...
%    state sequence, only allowing inference on the VAR(1) parameters.
    %MRS3: would it be clearer to say you appplied only the inference part of the process on the clustered data?
    %BJC3: like this?
  ...
  with separate VAR(1) parameters. % BJC2: just a guess for now
   ...
   to cluster them. % cite scikit-learn  
%   \item This is necessary for us to perform any kind of mechanistic speculation.
  ...
  \begin{itemize}
   ...
   behavior.~\cite{coscia_chemically_2019}  % cite new paper eventually too.
  \end{itemize}
  ...
  \begin{itemize}
   ...
   trajectories.
   %MRS3: seems like you should just be able to lump together the IHMM states to generate the IHMM estimate of T?
   %BJC3: That's what I do. Maybe I'm misunderstanding your suggestion. I added a little bit to the sentence
   % above to try and make more clear.
  \end{itemize}  
  ...
  %MRS3: ``used the IHMM algorithm'' -> ``used the parameter inference part of the IHMM algorithm''?
  ...
  % BJC: I'll use an example molecule for this section. I'll put files up so
  % that people can look at any trajectory.  
  ...
  \begin{itemize}
    ...
    dynamical behaviors. 
    %BJC: this might belong in methods
    %BJC: need to come up with a good justification for choice of distance threshold.
  ...
  % BJC: Show a trajectory that shares many states with another state. Color them the same to show that clustering did something
  ...
  % BJC: don't forget to mention in the methods that clustering is necessary in order to
  % construct a single set of parameters used to project MSDs
