...
%  explanation of the molecular interactions responsible for those correlations~\cite{dischinger_effect_2017}
  ...
  colored gray. (a) Membrane pores are approximately straight and (b) are packed 
  hexagonally.}\label{fig:membrane_rendering}
  \end{figure}
  ...
  the membrane in which solutes are placed. 
  We have listed the solutes studied along with the abbreviations that
...
%  \end{minipage}
  ...
  it might make sense to hypothesize that the water and polar solutes spend
  ...
  There are also a couple of questions that this study that are beyond the 
  scope of this study. First, we will not study the concentration dependence of the observed transport
  ...
  which occur between isolated solutes and the membrane.
...
%  observation time accumulated for all solutes.
  ...
  reasonable range of distances and angles. We determined the average percentage
  of solutes which actively participated in a hydrogen bond interaction with 
  monomers each frame. We only counted unique solute--monomer hydrogen bond 
  interactions, meaning solutes that hydrogen bond more than once simultaneously were classified as a
  ...
  the solutes. We scaled the head group RDF so that it would be visible
  next to solute RDFs.

%  of each individual
%  head group. For each head group, we obtained an RDF averaged over the
%  entire simulation trajactory. We randomly chose head group RDFs with replacement
...
%  head group positions are uncorrelated.
  ...
  leading to faster diffusion in the tails (see Figure~\ref{S-fig:total_density}),
  ...
  higher than that in the tails, we restricted our studies of transport to 
  solutes in the pore region. Transport of solutes in the pores will be the 
...
%  Set up simulations with high percent methanol
 ...
 \subsubsection{Transport of Small Polar Solutes}\label{section:general_transport_solutes}  
  ...
  trends observed across all systems and then focus on subsets of similar molecules.
  ...
  Of all the solutes, methanol likely is subject to the least hindrance by
...
%  \end{itemize}
  ...
  hydroxyl groups are replaced with thiols but the solutes become less
  stable in the pore. Finally, solutes with carbonyl groups, which includes
  the ketones, amides and hydrogen bond acceptors, associate with
  sodium ions, temporarily immobilizing them when the sodium ions are 
  simulateneously partially bound to a monomer head group. 
