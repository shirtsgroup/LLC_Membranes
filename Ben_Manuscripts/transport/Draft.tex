...
%  explanation of the molecular interactions responsible for those correlations~\cite{dischinger_effect_2017}
  ...
  colored gray. 
  %MRS3: probably this point below can be better addressed in the text; looks a little out of place here.
  ...
  %MRS3: maybe say ``we present studies of''? ``This work'' is the paper, so using past tense is weird.
...
Given that the pores will restrict motion of the solutes, we
  ...
  %MRS2: be consistent on tense.  What is presented in the paper is present tense.  We already studied it, we are presenting it.  
  ...
  There are also 
  %a couple of questions that are beyond
  ...
  %MRS3: added - there might be mechanisms that only happen with multiple solutes in contact
  likely
  not needed
  to understand the 
  %MRS3: added
  ...
  analysis 
  %MRS2: added to make sure it's clear the basic description is not JUST in the scripts.
...
%  observation time accumulated for all solutes.
  ...
  %MRS2: Be more explicit than 'instructive'' - ``Where it help to demonstrate X'' or something like that?
...
%  head group positions are uncorrelated.
  ...
  %MRS2: changed tense to present on the paper itself, past on studies leading to the paper.
...
%  the peak exhibited by acetone near 0.2 nm, which is due to sodium ion association.
  ...
  is 
  %MRS2: can you be more definitive than ``likely caused''?
...
%  \end{itemize}
  ...
  %MRS2: tweaking
  %The optimal design of new LLC membranes for a given separation is not obvious.
  ...
  %Experimental progress up to this point in time is the culmination of an iterative 
  %educated guess-and-check framework. 
  ...
  %as well as the complexity of the solutes themselves, have hindered the 
  %establishment of firm design criteria. 
...
%  We can use insights gained from our model to hypothesize and screen new monomers
%  in order to rapidly increase LLC membrane development.
