...
% but I think that as people read, they need to see the 3D picture as well to think through things.
...
\begin{document}
  ...
  find
  %found
  %MRS2: ``transport rate -> transport''
  that the transport rate of a species is dependent not only on 
  ...
  the effects of deep well injection and generate value.~\cite{dischinger_application_2017} 
  %BJC: could make last point more general
  %MRS2: ``generate value'' is pretty vague, yeah.
  
%  All of these separations are possible with current commercial membrane
%  separation techniques but they are too energy intensive to be worth the 
%  capital investment needed to start the processes  
% MRS2: maybe ``are possible'' -> ``are possible in part''?  Otherwise, it logically follows that they shouldn't have major drawbacks.
  All of these separations are possible with current commercial membrane
...
%  membranes.~\cite{zhou_supported_2005}
  ...
  %MRS2: is ``complexity'' the right word? Can we be more specific?
  Unfortunately, the complexity of LLC membranes has obscured the interpretation
  of experimental transport studies.~\cite{dischinger_effect_2017}  % BJC: ref to Sarah?

  %MRS2: odd to have a one sentence paragraph?  MAyber just join with the next paragraph.
  ...
  In our previous work, we used MD simulations to determine the most likely 
  structure of the hexagonal phase formed by the monomer Na-GA3C11, expanding 
  upon the most recent experimental characterization.~\cite{coscia_understanding_2018,feng_thin_2016}
...
%  Many of the separations we are interested in involve polar organic 
%  compounds.
  ...
  Previously, we have
  studied dry membrane systems and some systems with low water content. 
  We will observe how the radial density of monomer components and water 
  change within the oore based on membrane water content.
  
  % BJC: I don't really explicity comment on the partitioning of solutes
  % out of the pore. Maybe I can add some sort of chart. Need to think more.
  % MRS2: it should be mentioned somewhere, probably worth a paragraph or so.  
  % If WATER is much slower outside of the pores, everything else will be slow as well, so we can
  % justify transport there not being interesting
  %Then we will present 
  We then present
  our efforts to understand how small molecules, 
  ...
  %MRS2: tweak
  %Finally, and for the bulk of this work, we will 
  ...
  %MRS2: next line is a statement about what we want to do, not what we did?
  %We want to understand
  We investigate
  the differences in solute motion, specifically its mean squared displacement 
  (MSD), based on a solute's size, shape and chemical functionality. We will 
  study the interactions between solutes, the membrane, and water in order to
  determine which mechanism or mechanisms dominate.
...
%  where 
  ...
  We found that the distributions best fit a power law / exponential
  distribution.
  % BJC: need to do this. 
  % BJC: reporting median doesn't work great since it is usually equal
  % to the lowest possible value since time step is large.
  % Would probably be better to report fit parameters (alpha for power law)
  % Could use those values in analytical equations for median
  ...
  Before beginning the analysis of transport behavior, it is important to
  elucidate the topology of the membrane pores in solvated systems.  
  
  In contrast to our previous work with a dry version of this 
  membrane, the pore region of the 5 and 10 wt\% water 
  systems are primarily filled with water and sodium ions. We define the
  pore region based on the average COM position of the monomer head groups' 
  % BJC: not sure the best place in paper to define pore region. Maybe a system definitions section in Methods.
  % BJC: need to decide on exact definition. Should probably be the same as the 'Head Groups' line
  carboxylate oxygen atoms. Figure~\ref{fig:component_densities} plots the RDF of each membrane
  constituent in the dry system as well as the 5 and 10 wt\% water systems. 
  In dry systems, the pore center is densely filled with sodium ions and head
  groups. The peak head group density occurs within 0.25 nm of the pore center.
  In hydrated systems, water occupies, and is densest, at the center of each 
  pore. The density of sodium ions is somewhat uniform in the pore center of
  the 5 wt\% water system while it shows a maximum closer to the head groups
  in the 10 wt\% water system. The peak density of sodium ions is not at the
  pore center in the 10 wt\% water system likely because they are still
  loosely associated with the monomer's carboxylate head groups.
  ...
  Based on these observations, we expect solute transport to be fastest in the 10 wt
 \% water system.
  ...
  region of both systems. 28\% and 31\% of the total water is present in the tail
  regions of the 5 and 10 wt\% water systems respectively. See 
  Section~\ref{S-section:water_content_equil} of the Supporting Information for more details. This 
  result is due to a combination of the lower density of the tail region
  as well as oxygen atoms at the ends of each monomer tail which can further 
  stabilize water molecules. See Figure~\ref{S-fig:total_density} of the 
  Supporting Information for a graphical illustration of this point.
...
%  \bigskip
  ...
  methanol, has an MSD about 175 times larger in the 10 wt\% versus the 5 wt 
 \% water system. Clearly the equilibrium water content of a given LLC system will 
  ...
  Equation~\ref{eqn:stokes-einstein} is plotted and converges to the same value as the
  ...
  made in the pore region are about 45\% larger than those made outside
  ...
  controlled by membrane properties other than density must be prevalent.
...
%  entrapment usually occur when solutes are far from the pore center.
  ...
  head groups, 25\% go to the ether linkages between the tails and the 
  ...
  monomer head groups and the mean hydrogen bond lifetime. Solutes with multiple
  hydroxyl groups donate hydrogen bonds most frequently and tend to stay hydrogen
  bonded longer. Hydrogen bonds donated by nitrogen are far less common and tend 
  to be short-lived as supported by acetamide and urea.
  ...
  average length of time that pairs stay coordinated. The degree and length of 
  ...
  The 5 highest coordinated solutes, which also have the longest coordinated dwell times,
  all contain carbonyl groups (except for dimethyl sulfoxide which has an 
  analogous sulfinyl group) which leaves a highly electron-dense oxygen atom 
  exposed and ready to interact with sodium. 
  %BJC: need to verify following  
...
%  in which polar groups bind to sodium ions for at least a short period of time. 
  ...
  tetrahydrofuran molecules when they dissolved a sodium salt in the solvent.~\cite{carvajal_studies_1965}
  ...
  region, hops made by methanol are larger than those by any other solute. The
  small size of methanol allows it to move relatively unhindered. 
...
%  the head groups.
  ...
  This is largely due to the large net charge of the carboxylate groups as well
  ...
  of ether linkages increases with molecule size. (d) The average location of 
...
%  molecular glue which holds solutes in place, especially when there are many.
  ...
  at the same time.}\label{fig:multi_hbonds}
  \end{figure}
  ...
  it to favor positions near monomer head groups. Both diols have comparable 
  densities close to the pore center, however propylene glycol's density has a
  large peak near the monomer head groups relative to ethylene glycol. Propylene
  glycol can form more highly stabilized hydrogen bonds with carboxylate groups,
  explaining the slightly higher incidence of hydrogen bonds shown in 
  Figure~\ref{fig:multi_hbonds}. The average hydrogen bond lifetime of propylene
  glycol with monomers is 1.95 ns compared to 1.34 ns for ethylene glycol.  % BJC: 2.33 ns vs 1.62 ns for hbonds with carboxylate
  Somewhat counterintuitively, there is a relatively high density of ethylene
  glycol molecules beyond the head group region probably due to its relatively 
  small size. This likely contributes to the somewhat large error bars on its MSD
  in Figure~\ref{fig:msds}. 

  \subsection{Transport of Ketones and Amides}
  
  The 4 ketone-like molecules tested show a surprising range of transport
  behaviors. Urea, acetic acid, acetamide and acetone are all characterized by
...
%  significantly more time in the pore center and therefore encounters more sodium ions. 
  ...
  Stokes-Einstein predicted behavior increases as with molecular size increases.
  ...
  large variance of its MSD. Some can be accounted for by the higher 
  density of mercaptoethanol molecules outside the pore region, where 
  transport is inherently slower. Mercaptoethanol spends 20\% less 
  time in the pore region than ethylene glycol. Conversely, 
  mercaptoethanol hydrogen bonds with head groups about 40\% less 
  frequently, which is less than expected if equal amounts of both 
  solutes occupied the pore region. Consequently, shorter hydrogen 
...
%  center is 1.43 nm compared to 1.20 for ethylene glycol. % hop_location.py. Even larger difference if cut-off lower
  ...
  spends 38\% less time in the pore region than glycerol.
...
%  population of 2,3-dimercapto-1-propanol then becomes more dense into the tail region.
  ...
  bonds as 2,3-dimercapto-1-propanol. % which is consistent with the number of hydroxyl groups per solute. 
  ...
  DMSO has a comparable if not larger MSD than acetone even though it
  is a slightly larger molecule. DMSO spends 17\% more time in the pore
  than acetone. 
...
%  the peak exhibited by acetone near 0.2 nm, which is due to sodium ion association.
  ...
  tetrahydrofuran and dimethyl formamide. The MSDs of ethyl acetate, propylene
  carbonate and acetone are only marginally larger.
  ...
  tetrahydrofuran perform slightly larger hops but have the third and
  first lowest hop frequencies respectively (see Figure~\ref{fig:hopfreq}).
...
%  \end{itemize}
  ...
  Assocaition between sodium and solutes in this set are also among the longest,
