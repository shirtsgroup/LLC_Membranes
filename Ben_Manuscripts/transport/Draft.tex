...
\begin{document}
  ...
  size, limiting selectivity.~\cite{werber_materials_2016}.
...
%  membranes.~\cite{zhou_supported_2005}
  ...
  from experiment. 
%  These membranes have been primarily characterized with size-exclusion experiments
%  which can give an estimate of pore size as well as selectivity based on differences
%  in solute sizes~\cite{zhou_supported_2005}. 
  Size exclusion does not appear to be sufficient to describe these membranes.
  Similar membranes do not separate purely on the basis of sides.
  ...
  %BJC4: Hmm, by characerization, I mean testing their ability to separate, so I'm 
  ...
  %MRS4: expand the above.  What was discovered, and why was it unfortuate to the thesis?  
  % I though it actually helped the thesis here since there was something happening that WASN'T just size dependence?
  %BJC4: gave it another shot above.
...
%MRS4: probably shouldn't capitalize the names.  Maybe make the yellow a bit darker for the text.
%BJC4: tried a new yellow. I think it's more readable. Hard to say
...
%  \begin{tabular}{ |lcc|lcc| }
  \begin{tabular}{ |m{3.5cm}m{1.4cm}m{2cm}|m{3.8cm}m{1.4cm}m{2cm}| }
    ...
    \color{amber}{tetrahydrofuran} & \color{amber}{THF} &     
    \begin{minipage}{.1\textwidth}
    ...
    \color{amber}{dimethylformamide} & \color{amber}{DMF} &     
    \begin{minipage}{.1\textwidth}
...
%  \end{minipage}
  ...
  %MRS4: Was Berendsen pressure control used for the transport as well?  Should specify?  I thought those might have been NVT? If so, should put in an acknowledgement that it's not quite right, cite my and Pascal's paper.
  ...
  %since it has been parameterized 
  ...
  %MRS4: check that this change is correct. 
...
%  where 
  ...
  %MRS4: I would put in supporting.
...
%  observation time accumulated for all solutes.
 ...
 composed
  ...
  %MRS4: dotted horizontal line not defined.
  \end{figure}
  
  %MRS4: this would be a good place to say that these waters won't be considered and why.
  ...
  %MRS4: did it really decrease? From what?  What was the previous definision.
  ...
  %MRS4: might not be clear to the reader what the consequence of this MSD value is above, since it's not compared to anything.
  %MRS4: interesting the water in the tails is almost as fast as the water in the 10% weight. 
  ...
  hydrogen bonding. 
  %MRS4: a bit confusing, since H-bonding occurs in both 10 and 5?
Water molecules are less likely to hydrogen bond while in the
  ...
  %MRS4: but there is also H-bonding to ions and carboxylate groups, which seems like they should be even stronger than water-water.
 hydrogen bonding between water molecules near the pore center versus those in 
  ...
  %MRS4: think about reading over/describiing the tail water here
 ...
 \subsubsection{Transport of Small Polar Solutes}\label{section:general_transport_solutes}  
  ...
  %MRS4: sentence here seems like out of scope for the section title. 
  Solutes
  with hydrophobic character partition out of the pore and are slowed by densely
  packed organic monomer components. 
  %MRS4: this above seems like a general statement for all transport, not just small polar solutes?  Should there be a bit of an introduction to transport as a whole before going into specific molecular classes?
  ...
  %MRS4: intersect where?  Might not be clear to readers that next sentence is a continuation of the process in the sentence above.
...
%  molecular glue which holds solutes in place, especially when there are many.
  ...
  \end{subfigure}
  %MRS4: suggest cutting off the density in height at 1.0, so that the scales of the other molecules can be seen.
  %MRS4: below - specify which set this is.
...
%  significantly more time in the pore center and therefore encounters more sodium ions. 
  ...
  %MRS4: seems like we could make it stronger with ``likely do''?
...
%  \end{itemize}
  ...
  %MRS4: abrupt ending . . . think about a better way to wrap up that focuses on what was learned as well.
  %BJC: future work? i.e. talk about mathematical modeling 
  %MRS4: the modeling is scietifically a bit different.  MAybe nto
