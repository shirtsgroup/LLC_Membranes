...
%  	\item Once on the nm to sub-nm pore size regime, size-exclusion 
...
%  	\item In order to handle complex waste streams with molecules of similar size,
  ...
  %BJC2: Changed format from numbered questions to more of a roadmap for this paper  
...
%%  	governing model. 
%  	
%        %MRS2: this is beyond the scope of what we can really answer. We can hypothesize things, but 
%        % we really haven't really done any experiments to confirm these.  So this should be deemphasized.
...
%    mechanisms in these complex LLC systems.
  ...
  %MRS2: depends on whether it's necessary to support the main hypothesis. 
  %BJC2: made more vague and included specifics in membrane structure section
...
%  \end{itemize}
  ...
  % MRS2: this might be slightly larger than the hydrodynamic radius?  We might not be able to place it 
  % directly into stoke-einstein.
  % BJC2: Yeah, I've thought about this. Actually calculating the hydrodynamic
  % radius would involve a viscosity calculation (which I have the tools to do)
  \begin{itemize}
...
%    that a spline is still justified.	
  ...
  %BJC2: sensitivity added to supporting information
  ...
  \begin{itemize}
    ...
    % BJC1: NaCl starts to crystallize for higher concentrations leading to lower coordination numbers
...
%    concentration (See section~\ref{S-section:coord_number} of the SI)
  ...
  %TODO redo analysis on pure water system for this section.  
  ...
  %MRS2: I wonder to what extent it would help to show the curves on top of each other to make it clear
  %how much things are displaced.
  %BJC2: I'll see what I can do without overly cluttering the figure
  ...
  \begin{itemize}
    ...
    and oxygen atoms at the ends of each monomer tail which can stabilize water molecules. %TODO Total density as function of distance from pore center.
  \end{itemize}
...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    %MRS2: might need to point out that they are not that undercoordinated, as they also coordinate with solutes.
    %BJC2: not solutes here, because the final calculations will be done for a pure water system.
%    \item The low levels of coordination with water under confinement are likely driven
...
%  	\item Transport is clearly affected by factors other than molecular size. 
  ...
  %MRS2: probably, perhaps as a colored background.  But might make it too noisy to read.  MAybe color them by chemical group?
  %BJC1: should the points be labeled with residue names?
  %MRS2: as long as there is a table in SI with the relevant information, probably isn't needed.  That would also make it hard to read the plot.   
  %MRS2: for the plots with fewer points (like alcohol), the individual points should be labeled, there is room.
  %MRS2: as discussed, maybe for each solute group, have a MSD plot a) and RDF plt b) and other important plots in c) and d)
  \begin{subfigure}{0.45\textwidth}
...
%    are 5 times more water molecules in the system.  % BJC1: more like 3 times more water molecules because many are in the tails.
  ...
  \begin{itemize}
    ...
    \item Some kind of pair time distribution  %TODO
...
%    within 2.5 \AA~of a sodium ion for the entire length of a trapping occurence.
  ...
  % BJC2: moved to supporting since anecdotal
%  \begin{figure}[!htb]
%  \centering
...
%  \end{subfigure}
%MRS2: maybe include the MSD vs size plot subset here. 
...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Urea and Acetamide both have hydrogen bond donating nitrogen atoms, however
  	nitrogen is a weaker hydrogen bond donor than oxygen due to its lower electronegativity. % citation
...
%  \end{subfigure}
  ...
  \begin{itemize}
    ...
    \item We replaced the carbonyl carbon of acetone with sulfur in order to create DMSO. 
  	...
  	their hydroxyl group analogs.  % citation
 \end{itemize}
...
%    (See Figure~\ref{fig:SOH_trace})  % Only methanol has greater (I think)
    ...
    of its MSD.
    \item Average hop length while in pore region maybe %TODO
...
%    responsible for these hops.
  ...
  %BJC2: Moved to supporting figure since anecdotal
%  \begin{figure}
%  \centering
%  \begin{subfigure}{0.45\textwidth}
...
%  It can easily reform the second hydrogen bond in close proximity
  ...
  \noindent 2,3-dimercapto-1-propanol exhibits slower transport than glycerol 
  because more of it partitions into the tail region. % BJC: should I give all the solutes nicknames (i.e. their residue names)?
  \begin{itemize}
    % BJC1: maybe next point better expressed as a percentage in each region.
    ...
    as 2,3-dimercapto-1-propanol. %BJC1: 52.2 / 16.4 ~= 3.2. Includes hbonds with water (recieved and donated) as well as with ether + carboxylate oxygen atoms.
  ...
  \begin{itemize}
    ...
    %BJC1: better to report a percentage?  
    %MRS2: yes, report as intensive quantity
...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    % since they spend more time in the pore region. 
  ...
  %MRS2: is this because you need H-bonding to make hops? We should have some sense of this.
  \begin{itemize}
...
%    associate with sodium in negligible amounts.
  ...
  %BJC2: moved to supporting information
%  \begin{figure}
%  \centering
...
%  \subsection{Design Considerations}
  ...
  %MRS2: should be a bit careful on what WILL happen, as opposed to what might happen.
  %BJC2: condensed and added to conclusions.
...
%    %MRS2: hydrogen isn't a counter ion in and of itself, since it's only available either as hydronium 
%    % or will protonate the head groups.
...
%  \end{enumerate}
  ...
  %BJC2: will just leave out concentration dependence since the following is purely speculative
  % and its one of the things I explicity said we would not address.
...
%  %MRS2: but will there be more crowding?  Will depend on the equilibrium concentration 
%  %of the different components in the pore, I suppose.  
%  %MRS2: we may need to test this at some point, but it also would involve trying to figure out what the 
%  %equilibrium concentration inside the pores was.
%  \begin{itemize}
...
%  \end{itemize}
  ...
  % MRS2: recommendations are less important, as they are hypothetical. 
  % BJC2: condensed above section so that this is the only discussion of possible modifications.
  % BJC2: seems worth mentioning to make clear the value of our work.
