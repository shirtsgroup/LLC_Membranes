...
\begin{document}
  ...
  %MRS: may need to discuss the fact that the amount of water present in the tails may 
  %MRS: or may not be experimentally consistent; but if we don't put it in, the results will 
  %MRS: not be thermodynamically consistent, which will give issues with measurements and calculations.
  %MRS2: hopefully, some experiments could let us know about different diffusion regimes . . . 
...
%  described in our previous work. 
  ...
  \begin{itemize}
	\item The MSD for an anomalous diffusion process is of the form $$t^\alpha$$
	\item A value of $\alpha < 1$ tells us that the molecules exhibits subdiffusion
	\item Subdiffusive, non-ergodic processes with uncorrelated steps help distinguish 
	a CTRW from other anomalous subdiffusive process such as fraction brownian motion 
	and random walks on a fractal.
	\item See supporting info for proof of the system's non-ergodicty and uncorrelated steps.
  \end{itemize}

  We fit our data to a CTRW model by using our data in order to construct 
  dwell time and hop length distributions.
  \begin{itemize}
	...
	\item The distribution of dwell	times is expected to fit a heavy tailed distribution: 
  \end{itemize}
  ...
  \begin{itemize}
	...
	dwell time distribution, solutes exhibit Brownian motion. 
  ...
  We quantified the coordination of solutes with surrounding molecules and
  functional groups in two ways.
  \begin{itemize}
	\item By counting the number of coordinated molecules to a given solute
	based on a distance cut-off. Gives frame-by-frame information. 
	\item By averaging 3D spherical radial distribution function calculated for
	each solute.
  \end{itemize}
  ...
  \subsection{Macroscopic Diffusion Coefficients}

  \subsubsection{Simulated Predictions}\label{section:D_macro}
