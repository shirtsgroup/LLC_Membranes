...
%  	other is relatively deep in the head group region.
  ...
  \begin{figure}[!htb]
  \centering
  \includegraphics[width=0.45\textwidth]{ketone_rdf.pdf}
  \caption{}\label{fig:ketones_rdf}
  \end{figure}
  
  \subsection*{Transport of Thiols}
  
  We studied the transport properties of sulfur analogs of glycerol, ethylene glycol 
  and acetone.
...
%  to some of the solutes studied above.
  ...
  \begin{itemize}
    ...
    \item Conversely, Mercaptoethanol also exhibits some of the highest single solute MSDs % BJC: hypothesis about observation. Remains to be quantified
    \item It hydrogen bonds with head groups 6 times less frequently than ethylene glycol.
    % Dwell time as a function of hbonds?
    \item Some of this can be accounted for by the higher density of mercaptoethanol 
    molecules in the head group / tail region.
...
%  It can easily reform the second hydrogen bond in close proximity
  ...
  \begin{itemize}
    ...
    \item It also hydrogen bonds with water molecules which increases its stability in the pore region
    \item 2-3-Dimercapto-1-propanol preferentially hydrogen bonds with head groups so it gravitates
    towards the head group region.
  \end{itemize} 
  ...
  \begin{itemize}
  	...
  	\item There are 20 \% more acetone than DMSO molecules in the region beyond 0.8 nm from the pore center.  % based on actual number, not density
...
%    \item This may be due to the higher polarity and the pyramidal structure of DMSO.
%    \item DMSO is more stable in water than acetone and less stable in the head group region than acetone.
%    \item There is a peak in the DMSO radial density function in the tails which means some of them still
%    find their way back there and get trapped.
  \end{itemize}  
