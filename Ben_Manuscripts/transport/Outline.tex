...
%  \end{figure}
  ...
  \noindent A solute's size, shape, and polarity dictate the degree to which they 
  are influenced by each entrapment mechanism.
  \begin{itemize}
    \item In general, solutes can move fastest in the pore center, where
    there is comparatively little resistance to diffusion.
%    \item The smallest molecular weight solutes move fastest.
    \item Low molecular weight, highly water-soluble, hydrogen bond donating molecules spend the most time
    in the pore center where they are slowed down primarily by hydrogen bonds
    with monomer carboxylate groups.
    %BJC: this isn't fully accurate yet. Need to come back to this
    \item Solutes that cannot donate hydrogen bonds, or are less miscible in water, 
    tend to partition into the head group region where entrapment occurs. Most of
    the anti-correlated motion occurs here.
    \item The inverse relationship between solute size and mobility is disrupted
    by solutes' preference for water. 
    ...
    solubility.
    \item Small, planar molecules exhibit some of the shortest MSDs because
    
  	\item The well-established trend of decreasing solute mobility with 
  	size is not defied here, however 
  	...
  	by excessive trapping if there are many hydrogen bonds being formed.
...
%  specific groups of molecules in the discussion that follows.
  ...
  \subsection{Mean Squared Displacement Predictions}\label{section:msd_predictions}
