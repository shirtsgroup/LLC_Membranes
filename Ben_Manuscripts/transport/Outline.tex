...
%  \end{itemize}
  ...
  a hard spherical particle as a function inversely related to its radius.
  \begin{itemize}
  	\item $D = \dfrac{k_bT}{6\pi\eta r}$ where $k_b$ and $T$ are Boltzmann
  	constant and the system temperature respectively and $\eta$ is the 
  	system's viscosity.
  	\item For calculation relevant to this study, we assume that the viscosity
  	is constant.
  	\item Additionally, we will apply the relationship to solute MSDs which is
  	valid because all trajectories are of the same length.
  	\item It is well-known that the relationship breaks down when solute size becomes
  	on the order of solvent size because the solute can no longer be treated
  	as a non-interacting hard sphere. 
  \end{itemize}
  
  Gierer and Wirtz introduced a microfriction correction factor in order to 
  address this issue. 
  \begin{itemize}
...
%    that a spline is still justified.	
  ...
  \begin{itemize}
    ...
    Section~\ref{S-section:hbond_sensitivity} of the SI.
  ...
  normalized by the volume of the annular bin 
  to which they belong. The 5 wt \% system (a) appears to be much more crowded by monomers
  than the 10 wt \% system (b). Head groups in (a) are densest about 0.2 nm closer to the
  pore center than in (b). A small amount of tail components find their way close to the 
  pore center of the 5 wt \% system.
...
%  	solute and solvent are of comparable size.
  	...
  	for solutes with radius less than x nm, we fit the Stokes-Einstein relationship 
  	...
  	\item We believe methanol is subject to the least hindrance due to its 
  	size 
